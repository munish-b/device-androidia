...
// Default constructor
SurfaceSender::SurfaceSender() : mBufferLine(-1) {
  // Config
...
bool SurfaceSender::Start() {
  ...
  if (err < 0 && module == NULL) {
    HWCERROR(eCheckSurfaceSender,
             "Could not get gralloc module handle for surface %s",
             mProps.mSurfaceName);
  }
  ...
  err = mGralloc->lock(mGralloc, mBuffer->handle, GRALLOC_USAGE_SW_WRITE_MASK,
                       0, 0, mBuffer->width, mBuffer->height, &dstPtr);
  if (err) {
    HWCERROR(eCheckSurfaceSender,
...
bool SurfaceSender::Frame() {
  ...
  err = mGralloc->lock(mGralloc, mBuffer->handle, GRALLOC_USAGE_SW_WRITE_MASK,
                       0, 0, surfaceWidth, surfaceHeight, (void **)&dstPtr);
  if (err) {
