...
*
* File Name:            surface_sender.cpp
*
* Description:          A class to create surfaces and send them to
*                       SurfacFlinger see surface_sender.h for a full
*                       description.
*
*                       The body of the classes constructor are in
*                       surface_sender.h.
*
* Environment:
*
...
uint32_t SurfaceSender::GetPixelBytes(SPixelWord &pixel)
{
    ...
    HWCLOGI("%s: %s \n\t color: %x colorspace: %d", __FUNCTION__, mProps.mSurfaceName,
            mProps.mRGBAColor, mProps.mColorSpace);
    ...
    else if (mProps.mColorSpace == ecsRGB)
    {
        ...
        HWCLOGI("\t bytes[0-2]: %x %x %x", pixel.bytes[0], pixel.bytes[1], pixel.bytes[2]);
    }
    ...
    {
        HWCERROR(eCheckSurfaceSender, "Color Space %d not supported yet", mProps.mColorSpace);
        result = 1;
    }
...
void SurfaceSender::calculatePeriod()
{
    ...
    HWCLOGI("Surface %s Target frame period %lld", mProps.mSurfaceName, mTargetFramePeriod);
...
void SurfaceSender::calculateTargetUpdateTime()
{
    ...
    //HWCLOGI("Next update time for surface %s is %lld", mProps.mSurfaceName, mNextUpdateTime);
}
...
bool SurfaceSender::Start()
{
    ...
    if (err < 0 && module == NULL)
    {
        HWCERROR(eCheckSurfaceSender, "Could not get gralloc module handle for surface %s", mProps.mSurfaceName);
    }

    mGralloc = (struct gralloc_module_t *)module;


    // create surface and native window via SurfaceFlinger client
    mClient = new SurfaceComposerClient();
    mSurfaceControl = mClient->createSurface(
            String8((const char *)mProps.mSurfaceName), mProps.mWidth, mProps.mHeight,
            ecsRGB, 0);
    ...
    if (native_window_set_buffers_format(mWindow, mProps.mColorSpace) == -ENOENT)
    {
        HWCERROR(eCheckSurfaceSender, "native window set buffer format failed.");
    }

    const uint32_t bc = 3;
    err = native_window_set_buffer_count(mWindow, bc);
    if(err < 0)
    {
        HWCERROR(eCheckSurfaceSender, "SurfaceSender::Start(%s) - failed to set buffer count to %d", mProps.mSurfaceName, bc);
    }
    else
    {
        HWCERROR(eCheckSurfaceSender, "SurfaceSender::Start(%s) - set buffer count to %d", mProps.mSurfaceName, bc);
    }
    ...
    HWCLOGI("SurfaceSender %s: surface size =%dx%d colour %x\n"
    ...
    else
    {
        ...
        {
            case 1:
                mBackgroundPixel.bytes[3] = mBackgroundPixel.bytes[2] = mBackgroundPixel.bytes[1] = mBackgroundPixel.bytes[0];
                mForegroundPixel.bytes[3] = mForegroundPixel.bytes[2] = mForegroundPixel.bytes[1] = mForegroundPixel.bytes[0] = ~mBackgroundPixel.bytes[0];
                break;

            case 2:
                mBackgroundPixel.word16[1] = mBackgroundPixel.word16[0];
                mForegroundPixel.word16[1] = mForegroundPixel.word16[0] = ~mBackgroundPixel.word16[0];
                break;

            default:    // 3 or 4 bytes
                mForegroundPixel.word32 = mBackgroundPixel.word32 ^ 0xffffff; // Don't invert alpha which is in byte 3
                break;
        }
    ...
    if(mFence != -1)
    {
        ...
        if (err < 0)
        {
            HWCERROR(eCheckSurfaceSender, "SurfaceSender::Start(%s) - ERROR(%d): fence(%d) NEVER SIGNALLED", mProps.mSurfaceName, err, mFence);
    ...
    if (mProps.mColorSpace == ecsNV12YTiledIntel)
    {
        ...
        WriteNV12Chroma(mpBackgroundChromaNV12, mBackgroundPixel.chroma, mBuffer->width);
        WriteNV12Chroma(mpForegroundChromaNV12, mForegroundPixel.chroma, mBuffer->width);
    }
    else
    {
    ...
    err = mGralloc->lock(mGralloc, mBuffer->handle, GRALLOC_USAGE_SW_WRITE_MASK, 0, 0,
                         mBuffer->width, mBuffer->height, &dstPtr);
    if (err)
    {
        HWCERROR(eCheckSurfaceSender, "SurfaceSender::Start - Gralloc lock failed with err = %d", err);
    }
    else
    {
        unsigned char *ptr = (unsigned char *)dstPtr;
        FillBufferBackground(ptr);
        mGralloc->unlock(mGralloc, mBuffer->handle);
    }
...
void SurfaceSender::WritePixels(unsigned char *ptr, SPixelWord &currentPixel, unsigned numPixels)
{
    switch(mBytesPerPixel)
    {
    case 4:
...
void SurfaceSender::WriteNV12Chroma(unsigned char *ptr, uint32_t chroma, unsigned numPixels)
{
    numPixels >>= 2;
    for (uint32_t px = 0; px < numPixels; ++px)
    {
...
{
    //printf("%04d: PreFrame -- %s\n", int((android::elapsedRealtime() / 1000) % 20), mProps.mSurfaceName);
    if (mWindow->dequeueBuffer(mWindow, &mBuffer, &mFence))
    {
...
bool SurfaceSender::PostFrame()
{
    ...
    {
        HWCERROR(eCheckSurfaceSender, "SurfaceSender::PostFrame - Buffer unlock and post failed");
        return false;
    }
    return true;
}
...
bool SurfaceSender::Frame()
{
    ...
    if(mFence != -1)
    {
        ...
        if (err < 0)
        {
            HWCERROR(eCheckSurfaceSender, "SurfaceSender::Frame(%s) - ERROR(%d): fence(%d) NEVER SIGNALLED", mProps.mSurfaceName, err, mFence);
    ...
    else if (mProps.mLayer != epsStaticBackground)
    {
        ...
        mBufferLine.replaceValueFor(mBuffer->handle, mLine);
...
void SurfaceSender::DrawLine(unsigned lineNum, unsigned char *pBfr, unsigned char *pLineSrc)
{
    uint32_t stride         = mBuffer->stride * mBytesPerPixel;
    unsigned char *pLine;
...
void SurfaceSender::DrawLineNV12(unsigned lineNum, unsigned char *pBfr, unsigned char *pLineSrc, unsigned char *pNV12ChromaSrc)
{
    uint32_t surfaceHeight  = mBuffer->height;
    uint32_t stride         = mBuffer->stride * mBytesPerPixel;
    unsigned char *pLine;
...
bool SurfaceSender::End()
{
    ...
    mpBackgroundLine = mpForegroundLine = mpBackgroundChromaNV12 = mpForegroundChromaNV12 = NULL;
    return true;
}
