...
drmModeObjectPropertiesPtr DrmShimPropertyManager::ObjectGetProperties(
  ...
  return props;
#else /*When Kernel dont have support to pass the supported properties to \
         Userspace*/
  HwcTestKernel::ObjectClass cl = HwcTestKernel::eOther;
  if (mChecks) {
    cl = mChecks->GetObjectClass(objectId);
  } else {
    HWCLOGE_COND(eLogNuclear,
                 "DrmShimPropertyManager::ObjectGetProperties no test kernel");
  }

  if (cl == HwcTestKernel::eOther) {
    // Not a CRTC or plane, let's not spoof properties.
    HWCLOGV_COND(eLogNuclear,
                 "objectId %d is not CRTC or plane, no spoofed properties",
                 objectId);
  ...
  HWCLOGV_COND(
      eLogNuclear,
      "DrmShimPropertyManager::ObjectGetProperties copying %d real properties",
      props->count_props);
  ...
  HWCLOGV_COND(eLogNuclear,
               "DrmShimPropertyManager::ObjectGetProperties generating %d "
               "spoof properties",
               mNumSpoofProperties);
    ...
    if (cl == mInfo[i].mClass) {
      ...
      *pProp++ = propId;
      ...
      if (propId == eDrmPlaneProp_type) {
        ...
        propValues[propCount] =
            (objectId == 5) ? DRM_PLANE_TYPE_PRIMARY : DRM_PLANE_TYPE_OVERLAY;
      }
  ...
  drmModeObjectPropertiesPtr newProps =
      (drmModeObjectPropertiesPtr)fpDrmMalloc(sizeof(drmModeObjectProperties));
  ...
  HWCLOGV_COND(eLogNuclear,
               "DrmShimPropertyManager::ObjectGetProperties returning @%p",
               newProps);
  return newProps;
#endif
}
