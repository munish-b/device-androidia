...
File Name:      drm_shim.cpp

Description:    Drm shim implementation pass through and check functions.
...
// Local functions
static int TimeIoctl(int fd, unsigned long request, void *arg, int64_t& durationNs);
...
int                 (*fpDrmModeAddFB)(int fd, uint32_t width, uint32_t height,
...
int                 (*fpDrmModeDirtyFB)(int fd, uint32_t bufferId,
                                           drmModeClipPtr clips,
                                           uint32_t num_clips);

drmModeCrtcPtr      (*fpDrmModeGetCrtc)(int fd, uint32_t crtcId);

int                 (*fpDrmModeSetCrtc)(int fd, uint32_t crtcId,
                                        uint32_t bufferId,
                                        uint32_t x, uint32_t y,
                                        uint32_t *connectors, int count,
                                        drmModeModeInfoPtr mode);

int                 (*fpDrmModeSetCursor)(int fd, uint32_t crtcId,
                                            uint32_t bo_handle,
                                            uint32_t width, uint32_t height);

int                 (*fpDrmModeMoveCursor)(int fd, uint32_t crtcId,
                                            int x, int y);
...
int                 (*fpDrmModeConnectorSetProperty)(int fd,
...
int                 (*fpDrmModeCrtcSetGamma)(int fd, uint32_t crtc_id,
                                            uint32_t size, uint16_t *red,
                                            uint16_t *green, uint16_t *blue);

int                 (*fpDrmModeCrtcGetGamma)(int fd, uint32_t crtc_id,
                                            uint32_t size, uint16_t *red,
                                            uint16_t *green, uint16_t *blue);

int                 (*fpDrmModePageFlip)(int fd, uint32_t crtc_id,
...
int                 (*fpDrmModeSetPlane)(int fd, uint32_t plane_id,
                                        uint32_t crtc_id,
                                        uint32_t fb_id, uint32_t flags,
                                        DRM_COORD_INT_TYPE crtc_x,
                                        DRM_COORD_INT_TYPE crtc_y,
...
drmModeObjectPropertiesPtr (*fpDrmModeObjectGetProperties)(int fd,
                                                        uint32_t object_id,
                                                        uint32_t object_type);

void                (*fpDrmModeFreeObjectProperties)(
...
int             (*fpDrmGetInterruptFromBusID)(int fd, int busnum, int devnum,
                                                int funcnum);

int             (*fpDrmGetMap)(int fd, int idx, drm_handle_t *offset,
                                drmSize *size, drmMapType *type,
                                drmMapFlags *flags, drm_handle_t *handle,
                                int *mtrr);

int             (*fpDrmGetClient)(int fd, int idx, int *auth, int *pid,
...
int             (*fpDrmCommandRead)(int fd, unsigned long drmCommandIndex,
                                void *data, unsigned long size);
int             (*fpDrmCommandWrite)(int fd, unsigned long drmCommandIndex,
                                void *data, unsigned long size);
int             (*fpDrmCommandWriteRead)(int fd, unsigned long drmCommandIndex,
                                void *data, unsigned long size);
...
int             (*fpDrmAddMap)(int fd, drm_handle_t offset, drmSize size,
...
int             (*fpDrmAddBufs)(int fd, int count, int size,
...
int             (*fpDrmGetLock)(int fd, drm_context_t context,
                                drmLockFlags flags);
...
int             (*fpDrmAgpAlloc)(int fd, unsigned long size,
                                unsigned long type, unsigned long *address,
                                drm_handle_t *handle);

int             (*fpDrmAgpFree)(int fd, drm_handle_t handle);

int             (*fpDrmAgpBind)(int fd, drm_handle_t handle,
...
int             (*fpDrmOpenOnce)(void *unused, const char *BusID,
                                    int *newlyopened);
...
/// First DRM function call will result in drmShimInit(false, false) in non-HWC process
...
int drmShimInit(bool isHwc, bool isDrm)
{
    ...
    if (libraryIsInitialized == 0)
    {
        ...
        if (!libDrmHandle)
        {
            ...
            if (!libDrmHandle)
            {
                HWCERROR(eCheckDrmShimBind, "Failed to open real DRM in %s or %s", cLibDrmRealPath, cLibDrmRealVendorPath);
                result = -EFAULT;
                return result;
            }
        }
...
#define GET_FUNC_PTR(FN) \
            err = getFunctionPointer(   libDrmHandle,                                \
                                        "drm" #FN,                                   \
                                        (void **)&fpDrm##FN,                         \
                                        state);                                      \
            if (err)                                                                 \
...
static Hwcval::Statistics::CumFreqLog<float> addFbTimeStat("drmModeAddFb_duration", 1);
                     ...
                     uint32_t bo_handle, uint32_t *buf_id)
{
    ...
    HWCLOGV_COND(eLogDrm, "drmModeAddFB(fd=%u,width=%u,height=%u,depth=%u,bpp=%u,pitch=%u, bo_handle=%x, buf_id=%p)",
        fd, width, height, (int)depth,
        (int) bpp, pitch, bo_handle, buf_id);

    int ret;
    int64_t startTime = systemTime(SYSTEM_TIME_MONOTONIC);
    WRAPFUNC(ret = fpDrmModeAddFB, (fd, width, height, depth, bpp, pitch, bo_handle, buf_id));
    ...
    if (checks)
    {
        ...
        checks->CheckAddFB(fd, width, height, 0, depth, bpp,
                           bo_handles, pitches, offsets, *buf_id, 0, modifier, ret);
    }
                                    ...
                                    uint32_t *buf_id, uint32_t flags)
{
    ...
    // Checks are done on the IOCTL, because for BXT HWC has to issue the ioctl directly.
    WRAPFUNC(retval = fpDrmModeAddFB2, (fd, width, height, pixel_format, bo_handles, pitches, offsets, buf_id, flags));

    return retval;
}

static Hwcval::Statistics::CumFreqLog<float> rmFbTimeStat("drmModeRmFb_duration", 1);
                            ...
                            drmModeClipPtr clips, uint32_t num_clips)
{
    ...
    HWCLOGI("drmModeDirtyFB(fd=%u, bufferID=%x, num_clips=%u", fd, bufferId, num_clips);
                                        ...
                                        drmModeModeInfoPtr mode)
{
    ...
    if (checks)
    {
        checks->CheckSetCrtcEnter(fd, crtcId, bufferId, x, y, connectors,
                                  count, mode);
    }
    ...
    if (eventHandler != 0)
    {
        ...
        if (checks)
        {
            checks->GetCrcReader().SuspendCRCs(crtcId, HwcCrcReader::CRC_SUSPEND_MODE_CHANGE, true);
        }
    }

    int ret;
    WRAPFUNC(ret = fpDrmModeSetCrtc, (fd, crtcId, bufferId, x, y, connectors, count, mode));
...
int drmModeMoveCursor(int fd, uint32_t crtcId,
                                            int x, int y)
{
    CHECK_LIBRARY_INIT
                                        ...
                                        uint64_t value)
{
    ...
    WRAPFUNCRET(int, fpDrmModeConnectorSetProperty, (fd, connector_id, property_id, value));
}
...
int drmModeCrtcSetGamma(int fd, uint32_t crtc_id,
                                            uint32_t size, uint16_t *red,
                                            uint16_t *green, uint16_t *blue)
{
    CHECK_LIBRARY_INIT
    ...
    WRAPFUNCRET(int, fpDrmModeCrtcSetGamma, (fd, crtc_id, size, red, green, blue));
}

int drmModeCrtcGetGamma(int fd, uint32_t crtc_id,
                                            uint32_t size, uint16_t *red,
                                            uint16_t *green, uint16_t *blue)
{
    CHECK_LIBRARY_INIT
    ...
    WRAPFUNCRET(int, fpDrmModeCrtcGetGamma, (fd, crtc_id, size, red, green, blue));
                                        ...
                                        void *user_data)
{
    ...
    if (checks)
    {
        checks->checkPageFlipExit(fd, crtc_id, fb_id, flags, original_user_data, ret);
    }
...
int drmModeSetPlane(int fd, uint32_t plane_id,
                                        uint32_t crtc_id,
                                        uint32_t fb_id, uint32_t flags,
                                        DRM_COORD_INT_TYPE crtc_x,
                                        DRM_COORD_INT_TYPE crtc_y,
                                        ...
                                        )
{
    ...
    if (checks)
    {
        checks->checkSetPlaneEnter(fd, plane_id,
                                        crtc_id,
                                        fb_id, flags,
                                        crtc_x, crtc_y,
                                        crtc_w, crtc_h,
    ...
    int ret;
    {
        ...
        ret = fpDrmModeSetPlane(fd, plane_id, crtc_id, fb_id, flags,
                                    crtc_x, crtc_y,
    ...
    if (checks)
    {
        checks->checkSetPlaneExit(fd, plane_id,
                                        crtc_id,
                                        fb_id, flags,
                                        crtc_x, crtc_y,
                                        crtc_w, crtc_h,
                                ...
                                uint64_t value)
{
        ...
        if (prop)
        {
            ...
            *destPtr = '\0';
            ...
            if (strcmp(propName, "DPMS") == 0)
            {
                ...
                {
                    checks->CheckSetDPMS(object_id, value, eventHandler.get(), crtc, reenableDPMS);
                }
            }
#ifdef DRM_PFIT_PROP
            ...
            else if (strcmp(propName, DRM_SCALING_SRC_SIZE_PROP) == 0)
            {
                if (object_type == DRM_MODE_OBJECT_CONNECTOR)
                {
                    checks->CheckSetPanelFitterSourceSize(object_id, (value >> 16), (value & 0xffff));
                }
            }
#endif
    ...
    int status;
    {
        ...
        WRAPFUNC(status = fpDrmModeObjectSetProperty, (fd, object_id, object_type, property_id, value));
    }
    int64_t durationNs = systemTime(SYSTEM_TIME_MONOTONIC) - startTime;

    if (status != 0)
    {
        HWCERROR(eCheckDrmCallSuccess, "drmModeObjectSetProperty %s failed %d", propName, status);
    }
    HWCCHECK(eCheckDrmCallSuccess);

    if (checks && (strcmp(propName, "DPMS") == 0))
    {
        checks->CheckSetDPMSExit(fd, crtc, reenableDPMS, eventHandler.get(), status);
    }
    ...
    if (durationNs > 1000000)
    {
        ...
        if (durationMs > 10.0)
        {
            HWCERROR(eCheckDrmSetPropLatencyX, "drmModeObjectSetProperty %s took %fms", propName, durationMs);
        }
        else
        {
            HWCERROR(eCheckDrmSetPropLatency, "drmModeObjectSetProperty %s took %fms", propName, durationMs);
        }
    }
...
{
#define DECODE_DRM(REQUEST) if (request == REQUEST) {return #REQUEST;}
    DECODE_DRM(DRM_IOCTL_I915_INIT)
    else DECODE_DRM(DRM_IOCTL_I915_FLUSH)
    else DECODE_DRM(DRM_IOCTL_I915_FLIP)
    else DECODE_DRM(DRM_IOCTL_I915_BATCHBUFFER)
    else DECODE_DRM(DRM_IOCTL_I915_IRQ_EMIT)
    else DECODE_DRM(DRM_IOCTL_I915_IRQ_WAIT)
    else DECODE_DRM(DRM_IOCTL_I915_GETPARAM)
    else DECODE_DRM(DRM_IOCTL_I915_SETPARAM)
    else DECODE_DRM(DRM_IOCTL_I915_ALLOC)
    else DECODE_DRM(DRM_IOCTL_I915_FREE)
    else DECODE_DRM(DRM_IOCTL_I915_INIT_HEAP)
    else DECODE_DRM(DRM_IOCTL_I915_CMDBUFFER)
    else DECODE_DRM(DRM_IOCTL_I915_DESTROY_HEAP)
    else DECODE_DRM(DRM_IOCTL_I915_SET_VBLANK_PIPE)
    else DECODE_DRM(DRM_IOCTL_I915_GET_VBLANK_PIPE)
    else DECODE_DRM(DRM_IOCTL_I915_VBLANK_SWAP)
    else DECODE_DRM(DRM_IOCTL_I915_HWS_ADDR)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_INIT)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_EXECBUFFER)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_EXECBUFFER2)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_PIN)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_UNPIN)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_BUSY)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_SET_CACHEING)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_GET_CACHEING)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_THROTTLE)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_ENTERVT)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_LEAVEVT)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_CREATE)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_PREAD)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_PWRITE)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_MMAP)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_MMAP_GTT)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_SET_DOMAIN)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_SW_FINISH)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_SET_TILING)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_GET_TILING)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_GET_APERTURE)
    else DECODE_DRM(DRM_IOCTL_I915_GET_PIPE_FROM_CRTC_ID)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_MADVISE)
    else DECODE_DRM(DRM_IOCTL_I915_OVERLAY_PUT_IMAGE)
    else DECODE_DRM(DRM_IOCTL_I915_OVERLAY_ATTRS)
    else DECODE_DRM(DRM_IOCTL_I915_SET_SPRITE_COLORKEY)
    else DECODE_DRM(DRM_IOCTL_I915_GET_SPRITE_COLORKEY)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_WAIT)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_CONTEXT_CREATE)
    else DECODE_DRM(DRM_IOCTL_I915_GEM_CONTEXT_DESTROY)
    else DECODE_DRM(DRM_IOCTL_I915_REG_READ)
    ...
    else DECODE_DRM(DRM_IOCTL_GEM_OPEN)
    else DECODE_DRM(DRM_IOCTL_GEM_FLINK)
    else DECODE_DRM(DRM_IOCTL_GEM_CLOSE)
#ifdef DRM_IOCTL_I915_EXT_IOCTL
    else DECODE_DRM(DRM_IOCTL_I915_EXT_IOCTL)
    else DECODE_DRM(DRM_IOCTL_I915_EXT_USERDATA)
#endif
...
static Hwcval::Statistics::CumFreqLog<float> flipRequestTimeStat("flip_request_duration", 1);
...
int drmIoctl(int fd, unsigned long request, void *arg)
{
    ...
    if (checks)
    {
        if (request == DRM_IOCTL_I915_SET_PLANE_180_ROTATION)
        {
            struct drm_i915_plane_180_rotation* rot = (struct drm_i915_plane_180_rotation*) arg;
            checks->CheckIoctlI915SetPlane180Rotation(rot);
        }
        else if (request == DRM_IOCTL_I915_RESERVED_REG_BIT_2)
        {
...
#else
        else if (request == DRM_IOCTL_MODE_SETDISPLAY)
        {
        ...
        {
            struct  drm_i915_set_plane_zorder* ord = (struct drm_i915_set_plane_zorder*) arg;
        ...
        else if (request == DRM_IOCTL_I915_EXT_IOCTL)
        {
            ...
            if (data)
            {
                HWCLOGD_COND(eLogAllIoctls, "fd %d Extended IOCTL %s arg 0x%x", fd, DrmDecode(data->sub_cmd), data->args_ptr);
            }
            else
            {
        ...
        {
            HWCLOGI_COND(eLogAllIoctls, "fd %d Ioctl request %s", fd, DrmDecode(request));
        }
    }
    ...
    if (testKernel)
    {
        ...
        {
            struct drm_i915_disp_screen_control* ctrl = (struct drm_i915_disp_screen_control*) arg;
        ...
        else if (request == DRM_IOCTL_MODE_ADDFB2)
        {
            ...
            HWCLOGV_COND(eLogDrm, "drmModeAddFB2(fd=%u,width=%u,height=%u,pixel_format=0x%x, bo_handles=(%x %x %x %x), ",
                fd, addFb2->width, addFb2->height, addFb2->pixel_format,
                addFb2->handles[0], addFb2->handles[1], addFb2->handles[2], addFb2->handles[3]);
            HWCLOGV_COND(eLogDrm, "  pitches=(%d %d %d %d), offsets=(%d %d %d %d), flags=%x",
                addFb2->pitches[0], addFb2->pitches[1], addFb2->pitches[2], addFb2->pitches[3],
                addFb2->offsets[0], addFb2->offsets[1], addFb2->offsets[2], addFb2->offsets[3], addFb2->flags);

#ifndef DRM_MODE_FB_MODIFIERS
            // Some old platforms (e.g. MCGR5.1) do not have support for modifiers. In this
            // case, bypass the checks.
            __u64 dummyModifier[] = { 0, DrmShimPlane::ePlaneYTiled, 0, 0 };
#endif

            checks->CheckAddFB(fd,
                               addFb2->width,
                               addFb2->height,
                               addFb2->pixel_format,
                               0,
        ...
        else if (request == DRM_IOCTL_I915_GETPARAM)
        {
            if (arg)
            {
                ...
                if (params && (params->param == I915_PARAM_CHIPSET_ID) && params->value)
                {
...
static int TimeIoctl(int fd, unsigned long request, void *arg, int64_t& durationNs)
{
    ...
    if (status != 0)
    {
        HWCLOGD_COND(eLogAllIoctls, "fd %d Ioctl %s return status 0x%x=%d", fd, DrmDecode(request), status, status);
    }
...
static void IoctlLatencyCheck(unsigned long request, int64_t durationNs)
{
    ...
    if (durationNs > 1000000)
    {
        ...
        // For GEM WAIT, we are waiting for rendering to complete, which could take a very long time
        ...
        {
            // We know these sometimes take a long time, but we don't know what they are for, so don't generate
            // errors
            HWCLOGW_COND(eLogDrm, "drmIoctl %s took %fms", drmName, durationMs);
        }
        else if (request == DRM_IOCTL_I915_GEM_EXECBUFFER2)
        {
            // This request should not take a long time, but when using the harness it often does.
            // This is believed to be something to do with the fact that we are filling the buffers
            // from the CPU rather than the GPU.
            // Correct fix is to use some form of GPU composition, perhaps by invoking the GLComposer
            // directly from the harness.
            // Incidentally, using -no_fill does not help even though this means we never
            // access the buffers from the CPU. Gary says this introduces different optimizations
            // in the kernel which will assume that it is a blanking buffer.
            //
            // So, only log the warning, not the error.
            HWCERROR(eCheckDrmIoctlLatency, "drmIoctl %s took %fms", drmName, durationMs);
        }
#if HWCVAL_HAVE_ZORDER_API
        else if (request == DRM_IOCTL_I915_SET_PLANE_ZORDER)
        {
            // Add to the log as this will help diagnose flicker, which is the error we are interested in
        ...
        else
        {
            if (durationMs > 10.0)
            {
                HWCERROR(eCheckDrmIoctlLatencyX, "drmIoctl %s took %fms", drmName, durationMs);
            }
            else
            {
                HWCERROR(eCheckDrmIoctlLatency, "drmIoctl %s took %fms", drmName, durationMs);
                    ...
                    int *mtrr)
{
    ...
    WRAPFUNCRET(int, fpDrmGetMap, (fd, idx, offset, size, type, flags, handle, mtrr));
...
int drmAddBufs(int fd, int count, int size,
                    drmBufDescFlags flags, int agp_offset)
{
    CHECK_LIBRARY_INIT
...
int drmMap(int fd, drm_handle_t handle, drmSize size,
                        drmAddressPtr address)
{
    CHECK_LIBRARY_INIT
...
int drmAgpAlloc(int fd, unsigned long size,
                    unsigned long type, unsigned long *address,
                    drm_handle_t *handle)
{
    CHECK_LIBRARY_INIT
...
int drmAgpBind(int fd, drm_handle_t handle,
                unsigned long offset)
{
    CHECK_LIBRARY_INIT
                            ...
                            unsigned long *next_key, void **next_value)
{
    ...
    WRAPFUNCRET(int, fpDrmSLLookupNeighbors, (l, key, prev_key, prev_value, next_key, next_value));
