...
HwcTimeline::HwcTimeline()
    : mpchName(NULL), mSyncTimeline(-1), mCurrentTime(0), mNextFutureTime(0) {
}
...
bool HwcTimeline::createFence(Fence* pFence) {
  ...
  Fence newFence =
      sw_sync_fence_create(mSyncTimeline, fenceName, mNextFutureTime);
  if (newFence < 0) {
    HWCERROR(eCheckFenceAllocation,
             "HwcTimeline %d : Failed to create new fence [%s] : %s",
             mSyncTimeline, fenceName, strerror(errno));
    return false;
  }

  if (*pFence < 0) {
    HWCLOGD_IF(SYNC_FENCE_DEBUG,
               "SyncTimeline %d(%s) : Created new fence %d(%s)", mSyncTimeline,
               mpchName, newFence, fenceName);
    *pFence = newFence;
    return true;
  }
...
bool HwcTimeline::mergeFence(Fence* pFence, Fence* pOtherFence) {
  ...
  if (SYNC_FENCE_DEBUG) {
    ...
    if (pInfo1) {
      HWCLOGD_IF(pInfo1, "Fence Info1: %s status %d", pInfo1->name,
                 pInfo1->status);
    ...
    if (pInfo2) {
      HWCLOGD_IF(pInfo2, "Fence Info2: %s status %d", pInfo2->name,
                 pInfo2->status);
      struct sync_pt_info* pSyncPointInfo = NULL;
    ...
    if (pInfo1 && pInfo2) {
      ...
      snprintf(fenceName, sizeof(fenceName), "[%s && %s]", pInfo1->name,
               pInfo2->name);
    } else {
      // Create a combined Fence name from handles.
      snprintf(fenceName, sizeof(fenceName), "[F%d && F%d]", *pFence,
               *pOtherFence);
    }
    if (pInfo1)
  ...
  if (mergedFence < 0) {
    HWCERROR(eCheckFenceAllocation, "Failed to merge fence [%s] : %s",
             fenceName, strerror(errno));
    return false;
  } else {
    // Close the two component fences for the merge.
    HWCLOGD_IF(SYNC_FENCE_DEBUG, "HwcTimeline : Merged fence %d(%s)",
               mergedFence, fenceName);
    CloseFence(*pFence);
...
void HwcTimeline::advance(uint32_t ticks) {
  ...
  HWCLOGD_IF(SYNC_FENCE_DEBUG, "Release mSyncTimeline %d(%s) +%u",
             mSyncTimeline, mpchName, ticks);

  int err = sw_sync_timeline_inc(mSyncTimeline, ticks);
  mCurrentTime += ticks;

  if (err < 0) {
    HWCERROR(eCheckInternalError, "Failed to advance sync timeline %d(%s)",
             mSyncTimeline, mpchName);
  }
}
...
void HwcTimeline::advanceTo(uint32_t absSync) {
  ...
  if (delta > 0) {
    HWCLOGD_IF(SYNC_FENCE_DEBUG, "advanceTo( %u ) mCurrentTime %u => delta %u",
               absSync, mCurrentTime, delta);
...
void HwcTimeline::dumpTimeline(const char* pchPrefix) {
  HWCLOGD_IF(SYNC_FENCE_DEBUG, "%s - SyncTimeline %d(%s) mNextFutureTime %u",
             pchPrefix ? pchPrefix : "", mSyncTimeline, mpchName,
             mNextFutureTime);
}
...
void HwcTimeline::dumpFence(Fence fence, const char* pchPrefix) {
  if (SYNC_FENCE_DEBUG) {
    if (fence >= 0) {
      ...
      HWCLOGD("%s - Info: %s status %d", pchPrefix ? pchPrefix : "",
              pInfo ? pInfo->name : "n/a", pInfo ? pInfo->status : -1);
