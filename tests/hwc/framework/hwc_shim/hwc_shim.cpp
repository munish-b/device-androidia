...
void HwcShim::GetDisplayInfo() {
  ...
  for (int i = 0; i < gfx_resources->count_connectors; i++) {
    ...
    HWCLOGI("connector %d\t%d\t%d\t%d\t%dx%d\t\t%d\t%d\n",
            ...
            connector->connection, connector->connector_type,
            connector->mmWidth, connector->mmHeight, connector->count_modes,
    ...
    encoder =
        drmModeGetEncoder(gfx_fd, connector->encoders[connector->encoder_id]);

    HWCLOGI("encoder %d: %d, %d, %d", connector->encoder_id,
    ...
    for (int j = 0; j < connector->count_modes; j++) {
      ...
      HWCLOGI("mode  %s %d %d %d %d %d %d %d %d %d 0x%x 0x%x %d\n", mode->name,
              ...
              mode->vsync_end, mode->vtotal, mode->flags, mode->type,
              mode->clock);
  ...
  for (int c = 0; c < gfx_resources->count_crtcs; c++) {
    ...
    if (crtc == NULL) {
      HWCLOGI("could not get crtc %i", gfx_resources->crtcs[c]);
      continue;
    }

    HWCLOGI("%d\t%d\t(%d,%d)\t(%dx%d)", crtc->crtc_id, crtc->buffer_id, crtc->x,
  ...
  for (uint32_t p = 0; p < plane_resources->count_planes; p++) {
    ...
    HWCLOGI("%d\t%d\t%d\t%d,%d\t\t%d,%d\t%d", ovr->plane_id, ovr->crtc_id,
            ovr->fb_id, ovr->crtc_x, ovr->crtc_y, ovr->x, ovr->y,
            ovr->gamma_size);
...
int HwcShim::OnQuery(int what, int *value) {
  int ret =
      -1;  // hwc_composer_device->query(hwc_composer_device, what, value);
  return ret;
}
