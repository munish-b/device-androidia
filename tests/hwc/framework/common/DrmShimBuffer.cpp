...
void LogProtection(int priority, buffer_handle_t handle, const char* desc)
{
    ...
    HWCLOG_COND(priority, eLogIvp, "iVP %s: Handle %p %s is_encrypted %d pavp_session_id %d pavp_instance_id %d",
        desc, handle, ((bi.usage & GRALLOC_USAGE_PROTECTED) ? "PROT" : ""),
...
DrmShimBuffer::DrmShimBuffer(buffer_handle_t handle, Hwcval::BufferSourceType bufferSource)
 :  mHandle(handle),
    mDsId(0),
    mAcquireFenceFd(-1),
    mNew(true),
...
DrmShimBuffer::~DrmShimBuffer()
{
    ...
    --mCount;
    HWCLOGD_COND(eLogBuffer, "DrmShimBuffer::~DrmShimBuffer Deleted buf@%p", this);
}
...
DrmShimBuffer* DrmShimBuffer::SetCompositionTarget(Hwcval::BufferSourceType bufferSource)
{
    mBufferSource = bufferSource;
    return this;
}
...
{
    return ((mBufferSource != Hwcval::BufferSourceType::Input) && (mBufferSource != Hwcval::BufferSourceType::Hwc));
}
...
DrmShimBuffer* DrmShimBuffer::SetDetails(const Hwcval::buffer_details_t& details)
{
    mDetails = details;
    UpdateMediaDetails();
    UpdateResolveDetails();

    // Normally buffer protection state is unknown. This will be ovewritten if this is an iVP target.
    mShouldBeProtected = eProtDontCare;
    return this;
}
...
// On older builds, this will be "name" - actually a number and a system-global ID
...
DrmShimBuffer* DrmShimBuffer::UpdateMediaDetails()
{
    ...
    if (mHandle != 0)
    {
      ...
      if (1 /*GetGralloc().queryMediaDetails(mHandle, &mMediaDetails) != 0*/) {
        ...
        {
            HWCLOGV_COND(eLogProtectedContent, "DrmShimBuffer::updateMediaDetails queryMediaDetails handle %p -> session %d instance %d",
      ...
      mResolveDetails.magic = sizeof(hwc_buffer_resolve_details_t);
        ...
        else
        {
            HWCLOGV_COND(eLogRenderCompression,
                "DrmShimBuffer::UpdateResolveDetails getBufferResolveDetails handle %p state %s hint %s", mHandle,
                mResolveDetails.state == INTEL_UFO_BUFFER_STATE_AUX_DISABLED ? "AUX_DISABLED" :
                mResolveDetails.state == INTEL_UFO_BUFFER_STATE_NO_CONTENT ? "NO_CONTENT" :
                mResolveDetails.state == INTEL_UFO_BUFFER_STATE_COMPRESSED ? "STATE_COMPRESSED" : "UNKNOWN",

                mResolveDetails.hint == INTEL_UFO_BUFFER_HINT_RC_UNDEFINED ? "NONE" :
                mResolveDetails.hint == INTEL_UFO_BUFFER_HINT_RC_FULL_RESOLVE ? "FULL RESOLVE" :
                mResolveDetails.hint == INTEL_UFO_BUFFER_HINT_RC_PARTIAL_RESOLVE ? "PARTIAL (NO CC) RESOLVE" :
                mResolveDetails.hint == INTEL_UFO_BUFFER_HINT_RC_DISABLE_RESOLVE ? "NO RESOLVE (RC AND CC)" :
                mResolveDetails.hint == INTEL_UFO_BUFFER_HINT_MMC_COMPRESSED ? "MMC COMPRESSED" : "UNKNOWN" );
        }
    }
    else
    {
...
// This function returns true if the buffer's media details state that the buffer is encrypted
// However, this is not always a correct reflection of whether the buffer is actually encrypted
// because iVP has a tendency to clear this flag in the composition target, even when the buffer actually is
// encrypted (and we know that because one of the composition sources is encrypted).
bool DrmShimBuffer::HasMediaDetailsEncrypted()
{
    char strbuf[HWCVAL_DEFAULT_STRLEN];
    bool ret = (mMediaDetails.is_encrypted  != 0);
    HWCLOGD_COND(eLogProtectedContent, "DrmShimBuffer::HasMediaDetailsEncrypted %s => %s",
...
bool DrmShimBuffer::UpdateDetails()
{
    ...
    if (mHandle)
    {
        ...
        {
            HWCLOGW( "DrmShimBuffer::UpdateDetails can't update info for buf@%p handle %p", this, mHandle);

            // This probably means that the handle is no longer valid.
            return false;
        }

        if ((bi.allocWidth != mDetails.allocWidth) || (bi.allocHeight != mDetails.allocHeight))
        {
            HWCLOGD("DrmShimBuffer::UpdateDetails %s alloc %dx%d updated to %dx%d",
                IdStr(strbuf), mDetails.allocWidth, mDetails.allocHeight, bi.allocWidth, bi.allocHeight);
        }

        if ((bi.width != mDetails.width) || (bi.height != mDetails.height))
        {
            HWCLOGD("DrmShimBuffer::UpdateDetails %s size %dx%d updated to %dx%d",
                IdStr(strbuf), mDetails.width, mDetails.height, bi.width, bi.height);
        }
...
int DrmShimBuffer::GetCurrentGlobalId()
{
    ...
    if (mHandle)
    {
        if (GetBufferInfo(mHandle, &bi))
        {
            HWCLOGW( "DrmShimBuffer::UpdateDetails can't update " BUFIDSTR " for buf@%p handle %p", this, mHandle);
...
// Recursively expand a transform using the "combined from" lists in its constituent DrmShimBuffer.
// The result will be a list of all the constituent transforms that should align with the original layer list.
void DrmShimBuffer::AddSourceFBsToList(DrmShimSortedTransformVector& list, DrmShimTransform& thisTransform, uint32_t sources)
{
    ...
    HWCLOGV_COND(eLogCombinedTransform, "DrmShimBuffer::AddSourceFBsToList Enter: transform@%p, buf@%p, sources 0x%x",
                 &thisTransform, thisTransform.GetBuf().get(), sources);

    if (mCombinedFrom.size() > 0)
    {
        HWCLOGV_COND(eLogCombinedTransform, "%s transform@%p adding srcs %s:",
            IdStr(str), &thisTransform, DrmShimTransform::SourcesStr(sources, strbuf2));
        for (uint32_t i=0; i<mCombinedFrom.size(); ++i)
        {
            DrmShimTransform& transform = mCombinedFrom.editItemAt(i);
            DrmShimTransform combinedTransform(transform, thisTransform, eLogCombinedTransform);
            transform.GetBuf()->AddSourceFBsToList(list, combinedTransform, sources);
        }
        HWCLOGV_COND(eLogCombinedTransform, "DrmShimBuffer::AddSourceFBsToList Exit: transform@%p, buf@%p, sources 0x%x",
                     &thisTransform, thisTransform.GetBuf().get(), sources);
        return;
    }
    ...
    if (HWCCOND(eLogCombinedTransform))
    {
        ...
        HWCLOGV("  Adding original %s transform@%p->%p pos %d/%d srcs: %s.",
            IdStr(str), &thisTransform, &copyTransform, pos, list.size(), DrmShimTransform::SourcesStr(sources, strbuf2));
        ...
        for (uint32_t i=0; i<list.size(); ++i)
        {
            ...
            if (s >= tmp.size())
            {
                HWCERROR(eCheckInternalError, "TRANSFORM MISMATCH: TOO MANY TRANSFORMS IN RESULT");
                continue;
            }
            ...
            {
                HWCERROR(eCheckInternalError, "TRANSFORM MISMATCH: RESULT CONTAINS buf@%p NOT IN SOURCE", tr.GetBuf().get());
                continue;
            }
        ...
        if (s+1 < tmp.size())
        {
            HWCERROR(eCheckInternalError, "TRANSFORM MISMATCH: NOT ALL SOURCES COPIED");
        }
    }
...
void DrmShimBuffer::SetAllCombinedFrom(const DrmShimTransformVector& combinedFrom)
{
    HWCLOGD("SetAllCombinedFrom: buf@%p handle %p combined from %d transforms", this, mHandle, combinedFrom.size());
...
DrmShimBuffer* DrmShimBuffer::SetLastHwcFrame(Hwcval::FrameNums fn, bool isOnSet)
{
    mLastHwcFrame = fn;
...
// This function determines whether a buffer is still 'current', i.e. the content is unchanged.
// HWC (from HWC 2.0 changes onwards) will do this by looking to see when the reference count of the buffer
// goes to zero.
//
// That is too complex for us right now. So we are looking to see if the buffer was used in the last frame
// on ANY of the displays.
//
// I suspect that this will need some work for HWC 2.0 to get it fully working. For example if one of the
...
const char* DrmShimBuffer::GetHwcFrameStr(char* str, uint32_t len)
{
    ...
    {
        snprintf(str, len, "frame:%d.%d.%d", mLastHwcFrame[0], mLastHwcFrame[1], mLastHwcFrame[2]);
    }
...
void DrmShimBuffer::SetBufCopy(android::sp<android::GraphicBuffer>& buf)
{
    if ((mBufCpy.get() == 0) && buf.get())
    {
        ...
        if (sNumBufCopies > 10)
        {
            HWCLOGI("%d copies of buffers stored for transparency filter detection", sNumBufCopies);
        }
    }
...
// This logs out all the interesting identification info including gralloc handle, buffer objects and framebuffer IDs.
char* DrmShimBuffer::IdStr(char* str, uint32_t len) const
{
    uint32_t n = snprintf(str, len, "buf@%p handle %p " BUFIDSTR " 0x%x ", this, mHandle, GetGlobalId());
    if (n>=len) return str;
    ...
    if (HWCCOND(eLogBuffer))
    {
        for (uint32_t i=0; i<mBos.size(); ++i)
        {
            ...
            // If the bo's reverse pointer to the buffer is wrong, log out what it actually points to.
    ...
    if (HWCVAL_MAX_CRTCS >= 3)
    {
        n += snprintf(str+n, len - n, " (last seen %s)", android::String8(mLastOnSetFrame).string());
    }
...
void DrmShimBuffer::ReportStatus(int priority, const char* str)
{
    ...
    if (HwcGetTestConfig()->IsLevelEnabled(priority))
    {
        HWCLOG(priority, "%s: %s %s %s %s", str, IdStr(strbuf),
            mUsedByIvp?"+UsedByIvp" : "-UsedByIvp",
            GetSourceName(),
            mBlanking?"+Blanking":"-Blanking"
             );
        HWCLOG(priority, "  Size %dx%d Alloc %dx%d DrmFormat %x Usage %x",
#ifdef HWCVAL_FB_BUFFERINFO_FORMAT
            mDetails.width, mDetails.height, mDetails.allocWidth, mDetails.allocHeight, mDetails.fb_format, mDetails.usage);
#else
            mDetails.width, mDetails.height, mDetails.allocWidth, mDetails.allocHeight, mDetails.drmformat, mDetails.usage);
#endif
...
// The transparency state is cached so that the determination is done at most once for each buffer
// This checking should only be done for buffers that are in front of a NV12 layer and have remained in the layer list
// for a long time.
bool DrmShimBuffer::IsBufferTransparent(const hwc_rect_t& rect)
{
    ...
    HWCLOG(logLevel, "DrmShimBuffer::IsBufferTransparent entry buf@%p handle %p %s rect(%d, %d, %d, %d)", this, mHandle,
        (mBufferContent == BufferContentType::ContentNull) ? "Null" : ((mBufferContent == BufferContentType::ContentNotNull) ? "Not Null" : ""),
    ...
    if (mBufferContent == BufferContentType::ContentNotTested)
    {
        ...
        if (HasBufCopy())
        {
            // We MUST query the buffer copy for details rather than just using mDetails,
            // because it will have a different pitch to the original buffer seeing as we have
            ...
            if (GetBufferInfo(mBufCpy->handle, &bi))
            {
                HWCERROR(eCheckGrallocDetails,
                         "DrmShimBuffer::IsBufferTransparent can't get info for buf@%p handle %p copy %p",
                         this, mHandle, mBufCpy->handle);

                return false;
            }

            mBufferContent = IsBufferTransparent(pGralloc, bi, mBufCpy->handle, rect) ? BufferContentType::ContentNull : BufferContentType::ContentNotNull;
        }
        else
        {
    ...
    HWCLOG(logLevel, "DrmShimBuffer::IsBufferTransparent exit buf@%p handle%p %s", this, mHandle,
        (mBufferContent == BufferContentType::ContentNull) ? "Null" : ((mBufferContent == BufferContentType::ContentNotNull) ? "Not Null" : ""));

    return (mBufferContent == BufferContentType::ContentNull);
}

// Static function to read a buffer and find out if it is empty within a certain rectangle.
bool DrmShimBuffer::IsBufferTransparent(struct gralloc_module_t* gralloc, Hwcval::buffer_details_t& bi, buffer_handle_t handle, const hwc_rect_t& rect)
{
    if ((bi.height < rect.bottom) || (bi.width < rect.right))
    {
        HWCLOGD("video (%d, %d, %d, %d) overlay %dx%d", rect.left, rect.top, rect.right, rect.bottom, bi.width, bi.height);
        HWCLOGD("DrmShimBuffer::IsBufferTransparent: Not fully covering video: handle %p", handle);
        return false;
    }

    uint8_t* data = 0;
    int err = gralloc->lock(gralloc, handle, GRALLOC_USAGE_SW_READ_OFTEN, 0, 0,
           0, 0, (void **)&data);

    if (err || (data == 0))
    {
        HWCLOGW("DrmShimBuffer::IsBufferTransparent: Gralloc lock of real buffer handle %p failed with err=%d", handle, err);
        return false;
    }
    ...
    for (int i=rect.top; i<rect.bottom; ++i)
    {
        ...
        for (j=0; j<((lineWidthBytes-7) / 8); ++j)
        {
            ...
            if (p[j] != 0)
            {
                HWCLOGD("DrmShimBuffer::IsBufferTransparent: Not null line %d doubleword %d 0x%llx", i, j, p[j]);
                ret = false;
                break;
            }
        }
        ...
        for (; j<lineWidthBytes; ++j)
        {
            if (pbyte[j] != 0)
            {
                HWCLOGD("DrmShimBuffer::IsBufferTransparent: Not null line %d byte %d", i, j);
                ret = false;
                break;
            }
        }
    ...
    HWCLOGD("DrmShimBuffer::IsBufferTransparent tested %d doublewords. linewidthbytes=%d (%d, %d, %d, %d)", n, lineWidthBytes, rect.left, rect.right, rect.top, rect.bottom);
...
void DrmShimBuffer::ReportCompositionMismatch(uint32_t lineWidthBytes,
    ...
    double SSIMIndex, unsigned char* cpyData, unsigned char* refData)
{
    ...
    HWCERROR(IsFbt() ? eCheckSfCompMatchesRef : eCheckHwcCompMatchesRef,
        "CompareWithRef: Composition mismatch %d with real buffer handle %p from frame:%d at line %d",
        mCompMismatchCount, mHandle, mLastHwcFrame, mismatchLine);
    // NB %% does not work
    HWCLOGE("  -- %2.6f%%%% of bytes mismatch; RMS = %3.6f; SSIM index = %f (frame:%d)",
    ...
    {
        HwcTestDumpGrallocBufferToDisk("real", mCompMismatchCount, mBufCpy->handle, DUMP_BUFFER_TO_TGA);
        HwcTestDumpGrallocBufferToDisk("ref", mCompMismatchCount, mRef->handle, DUMP_BUFFER_TO_TGA);
    }
...
bool DrmShimBuffer::CompareWithRef(bool useAlpha, hwc_rect_t* rectToCompare)
{
    ...
    if (mRef.get() == 0)
    {
        HWCERROR(eCheckInternalError, "CompareWithRef: %s NO REF!!",
            IdStr(strbuf));
        return false;
    }
    else
    ...
    HWCLOGD("CompareWithRef: Comparing real %p ref %p (%d, %d) %dx%d Pitch %d Bytes Per Pixel %d",
        cpyData, refData, left, top, width, height, cpyBi.pitch, bytesPerPixel);

    bool same = true;
    for (uint32_t i=0; i<height; ++i)
    {
        uint8_t* realDataLine = cpyData + ((i + top) * cpyBi.pitch) + (left * bytesPerPixel);
        uint8_t* refDataLine = refData + ((i + top) * refBi.pitch) + (left * bytesPerPixel);
    ...
    if (!same)
    {
        ...
        for (uint32_t i = 0; i < height; ++i)
        {
            ...
            BufRowPointers[i] = cpyData + ((i + top) * cpyBi.pitch) + (left * bytesPerPixel);
        }

        for (uint32_t i = 0; i < height; ++i)
        {
            // assign pointer for row
            RefRowPointers[i] = refData + ((i + top) * refBi.pitch) + (left * bytesPerPixel);
        }
        ...
        DoSSIMCalculations(dinf, (dssim_rgba**)BufRowPointers,
                                 (dssim_rgba**)RefRowPointers,
                                 width, height, blur_type,
                                 hasAlpha);
        ...
        HWCLOGD("SSIM indices per channel: %f %f %f",
            channelResult[0], channelResult[1], channelResult[2]);
        ...
        {
            ReportCompositionMismatch(lineWidthBytes, cpyBi.pitch, refBi.pitch, SSIMIndex, cpyData, refData);
        }
        else
        {
            HWCLOGI("CompareWithRef: %s: Comparison passed with SSIM Index = %.6f (frame:%d)", IdStr(strbuf), SSIMIndex, mLastHwcFrame);
        }
    }
    else
    {
...
Hwcval::ValidityType DrmShimBuffer::IsProtectionValid(HwcTestProtectionChecker& protChecker, uint32_t hwcFrame)
{
    HWCLOGD("DrmShimBuffer::IsProtectionValid handle %p session %d instance %d frame:%d",
        mHandle, mMediaDetails.pavp_session_id, mMediaDetails.pavp_instance_id, hwcFrame);
    return protChecker.IsValid(mMediaDetails, hwcFrame);
}

Hwcval::ValidityType DrmShimBuffer::ProtectedContentValidity(HwcTestProtectionChecker& protChecker, uint32_t hwcFrame)
{
...
const char* DrmShimBuffer::EncryptionStr(char* strbuf, size_t len)
{
    ...
    if (mMediaDetails.is_encrypted)
    {
        n += snprintf(strbuf+n, len-n, "ENCRYPT:%d:%d", mMediaDetails.pavp_session_id, mMediaDetails.pavp_instance_id);
    }
