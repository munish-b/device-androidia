...
void DrmShimChecks::LogModes(uint32_t connId, const char* str,
  ...
  for (int i = 0; i < pConn->count_modes; ++i) {
    ...
    HWCLOGI("  Clock %d vrefresh %d flags 0x%x aspect %s type %d %s",
            ...
            AspectStr(pMode->HWCVAL_DRM_ASPECT), pMode->type,
...
void DrmShimChecks::CheckGetEncoder(uint32_t encoder_id,
  ...
  if (pEncoder) {
    ...
    mPossibleCrtcsForEncoder.replaceValueFor(encoder_id,
...
void DrmShimChecks::CheckSetCrtcEnter(int fd, uint32_t crtcId,
  ...
  HWCLOGA("  Crtc %d Mode %s clock %d vrefresh %d flags %x aspect %s type %d",
          ...
          AspectStr(mode->HWCVAL_DRM_ASPECT), mode->type);
...
void DrmShimChecks::CheckGetPlaneExit(uint32_t plane_id,
  ...
  if (mState->IsOptionEnabled(eOptSpoofNuclear)) {
    HWCLOGD("Plane %d has %d formats. [0]=0x%x", plane_id,
            pPlane->count_formats, pPlane->formats ? pPlane->formats[0] : 0);
    if (pPlane->count_formats <= 2) {
      ...
      pPlane->formats =
          (uint32_t*)malloc(sizeof(uint32_t) * pPlane->count_formats);
...
void DrmShimChecks::CheckAddFB(int fd, uint32_t width, uint32_t height,
  ...
  if ((ret == 0) && (buf_id > 0)) {
    ...
    if (flags & DRM_MODE_FB_AUX_PLANE) {
      ...
      mWorkQueue.Push(new Hwcval::Work::AddFbItem(
          fd, boHandle, buf_id, width, height, pixel_format, pitches[1],
          offsets[1], modifier[1]));
    } else {
      mWorkQueue.Push(new Hwcval::Work::AddFbItem(fd, boHandle, buf_id, width,
                                                  height, pixel_format));
    }
  } else {
...
android::sp<HwcTestBufferObject> DrmShimChecks::GetBufferObject(
    uint32_t boHandle) {
  char strbuf[HWCVAL_DEFAULT_STRLEN];
...
int DrmShimChecks::CheckSetDisplayEnter(drm_mode_set_display* drmDisp,
                                        DrmShimCrtc*& pCrtc) {
  ...
  HWCCHECK(eCheckInvalidCrtc);
  if (crtc == 0) {
    HWCERROR(eCheckInvalidCrtc, "drmModeSetDisplay: Invalid Crtc %d",
             drmDisp->crtc_id);
    return 0;
  }
  ...
  if (mState->IsBufferMonitorEnabled()) {
    if (drmDisp->update_flag & DRM_MODE_SET_DISPLAY_UPDATE_ZORDER) {
      uint32_t order =
          drmDisp->zorder &
          0x7fffffff;  // clear top bit which is used to define panel/HDMI
      HwcTestCrtc::SeqVector* seq = mOrders.valueFor(order);

      HWCCHECK(eCheckSetDisplayParams);
      if (seq == 0) {
        HWCERROR(eCheckSetDisplayParams,
                 "drmModeSetDisplay: CRTC %d Z-order %d unknown",
                 drmDisp->crtc_id, order);
      } else {
        HWCLOGV_COND(eLogDrm, "Setting Z-Order to %d = (%d, %d, %d)", order,
                     (*seq)[0], (*seq)[1], (*seq)[2]);
      }
    ...
    HWCLOGD_COND(eLogDrm,
                 "SetDisplay: size %d version %d CRTC %d Zorder %d update_flag "
                 "0x%x planes %d",
                 drmDisp->size, drmDisp->version, drmDisp->crtc_id,
                 drmDisp->zorder, drmDisp->update_flag, drmDisp->num_planes);
    ...
    if (drmDisp->panel_fitter.mode != DRM_PFIT_OFF) {
      ...
      if (!crtc->SetPanelFitterSourceSize(drmDisp->panel_fitter.src_w,
                                          drmDisp->panel_fitter.src_h)) {
        okToSet = false;
      }
    ...
    for (uint32_t i = 0; i < drmDisp->num_planes; ++i) {
      ...
      // 2015-05-19: Plane update is required even when the FB ID has not
      // changed.
      // Otherwise, we get too many different errors generated when the same FB
      // ID is used following recomposition.
      //
      // Here we rely on HWC to set the contents of the structure correctly even
      // when the update flags are clear.
      // Fortunately they always do this, the idea is they maintain a structure
      // and pass is the same one to DRM
      ...
      // if (drmDisp->update_flag & DRM_MODE_SET_DISPLAY_UPDATE_PLANE(i))
      {
        ...
        HWCLOGD_COND(eLogDrm,
                     "SetDisplay: Plane %d upd 0x%x obj_type 0x%x FB %d tr %d "
                     "src(x,y,w,h)=(%f,%f,%f,%f) crtc=(%d,%d,%d,%d) "
                     "flags 0x%x rrb2 %d alpha %d UD 0x%" PRIx64,
                     drmPlane->obj_id, drmPlane->update_flag,
                     drmPlane->obj_type, drmPlane->fb_id, drmPlane->transform,
                     double(drmPlane->src_x) / 65536.0,
                     double(drmPlane->src_y) / 65536.0,
                     double(drmPlane->src_w) / 65536.0,
                     double(drmPlane->src_h) / 65536.0, drmPlane->crtc_x,
                     drmPlane->crtc_y, drmPlane->crtc_w, drmPlane->crtc_h,
                     drmPlane->flags, drmPlane->rrb2_enable, drmPlane->alpha,
                     drmPlane->user_data);
        ...
        if (drmPlane->obj_type == DRM_MODE_OBJECT_PLANE) {
          ...
          if ((plane == 0) || (plane->GetCrtc() != crtc)) {
            HWCERROR(eCheckPlaneIdInvalidForCrtc,
                     "drmModeSetDisplay: Crtc %d does not have plane %d",
        ...
        if (drmPlane->update_flag &
          ...
          hwTransform = DrmTransformToHalTransform(mState->GetDeviceType(),
                                                   drmPlane->transform);

          plane->SetHwTransform(hwTransform);
          HWCLOGD_COND(eLogDrm, "Performing transform %s on plane %d",
                       DrmShimTransform::GetTransformName(hwTransform),
                       drmPlane->obj_id);
        } else {
        ...
        // DRM_MODE_SET_DISPLAY_PLANE_UPDATE_PRESENT)
        {
          if (drmPlane->fb_id == 0) {
            ...
            // if (plane->GetCurrentDsId() != drmPlane->fb_id)
            {
              ...
              // Null DrmShimBuffer implies blanking.
              if (buf.get()) {
                // If the first frame since mode set, we need to ensure we have
                // 32-bit buffer (RGBA/RGBX).
                HWCCHECK(eCheckFirstFrame32bit);
                if (framesSinceModeSet == 0) {
                  // if (as it should) HWC is using one of their own blanking
                  // buffers, we can't
                  // find out the format because we only have FB ID and bo
                  // handle, we don't
                  // have the gralloc handle.
                  // So we just have to assume the format is OK in that case (it
                  // is likely to be)
                  if ((buf->GetHandle() != 0) &&
                    ...
                    if (!crtc->IsDRRSEnabled() || !crtc->IsDisplayEnabled()) {
                      HWCERROR(eCheckFirstFrame32bit,
                               "Plane %d format %s (0x%x, 0x%x) %s",
                               drmPlane->obj_id, FormatToStr(buf->GetFormat()),
                               buf->GetFormat(), buf->GetDrmFormat(),
                               buf->IdStr(strbuf));
                    }
                  }
                }
                // At this point we should check if the acquire fence has been
                // signalled
                // and raise eCheckDrmFence if it is not.
                //
                // However, we can't do this using buf->GetAcquireFenceFd()
                // because we
                // need to get the fence from the saved layer list, as the
                // DrmShimBuffer
                // does not preserve history in this respect.
                //
                // TODO: Think about.

                // Check the composition that created this buffer, if there was
                // one
                mCompVal->Compare(buf);
            ...
            {
                HWCLOGD_COND(eLogDrm, "Plane %d: skipped update, stays at FB
            %d", drmPlane->obj_id, drmPlane->fb_id);
            }
            */
            ...
            if (drmPlane->obj_type == DRM_MODE_OBJECT_PLANE) {
              ...
              if (drmPlane->flags & DRM_MODE_PAGE_FLIP_EVENT) {
                HWCLOGD(
                    "Detected callback to force main plane disabled on FB %d "
                    "plane %d",
                    drmPlane->fb_id, drmPlane->obj_id);
                plane->GetCrtc()->SetMainPlaneDisabled(true);
              }
              ...
              if (buf.get()) {
                // Don't perform checks against alloc width & height for
                // blanking buffers
                // as gralloc often only gives zeroes for these.
                if (!buf->IsBlanking() && !buf->IsBlack()) {
                  HWCCHECK(eCheckBufferTooSmall);
                  if ((w > buf->GetAllocWidth()) ||
                      (h > buf->GetAllocHeight())) {
                    HWCERROR(eCheckBufferTooSmall,
                             "Plane %d %s %dx%d (alloc %dx%d) Crop %fx%f "
                             "Display %dx%d",
                             drmPlane->obj_id, buf->IdStr(strbuf),
                             buf->GetWidth(), buf->GetHeight(),
                             buf->GetAllocWidth(), buf->GetAllocHeight(), w, h,
                             drmPlane->crtc_w, drmPlane->crtc_h);
                  }
                }

                if (mState->GetDeviceType() == HwcTestState::eDeviceTypeBXT) {
                  scalersUsed += BroxtonPlaneValidation(
                      crtc, buf, "Plane", drmPlane->obj_id, w, h,
                      drmPlane->crtc_w, drmPlane->crtc_h, hwTransform);

                  if (plane->GetPlaneIndex() >=
                      HWCVAL_BXT_FIRST_UNSUPPORTED_NV12_PLANE) {
                    HWCCHECK(eCheckPlaneFormatNotSupported);
                    if (buf->IsNV12Format()) {
                      HWCERROR(eCheckPlaneFormatNotSupported,
                               "Plane %d (index %d) does not support NV12.",
                  ...
                  // Check that the buffer's render compression status is
                  // consistent with the plane
                  if (buf->IsRenderCompressed() !=
                      drmPlane->render_compression) {
                    HWCCHECK(eCheckRCNotSupportedOnPlane);
                    HWCCHECK(eCheckRCNormalBufSentToRCPlane);
                    if (buf->IsRenderCompressed() &&
                        !drmPlane->render_compression) {
                      HWCERROR(eCheckRCNotSupportedOnPlane,
                               "Buffer is marked as render compressed, but "
                               "plane %d (index %d) does not support Render "
                               "Compression.",
                               plane->GetPlaneId(), plane->GetPlaneIndex());
                    } else {
                      HWCERROR(eCheckRCNormalBufSentToRCPlane,
                               "Non Render Compressed buffer sent to RC plane "
                               "(plane id %d index %d).",
                               plane->GetPlaneId(), plane->GetPlaneIndex());
                    }
                  }
                  ...
                  if (buf->IsRenderCompressed()) {
                    ...
                    if (!(buf->IsRenderCompressibleFormat())) {
                      HWCERROR(
                          eCheckRCInvalidFormat,
                          "Format %d not supported with Render Compression",
                          buf->GetFormat());
                    }

                    // Check the tiling. Only Y-tiling is compatible with RC.
                    HWCCHECK(eCheckRCInvalidTiling);
                    if (!((plane->GetTiling() != DrmShimPlane::ePlaneYTiled) ||
                          (plane->GetTiling() !=
                           DrmShimPlane::ePlaneYfTiled))) {
                      HWCERROR(eCheckRCInvalidTiling,
                               "Format with '%s' tiling is not supported with "
                               "Render Compression",
                               plane->GetTiling() == DrmShimPlane::ePlaneXTiled
                                   ? "X-Tiled"
                                   : "Linear");
                    }

                    // Check that the Aux buffer details are consistent with
                    // Gralloc
                    HWCCHECK(eCheckRCAuxDetailsMismatch);
                    if ((plane->GetAuxPitch() != buf->GetAuxPitch()) ||
                        (plane->GetAuxOffset() != buf->GetAuxOffset())) {
                      HWCERROR(eCheckRCAuxDetailsMismatch,
                               "Aux buffer details at plane (pitch: %d offset: "
                               "%d) do not match Gralloc "
                               "(pitch: %d offset: %d)",
                               plane->GetAuxPitch(), plane->GetAuxOffset(),
                               buf->GetAuxPitch(), buf->GetAuxOffset());
                    }
                  }
#endif
                  ...
                  HWCCHECK(eCheckDisplayCropEqualDisplayFrame);
                  if ((fabs(w - drmPlane->crtc_w) > 1.0) ||
                      (fabs(h - drmPlane->crtc_h) > 1.0)) {
                    HWCERROR(eCheckDisplayCropEqualDisplayFrame,
                             "Plane %d %s %dx%d (alloc %dx%d) Crop %fx%f "
                             "Display %dx%d",
                             drmPlane->obj_id, buf->IdStr(strbuf),
                             buf->GetWidth(), buf->GetHeight(),
                ...
                if ((fabs(w - drmPlane->crtc_w) > 1.0) ||
                    (fabs(h - drmPlane->crtc_h) > 1.0)) {
                  HWCERROR(eCheckDisplayCropEqualDisplayFrame,
                           "Plane %d NO BUF Crop %fx%f Display %dx%d",
                           drmPlane->obj_id, w, h, drmPlane->crtc_w,
                           drmPlane->crtc_h);
                }
              }

              // Set expected dimensions
              plane->SetSourceCrop(float(drmPlane->src_x) / 65536.0,
                                   float(drmPlane->src_y) / 65536.0, w, h);
              plane->SetDisplayFrame(drmPlane->crtc_x, drmPlane->crtc_y,
                                     drmPlane->crtc_w, drmPlane->crtc_h);
              ...
              if (buf.get() && (buf->GetHandle() != 0)) {
                ...
                // Allocated size of buffer must be at least full screen
                // Don't perform this check on blanking buffers, as gralloc may
                // not
                // get the alloc width & height
                HWCCHECK(eCheckMainPlaneFullScreen);
                if (!buf->IsBlanking()) {
                  if ((buf->GetAllocWidth() <
                       crtc->GetPanelFitterSourceWidth()) ||
                      buf->GetAllocHeight() <
                          crtc->GetPanelFitterSourceHeight()) {
                    HWCERROR(eCheckMainPlaneFullScreen,
                             "Size is %dx%d, size to fit is %dx%d",
                             buf->GetAllocWidth(), buf->GetAllocHeight(),
                             crtc->GetPanelFitterSourceWidth(),
                             crtc->GetPanelFitterSourceHeight());
                  }
              ...
              if ((drmPlane->obj_type == DRM_MODE_OBJECT_CRTC) ||
                ...
                if (mState->IsOptionEnabled(eOptPageFlipInterception)) {
                  if (drmPlane->user_data) {
                    // Setup page flip event
                    HWCLOGD_COND(eLogEventHandler,
                                 "Crtc %d saving user data %" PRIx64,
                                 crtc->GetCrtcId(), drmPlane->user_data);
                    crtc->SavePageFlipUserData(drmPlane->user_data);
                    drmPlane->user_data = (__u64)crtc->GetCrtcId();
#if !__x86_64__
                    drmPlane->user_data &=
                        0xffffffff;  // try clearing high order 32 bits
#endif
                    HWCLOGD_COND(eLogEventHandler,
                                 "Crtc %d Page flip user data shimmed with "
                                 "crtc %" PRIi64,
                                 crtc->GetCrtcId(), drmPlane->user_data);
                  }
                }
  ...
  // Broxton has only 2 scalers per pipe (1 on pipe C, which we presume isn't
  // being used)
  ...
  if (mState->GetDeviceType() == HwcTestState::eDeviceTypeBXT) {
    ...
    if (scalersUsed > maxScalers) {
      HWCERROR(eCheckNumScalersUsed,
               "%d scalers used on crtc %d (pipe %d), max supported is %d",
               scalersUsed, crtc->GetCrtcId(), crtc->GetPipeIndex(),
               maxScalers);
    }
  } else {
  ...
  // We cause a setdisplay fail if either (a) we just want to test this or (b)
  // we believe that to proceed with the SetDisplay will cause an error.
  ...
  // This is to help diagnose a specific lockup condition - can be removed
  // later.
  if (drmDisp->num_planes == 0) {
    HWCLOGW("Calling drmModeSetDisplay for CRTC %d with 0 planes NOW.",
            crtc->GetCrtcId());
  }
...
#define HWCVAL_DRM_BLEND_FUNC_PREMULT_PA \
  DRM_BLEND_FUNC(CONSTANT_ALPHA, ONE_MINUS_CONSTANT_ALPHA_TIMES_SRC_ALPHA)
#define HWCVAL_DRM_BLEND_FUNC_PREMULT DRM_BLEND_FUNC(ONE, ONE_MINUS_SRC_ALPHA)
#define HWCVAL_DRM_BLEND_FUNC_COVERAGE_PA        \
  DRM_BLEND_FUNC(CONSTANT_ALPHA_TIMES_SRC_ALPHA, \
                 ONE_MINUS_CONSTANT_ALPHA_TIMES_SRC_ALPHA)
#define HWCVAL_DRM_BLEND_FUNC_COVERAGE \
  DRM_BLEND_FUNC(SRC_ALPHA, ONE_MINUS_SRC_ALPHA)

#endif

void DrmShimChecks::SetPlaneBlend(DrmShimPlane* plane,
                                  drm_mode_set_display_plane* drmPlane) {
  char strbuf[HWCVAL_DEFAULT_STRLEN];
#ifdef HWCVAL_DRM_HAS_BLEND
  ...
  switch (drmPlane->blend_func) {
    ...
    default:
      HWCERROR(eCheckInvalidBlend,
               "Plane %d blend function 0x%" PRIx64 " colour 0x%" PRIx64,
               plane->GetPlaneId(), drmPlane->blend_func,
               drmPlane->blend_color);
      blend = BlendingType::PREMULTIPLIED;
  }
#else
...
uint32_t DrmShimChecks::DrmTransformToHalTransform(
  ...
  switch (deviceType) {
    ...
    default: {
      ...
      //
      // Note: the mappings for the DRM Nuclear interface are inverted in
      // relation to the HAL transforms.
      switch (drmTransform) {
...
void DrmShimChecks::CheckSetDisplayExit(drm_mode_set_display* drmDisp,
                                        DrmShimCrtc* crtc, int ret) {
  char strbuf[HWCVAL_DEFAULT_STRLEN];
  ...
  if (ret) {
    if (mState->IsSuspended() || crtc->WasSuspended()) {
      HWCLOGI(
          "SetDisplay failed to crtc %d %s status %d. Expected because POWER "
          "IS SUSPENDED.",
          drmDisp->crtc_id, crtc->ReportSetDisplayPower(strbuf), ret);
    } else {
      HWCERROR(eCheckDrmCallSuccess,
               "SetDisplay failed to crtc %d %s status %d)", drmDisp->crtc_id,
               crtc->ReportSetDisplayPower(strbuf), ret);
    }
  ...
  if (callDuration > HWCVAL_DRM_CALL_DURATION_WARNING_LEVEL_NS) {
    HWCLOGW("drmModeSetDisplay to crtc %d took %fms", drmDisp->crtc_id,
            ((double)callDuration) / 1000000.0);
  }
            ...
            if ((drmPlane->obj_type == DRM_MODE_OBJECT_CRTC) ||
              ...
              if (mState->IsOptionEnabled(eOptPageFlipInterception)) {
                if (drmPlane->user_data) {
                  ...
                  HWCLOGD_COND(eLogEventHandler,
                               "Crtc %d Page flip user data restored to %llx",
                               crtc->GetCrtcId(), drmPlane->user_data);
                }
              }
            }
...
bool DrmShimChecks::ConvertToSetDisplay(
    struct drm_mode_atomic* drmAtomic, drm_mode_set_display*& drmDisp,
    DrmShimCrtc::DrmModeAddFB2Func addFb2Func) {
  PushThreadState ts("ConvertToSetDisplay (locking)");
  HWCVAL_LOCK(_l, mMutex);
  SetThreadState("ConvertToSetDisplay (locked)");
  mWorkQueue.Process();

  HWCLOGD_COND(eLogNuclear, "DrmShimChecks::ConvertToSetDisplay atomic=%p",
               drmAtomic);
  ALOG_ASSERT(mPropMgr);
  ...
  for (uint32_t i = 0; i < drmAtomic->count_objs; ++i) {
    ...
    HWCLOGV_COND(
        eLogNuclear,
        "DrmShimChecks::ConvertToSetDisplay object %d has %d properties",
        obj_id, countProps);

    // Search for destination plane in the setdisplay structure
    DrmShimPlane* plane = mPlanes.valueFor(obj_id);
    if (plane == 0) {
      HWCERROR(eCheckPlaneIdInvalidForCrtc, "Nuclear: No such plane %d",
               obj_id);
      return false;
    }
    ...
    if (i > 0) {
      ...
      if (drmDisp->crtc_id == 0) {
        ...
        HWCLOGV_COND(eLogNuclear,
                     "DrmShimChecks::ConvertToSetDisplay D%d is crtc %d", d,
                     drmDisp->crtc_id);
        ...
        HWCLOGD_COND(eLogNuclear,
                     "DrmShimChecks::ConvertToSetDisplay CRTC %d numPlanes %d",
                     crtcId, drmDisp->num_planes);
        ...
        for (uint32_t pl = 0; pl < crtc->NumPlanes(); ++pl) {
          ...
          // If we have substituted the main plane id with the crtc id, in order
          // to
          // keep older (non-nuclear) platforms happy, we need to change the
          // object
          // type to DRM_MODE_OBJECT_PLANE.
          drmPlane->obj_type =
              (plane->GetCrtc()->GetCrtcId() == drmPlane->obj_id)
...
// Extension for Broxton which supports different blend options.
    ...
    if (drmPlane == 0) {
      HWCERROR(eCheckPlaneIdInvalidForCrtc,
               "Nuclear: CRTC %d does not have plane %d", crtcId, planeId);
      continue;
    }

    // Put the user data on all planes. Flags will indicate if they are
    // important.
    ...
    for (uint32_t j = 0; j < countProps; ++j) {
      ...
      Hwcval::PropertyManager::PropType propType =
          mPropMgr->PropIdToType(prop, propClass);

      HWCLOGV_COND(eLogNuclear, "Property %s value %" PRIi64,
                   mPropMgr->GetName(propType), value);

      HWCCHECK(eCheckIoctlParameters);
      if (propClass != ePlane) {
        HWCERROR(eCheckIoctlParameters,
                 "Nuclear: Plane %d has CRTC property 0x%" PRIx64 " specified",
                 planeId, prop);
        continue;
      }
      ...
      switch (propType) {
        ...
        case Hwcval::PropertyManager::eDrmPlaneProp_CRTC_ID: {
          ...
          if (value == 0) {
            ...
            HWCERROR(eCheckPlaneIdInvalidForCrtc,
                     "Nuclear: Plane %d does not belong to CRTC %d", obj_id,
                     value);
          }
          break;
        }
        case Hwcval::PropertyManager::eDrmPlaneProp_FB_ID: {
          if (value != 0) {
            if (drmPlane->fb_id == 0) {
              // Force an update all properties when a plane transitions from
              // disabled->enabled.
              drmPlane->update_flag |=
            ...
            if (drmPlane->fb_id != 0) {
              // Plane is disabled.
              // Matches behaviour of HWC SetDisplayPageFlipHandler - plane
              // attributes
              // do not persist after this.
              if (drmPlane->obj_type == DRM_MODE_OBJECT_CRTC) {
                if (crtc->MainPlaneIsDisabled()) {
                  HWCLOGD_COND(eLogNuclear,
                               "Nuclear: Main plane CRTC %d already disabled.",
                               drmPlane->obj_id);
                  break;
                } else {
                  HWCLOGD_COND(eLogNuclear,
                               "Nuclear: Disabling main plane CRTC %d",
                               drmPlane->obj_id);
                }

                // Under SetDisplay, if main plane is turned off it needs a
                // blanking buffer.
                drmPlane->fb_id = crtc->GetBlankingFb(addFb2Func, mShimDrmFd);
              ...
              drmDisp->update_flag &=
                  ~DRM_MODE_SET_DISPLAY_UPDATE_PLANE(drmPlaneIx);
            }
          }
          ...
          if ((mState->GetDeviceType() == HwcTestState::eDeviceTypeBYT) ||
            ...
            switch (value) {
              ...
              case HWCVAL_DRM_ROTATE_180:
                break;
              default:
                HWCERROR(eCheckNuclearParams,
                         "Nuclear: Crtc %d Plane %d transform value %d invalid",
                         crtcId, obj_id, value);
            }
          }
        ...
        case Hwcval::PropertyManager::eDrmPlaneProp_render_compression: {
          HWCLOGD_COND(eLogNuclear,
                       "Nuclear: Setting Render Compression field on plane %d "
                       "(crtc %d) to %d",
                       drmPlaneIx, crtcId, value);
#ifdef HWCVAL_ENABLE_RENDER_COMPRESSION
  ...
  for (uint32_t pl = 0; pl < DRM_MODE_SET_DISPLAY_MAX_PLANES; ++pl) {
    ...
    if ((drmPlane->fb_id != 0) && !pageFlipFlagSet) {
      if (mState->IsOptionEnabled(eOptSpoofNuclear)) {
        HWCLOGD_COND(eLogNuclear,
                     "Nuclear: crtc %d: setting plane [%d] user_data to "
                     "validation value: %d",
                     crtcId, pl, crtcId);
        drmPlane->user_data = crtcId;
      } else {
        HWCLOGD_COND(eLogNuclear,
                     "Nuclear: crtc %d: setting plane [%d] user_data to %p",
                     crtcId, pl, drmAtomic->user_data);
...
EXPORT_API void DrmShimChecks::AtomicShimUserData(
    struct drm_mode_atomic* drmAtomic) {
  uint32_t* pObjs = (uint32_t*)drmAtomic->objs_ptr;
...
#endif
      HWCLOGD_COND(eLogEventHandler,
                   "Crtc %d Nuclear:Page flip user data shimmed with crtc %llx",
                   crtc->GetCrtcId(), drmAtomic->user_data);
    }
    break;
  }
}

EXPORT_API void DrmShimChecks::AtomicUnshimUserData(
    struct drm_mode_atomic* drmAtomic) {
  uint32_t* pObjs = (uint32_t*)drmAtomic->objs_ptr;
  ...
  for (uint32_t i = 0; i < drmAtomic->count_objs; ++i) {
    ...
    if (mState->IsOptionEnabled(eOptPageFlipInterception)) {
      ...
      if (drmAtomic->user_data) {
        ...
        HWCLOGD_COND(eLogEventHandler,
                     "Crtc %d Nuclear: Page flip user data restored to %llx",
                     crtc->GetCrtcId(), drmAtomic->user_data);
      }
    }
    break;
...
void DrmShimChecks::CheckIoctlI915SetPlaneZOrder(
  ...
  HwcTestCrtc::SeqVector* seq = mOrders.valueFor(order);
  HWCLOGD_COND(eLogDrm, "Setting CRTC %d Z-Order to %d = (%d, %d, %d)", crtcId,
               order, (*seq)[0], (*seq)[1], (*seq)[2]);

  HWCCHECK(eCheckIoctlParameters);
  if (seq == 0) {
    HWCERROR(eCheckIoctlParameters, "CRTC %d Z-order %d unknown", crtcId,
             order);
  }
...
void DrmShimChecks::CheckIoctlI915DispScreenControl(
    struct drm_i915_disp_screen_control* ctrl, int status) {
  HWCVAL_LOCK(_l, mMutex);
  mWorkQueue.Process();

  // This ALWAYS fails on CHV with no side effects, so no point in doing the
  // check.
  // if (status != 0)
  //{
  //    HWCERROR(eCheckDrmCallSuccess, "DRM_IOCTL_I915_DISP_SCREEN_CONTROL
  //    failed crtc %d status %d",
  //        ctrl->crtc_id, status);
  //}

  if (status == 0) {
    DrmShimCrtc* crtc = mCrtcs.valueFor(ctrl->crtc_id);
    HWCLOGI("DispScreenControl: crtc %d @ %p enable %d", ctrl->crtc_id, crtc,
            ctrl->on_off_cntrl);
    ...
    HWCLOGI("DispScreenControl: crtc %d enable %d FAILED", ctrl->crtc_id,
            ctrl->on_off_cntrl);
  }
}
#endif
...
android::sp<DrmShimBuffer> DrmShimChecks::UpdateBufferPlane(
...
void DrmShimChecks::MoveDsIds(android::sp<DrmShimBuffer> existingBuf,
                              android::sp<DrmShimBuffer> buf) {
