...
// Option to generate SetDisplay calls from the Nuclear API so that Nuclear can
// be tested on
// platforms which don't support it.
DECLARE_CHECK(eOptSpoofNuclear, None, INFO,
              "Convert nuclear DRM calls to set_display", Opt)
...
// Widi Category
//=====================================
// A frame type change message is sent by the HWC when screen size or refresh
// rate change. These
// errors detect various patterns of duplicates.
DECLARE_CHECK(eCheckWidiConsecutiveDuplicateFrameInfo, HWC, ERROR,
              "A duplicate back-to-back FrameTypeChange message was detected",
              Widi)
DECLARE_CHECK(eCheckWidiInterleavedDuplicateFrameInfo, HWC, ERROR,
              "An interleaved FrameTypeChange message was detected", Widi)
DECLARE_CHECK(
    eCheckWidiNonConsecutiveDuplicateFrameInfo, HWC, ERROR,
    "A non-consecutive duplicate FrameTypeChange message was detected", Widi)

// This is really an internal error. TODO CHANGE.
DECLARE_CHECK(eCheckWidiDuplicateFrameInfo, HWC, ERROR,
              "A duplicate FrameTypeChange message was detected", Widi)

// The harness test expects direct mode in the current context.
// Direct mode = when a frame is presented to the HWC in the correct format for
// the widi encoder, then the HWC
// should not do any composition but pass the buffers straight through the
// encoder.
DECLARE_CHECK(eCheckWidiDirectModeExpected, HWC, ERROR,
              "Saw composition when expecting Widi direct mode", Widi)

// This indicates that an arbitrary limit in the number of the SetResolutions
// within certain number of frames
// has been exceeded. It always indicates a protocol error between HWC and widi
// stack.
DECLARE_CHECK(eCheckWidiExcessiveSetResolutions, HWC, ERROR,
              "Excessive setResolutions were received", Widi)

// HWC has to respond to a SetResolution msg with a BufferInfo msg. If it
// doesn't, then it is error.
DECLARE_CHECK(
    eCheckBufferInfoRequired, HWC, ERROR,
    "A 'BufferInfo' message was not received directly after a SetResolution",
    Widi)

// Extended mode consistency checks.
// This happens when full screen on the main display and partial screen on the
// widi.
// It indicates an inconsistency between the full screen state of the HDMI and
// the widi.
// HWC thinks it is an extended mode but it should not, because the widi is not
// full screen.
DECLARE_CHECK(eCheckWidiFullScreenScaling, HWC, ERROR,
              "Hwc is in extended mode without Widi in full screen video", Widi)

// Triggered when HWC is in partial screen video mode but not all the conditions
// are met.
DECLARE_CHECK(eCheckWidiPartialVideo, HWC, WARN,
              "Specification inconsistency with Widi partial video mode", Widi)

// Buffer sent to widi stack must be NV12Y format.
DECLARE_CHECK(eCheckWidiWrongFormat, HWC, ERROR,
              "Buffer being sent to Widi is in incorrect format", Widi)

// The HWC has sent a message which is not consistent with the frames actually
// sent.
DECLARE_CHECK(eCheckWidiWrongFrameType, HWC, ERROR,
              "Wrong frame type for last 'FrameTypeChange' message", Widi)

// NV12Y layer is not at the back of the stack as expected by spec.
// TODO James to review.
DECLARE_CHECK(eCheckWidiWrongLayer, HWC, ERROR,
              "Wrong layer sent to Widi display in fullscreen video mode", Widi)

// This happens when widi stack crashes or when there is an overrun after
// disconnection
// (= widi disconnected but HWC goes on sending frames).
DECLARE_CHECK(eCheckWidiOnFrameReadyError, HWC, ERROR,
              "A call to onFrameReady returned error", Widi)

// These errors indicate that the HWC has not generated the number of messages
// expected for a specific harness test.
DECLARE_CHECK(eCheckWidiUnexpectedBufferInfoCount, HWC, ERROR,
              "Unexpected buffer info count", Widi)
DECLARE_CHECK(eCheckWidiUnexpectedFrameCount, HWC, ERROR,
              "Unexpected frame count received", Widi)
DECLARE_CHECK(eCheckWidiUnexpectedFrameTypeChangeCount, HWC, ERROR,
              "Unexpected frame type change count", Widi)
