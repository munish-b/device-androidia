...
// This means we can have at most 4 levels of composition, and no more than 256 layers in any one composition,
// because we use one byte per level and the Z-order value is stored in a uint32_t.
// Should be enough for anybody, but if we ever need more, we can always use uint64_t.
...
const int DrmShimTransform::mTransformTable[Hwcval::eMaxTransform][Hwcval::eMaxTransform] =
{
    ...
    { eTransformFlipH, eTransformNone, eTransformRot180, eTransformFlipV, eTransformFlip45, eTransformRot270, eTransformRot90, eTransformFlip135},
    { eTransformFlipV, eTransformRot180, eTransformNone, eTransformFlipH, eTransformFlip135, eTransformRot90, eTransformRot270, eTransformFlip45},
    ...
    { eTransformFlip135, eTransformRot90, eTransformRot270, eTransformFlip45, eTransformFlipH, eTransformNone, eTransformRot180, eTransformFlipV},
    { eTransformFlip45, eTransformRot270, eTransformRot90, eTransformFlip135, eTransformFlipV, eTransformRot180, eTransformNone, eTransformFlipH},
...
{
    HWCLOGD_COND(eLogBuffer, "DrmShimTransform::~DrmShimTransform() deleted transform@%p", this);
}
...
DrmShimTransform::DrmShimTransform()
  : mZOrder(0),
    ...
    mDecrypt(false),
    mBlending(BlendingType::HWCVAL_PASSTHROUGH),
    mSources(0)
{
    // TODO: pass in image size as parameters so we have correct right & bottom crop
    mSourcecropf.left = mSourcecropf.top = 0.0;
    mSourcecropf.right = 0;
    mSourcecropf.bottom = 0;

    HWCLOGD_COND(eLogBuffer, "DrmShimTransform::DrmShimTransform() Created transform@%p", this);
}

// Identity transform, for OVERLAYs
DrmShimTransform::DrmShimTransform(android::sp<DrmShimBuffer>& buf, double width, double height)
  : mBuf(buf),
    mZOrder(0),
    mZOrderLevels(1),
    mXscale(1.0),
    ...
    mSources(0)
{
    ...
    HWCLOGD_COND(eLogBuffer, "DrmShimTransform::DrmShimTransform(&buf, double, double) Created transform@%p", this);
}
    ...
    mSources(0)
{
    ...
    HWCLOGD_COND(eLogBuffer, "DrmShimTransform::DrmShimTransform(double, double, double, double) Created transform@%p", this);
}
...
DrmShimTransform::DrmShimTransform(android::sp<DrmShimBuffer> &buf,
    ...
    if (transform & eTransformRot90)
    {
        mXscale = (displayframe.right - displayframe.left) / (sourcecropf.bottom - sourcecropf.top);
        mYscale = (displayframe.bottom - displayframe.top) / (sourcecropf.right - sourcecropf.left);
    }
    else
    {
        mXscale = (displayframe.right - displayframe.left) / (sourcecropf.right - sourcecropf.left);
        mYscale = (displayframe.bottom - displayframe.top) / (sourcecropf.bottom - sourcecropf.top);
    }
    ...
    HWCLOGD_COND(eLogBuffer, "DrmShimTransform::DrmShimTransform(&buf, int, layer*) Created transform@%p", this);
}

DrmShimTransform::DrmShimTransform(android::sp<DrmShimBuffer>& buf, uint32_t layerIx, const Hwcval::ValLayer& layer)
  : mBuf(buf),
    ...
    mSources(0)
{
    const hwc_frect_t& sourcecropf = layer.GetSourceCrop();
    const hwc_rect_t& displayframe = layer.GetDisplayFrame();
...
DrmShimTransform DrmShimTransform::Inverse()
{
    ...
    result.mSourcecropf = ToFRect(df);

    if (mTransform & eTransformRot90)
    {
...
DrmShimTransform::DrmShimTransform (DrmShimTransform& a, DrmShimTransform& b, HwcTestCheckType cond, const char* str)
  : mBuf(a.mBuf),
    mZOrder(b.mZOrder | (a.mZOrder >> (b.mZOrderLevels * Z_ORDER_LEVEL_BITS))),
    ...
    mSources(a.mSources)
{
    ...
    {
        HWCERROR(eCheckInternalZOrder, "Maximum Z-order nesting capability exceeded (%d+%d=%d)",
            a.mZOrderLevels, b.mZOrderLevels, mZOrderLevels);
    }

    if ((a.mTransform < Hwcval::eMaxTransform) && (b.mTransform < Hwcval::eMaxTransform))
    {
        mTransform = mTransformTable[a.mTransform][b.mTransform];
    }
    else
    {
        HWCERROR(eCheckInternalError, "Invalid transform (%d or %d)", a.mTransform, b.mTransform);
    }
    ...
    if (a.mTransform & eTransformRot90)
    {
        if (a.mTransform & eTransformFlipV)
        {
            // Translate 2nd source crop into space of 1st translation and use whichever coords are more restrictive
            ...
            mSourcecropf.right = min<double>((b.mSourcecropf.bottom - a.mYoffset)/a.mYscale + a.mSourcecropf.left,
        ...
        else
        {
            ...
            mSourcecropf.left = a.mSourcecropf.left - min<double>( (b.mSourcecropf.bottom - a.mYoffset) / a.mYscale, 0);
        }

        if (a.mTransform & eTransformFlipH)
        {
            ...
            mSourcecropf.bottom = min<double>((b.mSourcecropf.right - a.mXoffset)/a.mXscale + a.mSourcecropf.top,
        ...
        else
        {
            ...
            mSourcecropf.top = max<double>(a.mSourcecropf.bottom - (b.mSourcecropf.right - a.mXoffset) / a.mXscale, a.mSourcecropf.top);
        }
    ...
    else
    {
        ...
        if (a.mTransform & eTransformFlipH)
        {
            ...
            mSourcecropf.left = max<double>(a.mSourcecropf.right + ((a.mXoffset - b.mSourcecropf.right) / a.mXscale), a.mSourcecropf.left);
        }
        else
        {
            // Translate 2nd source crop into space of 1st translation and use whichever coords are more restrictive
            ...
            mSourcecropf.right = min<double>((b.mSourcecropf.right - a.mXoffset)/a.mXscale + a.mSourcecropf.left,
                                              a.mSourcecropf.right);
        }
        ...
        if (a.mTransform & eTransformFlipV)
        {
            ...
            //mSourcecropf.top = a.mSourcecropf.top - min<double>( (b.mSourcecropf.bottom - a.mYoffset) / a.mYscale, 0);
            mSourcecropf.top = max<double>(a.mSourcecropf.bottom + ((a.mYoffset - b.mSourcecropf.bottom) / a.mYscale), a.mSourcecropf.top);
        }
        else
        {
            double ycrop = b.mSourcecropf.top - a.mYoffset;
            ...
            mSourcecropf.bottom = min<double>((b.mSourcecropf.bottom - a.mYoffset)/a.mYscale + a.mSourcecropf.top,
                                               a.mSourcecropf.bottom);
        }
    }

    if (b.mTransform & eTransformRot90)
    {
        ...
        mXoffset = b.mXoffset + yOrigin * b.mXscale;
            // ... this mXoffset calculation actually gives offset from RHS. Corrected in the flip
            // ... see comment later.
    ...
    // Execute flips
    bool flipH = b.mTransform & eTransformFlipH;
    bool flipV = b.mTransform & eTransformFlipV;
    ...
    if (b.mTransform & eTransformRot90)
    {
        ...
        // This is necessary because the above calculation for mXoffset actually gives the offset from the RHS.
        flipH = !flipH;
    }
    ...
    switch (b.mBlending)
    {
        case BlendingType::HWCVAL_PASSTHROUGH:
            mPlaneAlpha = a.mPlaneAlpha;
            break;

        case BlendingType::COVERAGE:
            // Only makes sense if a is a HWC buffer
            if (b.GetBuf()->IsCompositionTarget())
            {
                HWCERROR(eCheckCompositionBlend, "Invalid blend %s on composition target handle %p", b.GetBlendingStr(strbuf), b.GetBuf()->GetHandle());
    ...
    HWCLOGD_COND(eLogBuffer, "DrmShimTransform::DrmShimTransform(&transform, &transform, check, str) Created transform@%p", this);
...
{
    if (transform < Hwcval::eMaxTransform)
    {
...
void DrmShimTransform::GetEffectiveDisplayFrame(hwc_rect_t& rect)
{
    ...
    if (mTransform & eTransformRot90)
    {
        rect.right = int(rect.left + ((mSourcecropf.bottom - mSourcecropf.top) * mXscale + 0.5));
        rect.bottom = int(rect.top + ((mSourcecropf.right - mSourcecropf.left) * mYscale + 0.5));
    }
    else
    {
        rect.right = int(rect.left + ((mSourcecropf.right - mSourcecropf.left) * mXscale + 0.5));
        rect.bottom = int(rect.top + ((mSourcecropf.bottom - mSourcecropf.top) * mYscale + 0.5));
    }
}
...
void DrmShimTransform::Log(int priority, const char* str) const
{
    ...
    HWCLOG(priority,"%s@%p %s z=%08x Sourcecropf(l,t,r,b)=(%4.1f,%4.1f,%4.1f,%4.1f) Offset=(%4.1f,%4.1f) Scale=(%1.3f,%1.3f)%s Tf=%s %s srcs %x",
        str,
        this,
        mBuf.get() ? mBuf->IdStr(strbuf) : "buf@0",
        GetZOrder(),
        mSourcecropf.left,mSourcecropf.top,mSourcecropf.right,mSourcecropf.bottom,
        mXoffset, mYoffset,
        mXscale, mYscale,
        mDecrypt ? " DECRYPT" : "",
        GetTransformName(),
        GetBlendingStr(strbuf2),
        SourcesStr(strbuf3));
}

android::String8 DrmShimTransform::GetBlendingStr(Hwcval::BlendingType blending)
{
    switch(blending)
    {
        case BlendingType::HWCVAL_PASSTHROUGH:
            return android::String8("PASSTHROUGH");

        case BlendingType::NONE:
            return android::String8("NONE");

        case BlendingType::COVERAGE:
            return android::String8("COVERAGE");

        case BlendingType::PREMULTIPLIED:
            return android::String8("PREMULT");

        default:
            return android::String8::format("UNKNOWN 0x%x", blending);
    }
}
...
{
    android::String8 str(GetBlendingStr(mBlending));

    sprintf(strbuf, "%s %sPXA %f",
            str.string(), mHasPixelAlpha ? "+" : "-", double(mPlaneAlpha));
...
bool DrmShimTransform::Compare(DrmShimTransform& actual, DrmShimTransform& orig, int display, HwcTestCrtc* crtc,
    uint32_t& cropErrorCount, uint32_t& scaleErrorCount, uint32_t hwcFrame)
{
        ...
        (fabs(mSourcecropf.bottom - mSourcecropf.top) < HWCVAL_CROP_MARGIN))
    {
        ...
        if ((fabs(actual.mSourcecropf.right - actual.mSourcecropf.left) < HWCVAL_CROP_MARGIN) ||
            (fabs(actual.mSourcecropf.bottom - actual.mSourcecropf.top) < HWCVAL_CROP_MARGIN))
        {
            // ... and there isn't
            return true;
        }
    ...
    if ((fabs(mSourcecropf.left - actual.mSourcecropf.left) > HWCVAL_CROP_MARGIN) ||
        (fabs(mSourcecropf.top - actual.mSourcecropf.top) > HWCVAL_CROP_MARGIN) ||
        (fabs(mSourcecropf.right - actual.mSourcecropf.right) > HWCVAL_CROP_MARGIN) ||
        (fabs(mSourcecropf.bottom - actual.mSourcecropf.bottom) > HWCVAL_CROP_MARGIN))
    {
        ...
        else
        {
            ...
            HWCLOGE("  D%d SC: Layer%2d (%6.1f,%6.1f,%6.1f,%6.1f) Scaled (%6.1f,%6.1f,%6.1f,%6.1f) actual (%6.1f,%6.1f,%6.1f,%6.1f) %s",
                display,
                GetLayerIndex(),
                (double) orig.mSourcecropf.left,
                (double) orig.mSourcecropf.top,
                (double) orig.mSourcecropf.right,
                (double) orig.mSourcecropf.bottom,
                (double) mSourcecropf.left,
                (double) mSourcecropf.top,
                (double) mSourcecropf.right,
                (double) mSourcecropf.bottom,
                (double) actual.mSourcecropf.left,
                (double) actual.mSourcecropf.top,
                (double) actual.mSourcecropf.right,
                (double) actual.mSourcecropf.bottom,
                GetBuf()->IdStr(strbuf));
        }
    }
    ...
    {
        HWCERROR(eCheckPlaneTransform,"Layer %d %s transform expected=%s actual=%s to display %d frame:%d",
            GetLayerIndex(), GetBuf()->IdStr(strbuf),
            GetTransformName(), actual.GetTransformName(), display, hwcFrame);
    }
    ...
    if (mBlending == BlendingType::HWCVAL_PASSTHROUGH &&
        (actual.mBlending == BlendingType::NONE || actual.mBlending ==BlendingType::PREMULTIPLIED))
    {
        return true;
    }

    if ((mLayerIndex == 0 && (mBlending == BlendingType::NONE || mBlending == BlendingType::PREMULTIPLIED) &&
        (actual.mBlending == BlendingType::NONE || actual.mBlending == BlendingType::PREMULTIPLIED || actual.mBlending == BlendingType::HWCVAL_PASSTHROUGH))
    ...
    else
    {
        ...
        {
            HWCERROR(eCheckPlaneBlending, "Layer %d %s incompatible blending: expected %s actual %s (frame:%d)",
                GetLayerIndex(), GetBuf()->IdStr(strbuf),
                GetBlendingStr(strbuf2), actual.GetBlendingStr(strbuf3), hwcFrame);
            return true;
        }
    }
    ...
    {
        if ((mBlending == BlendingType::NONE) && !actual.mHasPixelAlpha)
        {
            // HWC can represent no blending by remapping an RGBA buffer as RGBX
        }
        else
        {
            HWCERROR(eCheckPixelAlpha, "Layer %d %s per-pixel alpha is not being rendered: expected %s actual %s (frame: %d)",
                GetLayerIndex(), GetBuf()->IdStr(strbuf),
                GetBlendingStr(strbuf2), actual.GetBlendingStr(strbuf3), hwcFrame);
        }
    }
    ...
    {
        HWCERROR(eCheckPlaneAlpha, "Layer %d %s plane alpha rendered incorrectly: expected %f actual %f (frame:%d)",
            GetLayerIndex(), GetBuf()->IdStr(strbuf),
            mPlaneAlpha, actual.mPlaneAlpha, hwcFrame);
    }
...
// return false if no more checks are to be carried out on this display
bool DrmShimTransform::CompareDf(DrmShimTransform& actual, DrmShimTransform& orig, int display, HwcTestCrtc* crtc,
    uint32_t& scaleErrorCount)
{
    HwcTestCheckType errorCode;
    char strbuf[HWCVAL_DEFAULT_STRLEN];

    hwc_rect_t requestedDisplayFrame;
    GetEffectiveDisplayFrame(requestedDisplayFrame);

    hwc_rect_t effectiveDisplayFrame;
    actual.GetEffectiveDisplayFrame(effectiveDisplayFrame);

    if ((abs(requestedDisplayFrame.left - effectiveDisplayFrame.left) > HWCVAL_DISPLAYFRAME_SIZE_MARGIN) ||
        (abs(requestedDisplayFrame.top - effectiveDisplayFrame.top) > HWCVAL_DISPLAYFRAME_SIZE_MARGIN) ||
        (abs(requestedDisplayFrame.right - effectiveDisplayFrame.right) > HWCVAL_DISPLAYFRAME_SIZE_MARGIN) ||
        (abs(requestedDisplayFrame.bottom - effectiveDisplayFrame.bottom) > HWCVAL_DISPLAYFRAME_SIZE_MARGIN))
    {
        ...
        {
            hwc_rect_t origDisplayFrame;
            orig.GetEffectiveDisplayFrame(origDisplayFrame);

            ++scaleErrorCount;
            HWCLOGE("  D%d DF: Layer%2d (%6d,%6d,%6d,%6d) Scaled (%6d,%6d,%6d,%6d) actual (%6d,%6d,%6d,%6d) %s",
                display,
                GetLayerIndex(),
                origDisplayFrame.left,
                origDisplayFrame.top,
                origDisplayFrame.right,
                origDisplayFrame.bottom,
                requestedDisplayFrame.left,
                requestedDisplayFrame.top,
                requestedDisplayFrame.right,
                requestedDisplayFrame.bottom,
                effectiveDisplayFrame.left,
                effectiveDisplayFrame.top,
                effectiveDisplayFrame.right,
                effectiveDisplayFrame.bottom,
                GetBuf()->IdStr(strbuf));
        }
    }
...
DrmShimFixedAspectRatioTransform::DrmShimFixedAspectRatioTransform(uint32_t sw, uint32_t sh, uint32_t dw, uint32_t dh)
{
...
bool operator<(const DrmShimTransform& lhs, const DrmShimTransform& rhs)
{
    if (lhs.GetZOrder() == rhs.GetZOrder())
    {
        HWCERROR(eCheckInternalZOrder,
            "Warning: identical Z-orders in transform comparison (%p %p), you may get items overwritten in sorted vector",
            &lhs, &rhs);
...
uint32_t DrmShimTransform::Inverse(uint32_t transform)
{
    switch (transform)
    {
        case eTransformRot90:
            return eTransformRot270;

        case eTransformRot270:
            return eTransformRot90;
...
hwc_rect_t InverseTransformRect(hwc_rect_t& rect, const Hwcval::ValLayer& layer)
{
    ...
    const hwc_rect_t& dst = layer.GetDisplayFrame();
    ...
    // (as a transform...)
    DrmShimTransform videoDfInLayerSourceFrame(videoDfTransform, inverseLayerTransform, eLogVideo,
...
{
    return ((mSources & (1 << static_cast<int>(Hwcval::BufferSourceType::Ivp))) != 0);
}

bool DrmShimTransform::IsFromSfComp()
{
    return ((mSources & (1 << static_cast<int>(Hwcval::BufferSourceType::SfComp))) != 0);
}
...
const char* DrmShimTransform::SourcesStr(uint32_t sources, char* strbuf)
{
    ...
    *strbuf = '\0';
    ...
    if (sources & (1 << static_cast<int>(Hwcval::BufferSourceType::PartitionedComposer)))
    {
        p += sprintf(p, " PC");
    }
...
DrmShimCroppedLayerTransform::DrmShimCroppedLayerTransform(android::sp<DrmShimBuffer>& buf, uint32_t layerIx, const Hwcval::ValLayer& layer, HwcTestCrtc* crtc)
{
    ...
    if (layer.GetVisibleRegion().NumRects() > 0)
    {
        ...
        if ((bounds.left > df.left) ||
            (bounds.right < df.right) ||
            (bounds.top > df.top) ||
            (bounds.bottom < df.bottom))
        {
            ...
            inverseLayer.SetSourceCrop(ToFRect(df));
            inverseLayer.SetDisplayFrame(ToRect(layer.GetSourceCrop()));
            inverseLayer.SetTransformId(DrmShimTransform::Inverse(layer.GetTransformId()));
            inverseLayer.SetBlendingType(BlendingType::HWCVAL_PASSTHROUGH);
            DrmShimTransform inverseLayerTransform(buf, 0, inverseLayer);

            ValLayer boundsLayer;
            boundsLayer.SetSourceCrop(hwc_frect_t({0, 0, float(crtc->GetWidth()), float(crtc->GetHeight())}));
            boundsLayer.SetDisplayFrame(bounds);
            boundsLayer.SetTransformId(0);
            boundsLayer.SetBlendingType(BlendingType::HWCVAL_PASSTHROUGH);
            DrmShimTransform boundingTransform(buf, 0, boundsLayer);

            DrmShimTransform derived(boundingTransform, inverseLayerTransform, eLogCombinedTransform,
                "Visible regions: bounding box reverse transformed into source frame of reference");
            hwc_rect_t boundsInSource;
            derived.GetEffectiveDisplayFrame(boundsInSource);
            layerTransform.SetSourceCrop(ToFRect(boundsInSource));
        }
    }

    // Apply the portal of the physical screen
    DrmShimTransform croppedLayerTransform(layerTransform, crtc->GetScaleTransform(), eLogCroppedTransform,
