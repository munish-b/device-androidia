...
const int DrmShimTransform::mTransformTable
    [Hwcval::eMaxTransform][Hwcval::eMaxTransform] = {
        {eTransformNone, eTransformFlipH, eTransformFlipV, eTransformRot180,
         eTransformRot90, eTransformFlip135, eTransformFlip45,
         eTransformRot270},
        {eTransformFlipH, eTransformNone, eTransformRot180, eTransformFlipV,
         eTransformFlip45, eTransformRot270, eTransformRot90,
         eTransformFlip135},
        {eTransformFlipV, eTransformRot180, eTransformNone, eTransformFlipH,
         eTransformFlip135, eTransformRot90, eTransformRot270,
         eTransformFlip45},
        {eTransformRot180, eTransformFlipV, eTransformFlipH, eTransformNone,
         eTransformRot270, eTransformFlip45, eTransformFlip135,
         eTransformRot90},
        {eTransformRot90, eTransformFlip135, eTransformFlip45, eTransformRot270,
         eTransformRot180, eTransformFlipV, eTransformFlipH, eTransformNone},
        {eTransformFlip135, eTransformRot90, eTransformRot270, eTransformFlip45,
         eTransformFlipH, eTransformNone, eTransformRot180, eTransformFlipV},
        {eTransformFlip45, eTransformRot270, eTransformRot90, eTransformFlip135,
         eTransformFlipV, eTransformRot180, eTransformNone, eTransformFlipH},
        {eTransformRot270, eTransformFlip45, eTransformFlip135, eTransformRot90,
         eTransformNone, eTransformFlipH, eTransformFlipV, eTransformRot180}};
...
DrmShimTransform::DrmShimTransform(android::sp<DrmShimBuffer>& buf,
  ...
  if (transform & eTransformRot90) {
    mXscale = (displayframe.right - displayframe.left) /
              (sourcecropf.bottom - sourcecropf.top);
    mYscale = (displayframe.bottom - displayframe.top) /
              (sourcecropf.right - sourcecropf.left);
  } else {
    mXscale = (displayframe.right - displayframe.left) /
              (sourcecropf.right - sourcecropf.left);
    mYscale = (displayframe.bottom - displayframe.top) /
...
DrmShimTransform::DrmShimTransform(DrmShimTransform& a, DrmShimTransform& b,
  ...
  if ((a.mTransform < Hwcval::eMaxTransform) &&
      (b.mTransform < Hwcval::eMaxTransform)) {
...
android::String8 DrmShimTransform::GetBlendingStr(
    Hwcval::BlendingType blending) {
  switch (blending) {
    case BlendingType::HWCVAL_PASSTHROUGH:
...
const char* DrmShimTransform::GetBlendingStr(char* strbuf) const {
  ...
  sprintf(strbuf, "%s %sPXA %f", str.string(), mHasPixelAlpha ? "+" : "-",
          double(mPlaneAlpha));
...
bool DrmShimTransform::Compare(DrmShimTransform& actual, DrmShimTransform& orig,
  ...
  if (mBlending == BlendingType::HWCVAL_PASSTHROUGH &&
      (actual.mBlending == BlendingType::NONE ||
       actual.mBlending == BlendingType::PREMULTIPLIED)) {
    return true;
  }

  if ((mLayerIndex == 0 && (mBlending == BlendingType::NONE ||
                            mBlending == BlendingType::PREMULTIPLIED) &&
       (actual.mBlending == BlendingType::NONE ||
        actual.mBlending == BlendingType::PREMULTIPLIED ||
        actual.mBlending == BlendingType::HWCVAL_PASSTHROUGH)) ||
...
hwc_rect_t InverseTransformRect(hwc_rect_t& rect,
                                const Hwcval::ValLayer& layer) {
...
DrmShimCroppedLayerTransform::DrmShimCroppedLayerTransform(
    android::sp<DrmShimBuffer>& buf, uint32_t layerIx,
  ...
  if (layer.GetVisibleRegion().NumRects() > 0) {
    ...
    if ((bounds.left > df.left) || (bounds.right < df.right) ||
      ...
      boundsLayer.SetSourceCrop(hwc_frect_t(
