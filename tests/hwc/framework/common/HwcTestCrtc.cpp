...
HwcTestCrtc::HwcTestCrtc(uint32_t crtcId, uint32_t width, uint32_t height,
  ...
  mVBlankWatchdog.SetMessage(
      android::String8::format("VBlank watchdog Crtc %d", mCrtcId));
  mPageFlipWatchdog.SetMessage(
      android::String8::format("Page flip watchdog Crtc %d", mCrtcId));
  mSetDisplayWatchdog.SetMessage(
      android::String8::format("Set Display watchdog Crtc %d", mCrtcId));
  mDPMSWatchdog.SetMessage(
      android::String8::format("DPMS watchdog Crtc %d", mCrtcId));
...
void HwcTestCrtc::ConsistencyChecks(Hwcval::LayerList* ll, uint32_t hwcFrame) {
  ...
  for (uint32_t i = 0; i < ll->GetNumLayers(); ++i) {
    ...
    if (transformIx < mTransforms.size()) {
      ...
      if (pTransform->GetBuf()->IsFbt() || pTransform->GetBuf()->IsBlanking()) {
        ...
        HWCLOGV_COND(
            eLogCombinedTransform,
            "HwcTestCrtc::ConsistencyChecks transformIx=%d, pTransform=%p",
    ...
    if ((numLayers >= 2) && (i == (numLayers - 2 /* Skip FBT */)) &&
      ...
      // only skip the transforms which are derived from this buffer. However,
      // using
      // this code, occasional false errors are reported (in the order of 3 for
      // > 43000 checks). This is usually sufficient to visually check that the
      // test
      // is working before the Widi visualisation is turned off and the test is
      // run
      // for real. However, to guarentee that no checks trigger in error, this
      // code
      // should be left commented.

      while (pTransform /* && buf->IsCombinedFrom(pTransform->GetBuf()) */) {
        pTransform->SetLayerIndex(
            i);  // Sets this as processed (i.e. skip transform)
        ++matchCount;
      ...
      HWCLOGV_COND(eLogCombinedTransform,
                   "HwcTestCrtc::ConsistencyChecks Matched Widi Visualisation "
                   "Layer - handle %p to %d sources",
                   handle, matchCount);
      continue;
    }
    ...
    if ((pcValidity != ValidityType::Valid) &&
      ...
      if (pTransform && (pTransform->GetLayerIndex() == eNoLayer) &&
        ...
        DrmShimTransform croppedScreenTransform(
            *pTransform, GetCropTransform(), eLogCroppedTransform,
      ...
      if (layer.GetCompositionType() == CompositionType::TGT) {
        ...
        logTransformPriority =
            HWCERROR(errorCode, "D%d Layer %d %s is ON SCREEN P%d TOO FAR BACK",
                     GetSfSrcDisplayIx(), i,
                     pTransform->GetBuf()->IdStr(strbuf), GetDisplayIx());
      } else {
        ...
        HWCLOGW("Display %d Layer %d %s is ON SCREEN P%d TOO FAR FORWARD",
                GetSfSrcDisplayIx(), i, pTransform->GetBuf()->IdStr(strbuf),
                GetDisplayIx());
  ...
  for (uint32_t i = 0; i < mTransforms.size(); ++i) {
    ...
    if (HWCCOND(eLogCombinedTransform)) {
      transform.Log(ANDROID_LOG_VERBOSE,
...
void HwcTestCrtc::SetUserModeFinish(android::status_t st, uint32_t width,
