...
Debugfs::Debugfs()
{
    ...
    else
    {
        if (stat("/sys/kernel/debug/dri", &st))
        {
            ...
            {
                HWCLOGE("Debugfs::Debugfs - can't mount /sys/kernel/debug error %d", errno);
                return;
            }
        }
    ...
    for(int n = 0; n < 16; ++n)
    {
        int len = snprintf(mDebugfsPath, sizeof mDebugfsPath - 1, "%s/dri/%d", mDebugfsRoot, n);
...
CRCCtlFile::CRCCtlFile(Debugfs &dgfs)
    : mDbgfs(dgfs),
      mFd(-1)
{
}
...
bool CRCCtlFile::OpenPipe()
{
    ...
    if (mFd == -1)
    {
        HWCLOGE("CRCCtlFile::OpenPipe - ERROR can't open %s, error %d", filename, errno);
        return false;
    }
    return true;
}
...
bool CRCCtlFile::EnablePipe(enum pipe pipe, enum intel_pipe_crc_source source)
{
    ...
    ATRACE_CALL();
    HWCLOGD("CRCCtlFile::EnablePipe(%c, %s) - called", pipe_name(pipe), pipe_crc_sources[source]);
    ...
    {
        int n = snprintf(buf, sizeof buf - 1, "pipe %c %s", pipe_name(pipe), pipe_crc_sources[source]);
        if (n > 0)
        {
            write(mFd, buf, n);
        }
...
bool CRCDataFile::Open(enum pipe pipe)
{
    ...
    if (mFd == -1)
    {
        HWCLOGE("CRCDataFile::Open - ERROR can't open %s, error %d", filename, errno);
        return false;
    }
...
bool CRCDataFile::Read(crc_t &crc)
{
    ...
    else
    {
        ...
        {
            HWCLOGE("CRCDataFile::Read - pipe(%c) - ERROR expected %d bytes, only read %d",
    ...
    items = sscanf(buf, CRC_RES_FMT_STRING,
                        &crc.frame, &crc.crc[0], &crc.crc[1], &crc.crc[2], &crc.crc[3], &crc.crc[4]);

    // Timestamp the CRC res. This is less than ideal, as there is an inevitable delay between
    // debugfs writing to the CRC results file and the CRC reader thread reading from it. However,
    // it's better than nothing and will hopefully be accurate enough to give us a reasonable idea
    ...
    {
        // this suggests debugfs is outputting the data in a different format from that we're expecting
        HWCLOGE("CRCDataFile::Read - ERROR expected %d items, only read %d", PIPE_RESULT_WORDS, items);
        return false;
    }

    HWCLOGI_IF(PIPE_CRC_DEBUG_CRC_IFACE, "CRCDataFile::Read - pipe(%c) crc = %08x-%08x-%08x-%08x-%08x",
               pipe_name(mPipe), crc.crc[0], crc.crc[1], crc.crc[2], crc.crc[3], crc.crc[4]);
    return true;
}
