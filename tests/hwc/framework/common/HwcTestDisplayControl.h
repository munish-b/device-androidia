...
class HwcTestDisplayControl  //: public BnDisplayControl
    {
 public:
  HwcTestDisplayControl(uint32_t disp, sp<IDisplayControl> real,
...
class HwcTestDisplayModeControl  //: public BnDisplayModeControl
    {
 public:
  HwcTestDisplayModeControl(uint32_t disp, sp<IDisplayModeControl> real,
                            HwcTestKernel *testKernel);
  ...
  /// get current mode
  virtual status_t getMode(uint32_t *width, uint32_t *height, uint32_t *refresh,
                           uint32_t *flags, uint32_t *ratio);

  /// set mode
  virtual status_t setMode(uint32_t width, uint32_t height, uint32_t refresh,
                           uint32_t flags, uint32_t ratio);

#ifdef EXPERIMENTAL
  virtual status_t getScaleMode(uint32_t *scale) {
    return 0;
  }
  virtual status_t setScaleMode(uint32_t scale) {
    return 0;
  }
#endif
 private:
