...
public:
    ...
    virtual void SuspendCRCs(int crtcId, enum CRC_SUSPENSIONS reason, bool suspend) = 0;
...
class HwcCrcReader : public HwcCrcReaderInterface, public android::Thread
{
...
private:
    ...
    uint32_t    mCrcs;              // number of CRC results (equates to the number of VSYNCs since CRCs enabled)
    ...
    uint32_t    mPageFlips;         // the number of page flips since the CRC reader was enabled
    uint32_t    mRepeatedFrames;    // the number of page flips in which the displayed frame was repeated
...
public:
    ...
    void CheckEnabledState(HwcTestCrtc *crtc)
    {
        ...
        if (pReader) pReader->CheckEnabledState(crtc);
    }

    void NotifyPageFlip(HwcTestCrtc* crtc)
    {
        ALOGE("HwcCrcReaderShim - NotifyPageFlip");
        if (pReader) pReader->NotifyPageFlip(crtc);
    }

    void SuspendCRCs(int crtcId, enum HwcCrcReader::CRC_SUSPENSIONS reason, bool suspend)
    {
        ALOGE("HwcCrcReaderShim - SuspendCRCs");
        if (pReader) pReader->SuspendCRCs(crtcId, reason, suspend);
    }

    void run(const char *s)
    {
        ALOGE("HwcCrcReaderShim - run(%s)", s);
        if (pReader) pReader->run(s);
    }
...
protected:
    ...
    void DoRenderStall(HwcTestCrtc *crtc)
    {
        ...
        if (pReader) pReader->DoRenderStall(crtc);
    }
    ...
    void ProcessCRC(crc_t &res)
    {
        ...
        if (pReader) pReader->ProcessCRC(res);
    }

    void DebugCRC()
    {
        ALOGE("HwcCrcReaderShim - DebugCRC");
        if (pReader) pReader->DebugCRC();
    }
