...
int Mutex::lock( )
{
    ...
    for (timecount=0;;timecount++)
    {
        ...
        if ( mMutex.tryLock( ) == 0 )
            break;
        ALOGD_IF( (MUTEX_CONDITION_DEBUG && timecount==0), "Blocking on mutex %p thread %u", this, gettid() );
        usleep( mSpinWait );
        ...
        if (timecount * mSpinWait > mLongTime*10) {
            ALOGE( "Fatal Thread %u blocked by thread %u waiting for mutex %p", gettid( ), mTid, this );
            ALOG_ASSERT( 0 );
        }
    }
...
int Mutex::unlock( )
{
    ...
    mMutex.unlock( );
    ALOGD_IF( MUTEX_CONDITION_DEBUG, "Released mutex %p thread %u", this, gettid() );
    return 0;
}
...
status_t Mutex::tryLock()
{
    ...
    ALOGD_IF( MUTEX_CONDITION_DEBUG, "Function tryLock() returned %d mutex %p thread %u", ret, this, gettid() );
    return ret;
}
...
{
    ALOGD_IF( MUTEX_CONDITION_DEBUG, "Condition %p waitRelative Enter mutex %p mTid/tid %d/%d",
        this, &mutex, mutex.mTid, gettid() );
    ALOG_ASSERT( mbInit );
    ALOG_ASSERT( mutex.mTid == gettid( ) );
    mutex.mTid = 0;
    mutex.incWaiter( );
    mWaiters++;
    ALOGD_IF( MUTEX_CONDITION_DEBUG, "Condition %p waitRelative on mutex %p waiters %u/%u",
        this, &mutex, mWaiters, mutex.getWaiters() );
    int ret = mCondition.waitRelative( mutex.mMutex, timeout );
    mutex.decWaiter( );
    mWaiters--;
    ALOGD_IF( MUTEX_CONDITION_DEBUG, "Condition %p re-acquired mutex %p waiters %u/%u",
        this, &mutex, mWaiters, mutex.getWaiters() );
    mutex.mTid = gettid( );
...
{
    ALOGD_IF( MUTEX_CONDITION_DEBUG, "Condition %p wait Enter mutex %p mTid/tid %d/%d",
        this, &mutex, mutex.mTid, gettid() );
    ALOG_ASSERT( mbInit );
    ALOG_ASSERT( mutex.mTid == gettid( ) );
    mutex.mTid = 0;
    mutex.incWaiter( );
    mWaiters++;
    ALOGD_IF( MUTEX_CONDITION_DEBUG, "Condition %p wait on mutex %p waiters %u/%u mTid/tid %d/%d",
        this, &mutex, mWaiters, mutex.getWaiters(), mutex.mTid, gettid() );
    int ret = mCondition.wait( mutex.mMutex );
    mutex.decWaiter( );
    mWaiters--;
    ALOGD_IF( MUTEX_CONDITION_DEBUG, "Condition %p re-acquired mutex %p waiters %u/%u mTid/tid %d/%d",
        this, &mutex, mWaiters, mutex.getWaiters(), mutex.mTid, gettid() );
    mutex.mTid = gettid( );
    mutex.mAcqTime = systemTime(SYSTEM_TIME_MONOTONIC);
    return ret;
