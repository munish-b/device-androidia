    ...
    mSnapshotsRestored("snapshots_restored")
{
    ...
    virtCrtc->SetDimensions(
        mState->GetWirelessCrtcWidth(),
        mState->GetWirelessCrtcHeight(),
        0,
...
EXPORT_API void HwcTestKernel::SetHwcServiceShim(android::sp<HwcServiceShim> serviceShim)
{
...
void HwcTestKernel::IterateAllBuffers()
{
    ...
    HWCLOGV("IterateAllBuffers: mBuffers.size()=%d DrmShimBuffer::mCount=%d", mBuffers.size(), DrmShimBuffer::mCount);

    if ((mBuffers.size() > CURRENT_BUFFER_COUNT_WARNING_LEVEL) || (DrmShimBuffer::mCount > TOTAL_BUFFER_COUNT_WARNING_LEVEL))
    {
        HWCERROR(eCheckObjectLeak, "Too many buffer records: mBuffers.size()=%d, Total active buffer records=%d",
...
void HwcTestKernel::DetermineFullScreenVideo(uint32_t displayIx, uint32_t i, Hwcval::ValLayer& vl, const char* notes)
{
    ...
    // Try to work out
    // if all screens are showing the same video, and if it's full screen in one place.
    //
    // NOTE: The famous 1-page spec states that full screen video mode is only engaged
    ...
    if (buf->IsVideoFormat())
    {
        ...
        if (crtc)
        {
            ...
            float areaRatio = float(width * height) / float(crtc->GetWidth() * crtc->GetHeight());

            HWCLOGD_COND(eLogVideo, "Display %d layer %d (bottom) (l,t,r,b) = (%d, %d, %d, %d) area ratio %f",
                displayIx, i,
                df.left, df.top,
                df.right, df.bottom,
                (double) areaRatio);
                ...
                (areaRatio >= 0.9))
            {
                ...
                mDisplayFullScreenVideo[displayIx] = eTrue; // This condition only needs to be true on one screen
            }
            else
            {
            ...
            HWCLOGV_COND(eLogVideo, "D%d.%u Fmt %s is video", displayIx, i, buf->StrBufFormat());
        }
    }

    HWCLOGI_COND(eLogBuffer, "D%d.%u Fmt 0x%x %s Appearances %d %s FS:%s %s", displayIx, i, format,
                    (buf.get() ? buf->IdStr(strbuf) : "0"),
                    buf->GetAppearanceCount(),
                    buf->IsVideoFormat() ? "VIDEO " : "",
                    TriStateStr(mDisplayFullScreenVideo[displayIx]),
                    notes);
...
Hwcval::LayerList::VideoFlags HwcTestKernel::AnalyzeVideo()
{
    ...
    HWCLOGD_COND(eLogVideo, "Frame:%d:%d:%d mSingleFullScreenVideo = (allScreenVideo=%d) && (fullScreenVideo=%s) = %d. PartialScreenVideo=%d",
            mFN[0], mFN[1], mFN[2], allScreenVideo, TriStateStr(fullScreenVideo), singleFullScreenVideo, partialScreenVideo);
...
// Determine how a layer's display frame intersects with the screen: Is it fully visible, partially off screen, or
// completely off screen.
void HwcTestKernel::ValidateHwcDisplayFrame(const hwc_rect_t& layerDf, const hwc_rect_t& fbtDf, uint32_t displayIx, uint32_t layerIx)
{
    ...
    HWCERROR(check, "Display %d Layer %d Displayframe (%d, %d, %d, %d) screen (%d, %d, %d, %d)",
            displayIx, layerIx, layerDf.left, layerDf.top, layerDf.right, layerDf.bottom,
            fbtDf.left, fbtDf.top, fbtDf.right, fbtDf.bottom);
}

// Function to lookup a DrmShimBuffer from the gralloc handle.
android::sp<DrmShimBuffer> HwcTestKernel::lookupDrmShimBuffer(buffer_handle_t handle)
{
    ssize_t ix = mBuffers.indexOfKey(handle);
    if (ix < 0)
    {
        HWCLOGD_COND(eLogBuffer, "Could not find DrmShimBuffer for handle " PRIi64, handle);
    }
...
void HwcTestKernel::checkWidiBuffer(HwcTestCrtc *crtc,
                                    Hwcval::LayerList* ll,
    ...
    if (crtc && ll)
    {
        ...
        if (buf != NULL)
        {
            ...
            if (plane != NULL)
            {
                ...
                plane->GetTransform().SetBlend(BlendingType::HWCVAL_PASSTHROUGH, false, 1.0);
            ...
            {
                HWCERROR(eCheckBufferInfoRequired, "Saw a SetResolution message without a corresponding BufferInfo");
            }
        }
    }
}

// Overload for checking when only the handle is available (i.e. in the Widi shim)
void HwcTestKernel::checkWidiBuffer(buffer_handle_t handle)
{
    ...
    HWCLOGD("checkWidiBuffer CRTC %d frame:%d ll=%p handle=%p", crtc->GetCrtcId(), hwcFrame, ll, handle);
...
void HwcTestKernel::NotifyPageFlipHandlerExit(HwcTestCrtc* crtc, int firstUnsignalledRetireFence)
{
...
void HwcTestKernel::SetExtendedModeExpectation(bool singleFullScreenVideo, bool haveSecondDisplay, uint32_t hwcFrame)
{
    // Validate panel enable/disable state against shim assessment of the combination of
    // 1. the inputs from MDS (is there a video state, and is there an input timeout)
    ...
    HWCLOGD_COND(eLogVideo, "Extended mode check: @%p Sessions %d input state %d blanking req[0] %d single full screen video %d frame:%d",
        this, mVideoSessions.size(), mInputState, mCrtcByDisplayIx[0]->IsBlankingRequested(), singleFullScreenVideo, hwcFrame);

    if (mState->IsAutoExtMode())
    {
        mRequireExtendedMode = singleFullScreenVideo &&
                              haveSecondDisplay;
        mRequireEMPanel = haveSecondDisplay ? HwcGetTestConfig()->GetStableModeExpect()
                                            : HwcTestConfig::eOff;
        HWCLOGV_COND(eLogVideo, "mRequireEMPanel= %s", HwcTestConfig::Str(mRequireEMPanel));
    }
    else
    {
        // Legacy mode:
        // Extended mode requirement calculated using inputs from MDS
        mRequireExtendedMode = (mVideoSessions.size() > 0) &&    // Video session(s) in progress
                              singleFullScreenVideo &&
                              haveSecondDisplay;

        mRequireEMPanel = (mRequireExtendedMode && (!mInputState))
            ? HwcTestConfig::eOff : HwcTestConfig::eOn;
    }
    ...
    else
    {
        ...
        // Even if it does, the only effect would be some checks disabled for a few frames.
    ...
    {
        HWCERROR(eCheckExtendedModePanelControl, "Panel DPMS disabled when no other display active frame:%d", hwcFrame);
    }
    HWCCHECK(eCheckExtendedModePanelControl);

    // Log the power state
    HWCLOGV_COND(eLogVideo, "Display 0 power: %s", mCrtcByDisplayIx[0]->ReportPower(strbuf));

    HwcTestConfig::PanelModeType requireEMPanel;

    // Don't report an extended mode error if it's less than 4 frames since the state changed
    ...
    switch (requireEMPanel)
    {
        case HwcTestConfig::eOn:
        {
            if (!mCrtcByDisplayIx[0]->IsBlankingRequested())
            {
                ...
                {
                    HWCERROR(eCheckExtendedModePanelControl, "Panel DPMS Disabled when extended mode not expected and display not blanked frame:%d", hwcFrame);
                }
            }
        ...
        case HwcTestConfig::eOff:
        {
            if (mCrtcByDisplayIx[0]->IsDPMSEnabled())
            {
                HWCERROR(eCheckExtendedModePanelControl, "Extended mode expected but panel not DPMS disabled frame:%d", hwcFrame);
            }
        ...
        {
            HWCLOGD_COND(eLogVideo, "Extended mode change not stable (%d frames since change) or undefined, not validating frame:%d",
...
// Using these three calls, HWC tells the validation about any HWC compositions
// (particularly partitioned composer) that it carries out.
//
void* HwcTestKernel::CreateContext(const char* composer)
{
    return CreateCompositionContext(composer);
}

void HwcTestKernel::AddSource(void* ctx, const HwcTestKernel::HwcValLayer& layer, const char* description)
{
    if (!ctx) {
        HWCLOGW("HwcTestKernel::AddSource called with NULL ctx");
        return;
    }
    DrmShimTransformVector& contributors = *((DrmShimTransformVector*)ctx);
    AddCompositionSource(contributors, layer, description);
}

void HwcTestKernel::CheckComposition(void* ctx, const HwcTestKernel::HwcValLayer& layer, const char* description)
{
    PushThreadState ts("CheckComposition");

    if (!ctx) {
        HWCLOGW("HwcTestKernel::CheckComposition called with NULL ctx");
        return;
    }
    DrmShimTransformVector& contributors = *((DrmShimTransformVector*)ctx);
    CheckComposition(contributors, layer, description);
}

DrmShimTransformVector* HwcTestKernel::CreateCompositionContext(const char* composer)
{
    // Currently this only supports one composition context at a time
    // i.e. single threaded
    // Could be made multi-threaded if necessary.
    PushThreadState ts("CreateCompositionContext (locking)");
    HWCVAL_LOCK(_l,mMutex);
    SetThreadState("CreateCompositionContext (locked)");
    mWorkQueue.Process();

    // Do we want to record this composition?

    // Strip the composer string and prefix and suffix with :
    const int strbuflen = 64;
    char str[strbuflen];
    str[0] = ':';
    char* pStr = str+1;

    for (const char* p = composer; *p; p++)
    {
        if (isalnum(*p))
        {
            *pStr++ = *p;
    ...
    *pStr = '\0';
    ...
    if (strstr(mComposerFilter, str) != 0)
    {
        HWCLOGD_COND(eLogIvp, "Composition: Create context %s", composer);
        // Give HWC a context
        mCompContributors.clear();
        mCompLayerIx = 0;
        mCompSources = Hwcval::LayerList();
        return &mCompContributors;
    }
    else
    {
        HWCLOGD("Composition: ignored %s", composer);
        // Tell HWC we don't care about this composition
        return 0;
    }
}

bool HwcTestKernel::BelievedEmpty(buffer_handle_t handle)
{
    ...
    {
        HWCERROR(eCheckGrallocDetails, "HwcTestKernel::BelievedEmpty failed to get gralloc details");
        return false;
    }

    // HWC uses empty 16x16 buffers as inputs to create black output buffers.
    // We detect that this is believed to be a black buffer by the combination of:
    // (a) the handle is not known to us - was not in a layer list, and
    // (b) the buffer is 16x16.
    // Point (a) is not checked here, it is an entry condition.
    HWCLOGD_COND(eLogIvp, "BelievedEmpty: handle %p %dx%d", handle, bi.width, bi.height);
    return BelievedEmpty(bi.width, bi.height);
}
...
// Add an input to a HWC composition record that we are building up.
void HwcTestKernel::AddCompositionSource(DrmShimTransformVector& contributors,
                                         const HwcTestKernel::HwcValLayer& layer,
                                         const char* description)
{
    PushThreadState ts("AddCompositionSource (locking)");
    HWCVAL_LOCK(_l,mMutex);
    SetThreadState("AddCompositionSource (locked)");
    mWorkQueue.Process();
    ...
    HWCLOGD_COND(eLogBuffer, "HwcTestKernel::AddCompositionSource Enter handle %p", layer.handle);
    if (layer.handle)
    {
        buf = mBuffers.valueFor(layer.handle);
        HWCLOGD_COND(eLogBuffer, "HwcTestKernel::AddCompositionSource got buf@%p handle %p", buf.get(), layer.handle);
        ...
        if (buf == 0)
        {
            ...
            // (as a replacement for a layer with an invalid encryption state)?
            //
            if (!BelievedEmpty(layer.handle))
            {
                // No. So it is an error for us to be internally composing from a buffer
                // we have not previously come across.
                HWCLOGW("HWC Composition used on unknown buffer handle %p, assumed blanking", layer.handle);
                buf = RecordBufferState(layer.handle, Hwcval::BufferSourceType::Input, notes);
                buf->SetBlanking(true);
            }
            else
            {
                // Yes - this is a black buffer creted by HWC. Let's keep track of it.
                buf = RecordBufferState(layer.handle, Hwcval::BufferSourceType::Input, notes);
                buf->SetBlack(true);
        ...
        HWCLOGD_COND(eLogIvp,  "Composition: %s[%d] %s srcRect(l,t,r,b)=(%f, %f, %f, %f) destRect=(%d, %d, %d, %d) %s",
            description,
            mCompLayerIx,
            buf->IdStr(strbuf),
            (double) layer.sourceCropf.left,
            (double) layer.sourceCropf.top,
            (double) layer.sourceCropf.right,
            (double) layer.sourceCropf.bottom,
        ...
        DrmShimTransform transformObj(buf, mCompLayerIx, &layer);
        contributors.add(transformObj);

        HWCLOGD_COND(eLogIvp, "%s", transformObj.GetBlendingStr(strbuf));
    ...
    {
        Hwcval::Hwc1Layer vl(&layer, buf);

        if (mCompSources.GetNumLayers() > 0)
        {
            // bugfix here - save changing HWC
            vl.SetFlags(0);
        }
...
// Complete the recording of the composition:
// - Record the buffer state of the output buffer.
// - Attach the transform list to the DrmShimBuffer of the output buffer.
// - If HWC composition validation is enabled, also request an equivalent reference composition
//   so the results can be compared with HWC's composition later. (The request may be denied if the composition validation thread is busy).
void HwcTestKernel::CheckComposition(DrmShimTransformVector& contributors, const HwcTestKernel::HwcValLayer& layer, const char* description)
{
    HWCCHECK(eCountHwcComposition);
    PushThreadState ts("CheckComposition (locking)");
    HWCVAL_LOCK(_l,mMutex);
    SetThreadState("CheckComposition (locked)");
    mWorkQueue.Process();
    ...
    ++mPartitionedCompositionCount;

    if (layer.handle)
    {
        buf = RecordBufferState(layer.handle, Hwcval::BufferSourceType::PartitionedComposer, notes);

        HWCLOGD_COND(eLogIvp,  "Composition: %s target %s srcRect(l,t,r,b)=(%f,%f,%f,%f) %s",
            description,
            buf->IdStr(strbuf),
            (double) layer.sourceCropf.left,
            (double) layer.sourceCropf.top,
        ...
        bool sourcesSuitable = true;

        for (uint32_t i=0; i<contributors.size(); ++i)
        {
            DrmShimTransform& contrib = contributors.editItemAt(i);

            mPCScaleStat.Add(contrib.GetXScale());
            mPCScaleStat.Add(contrib.GetYScale());

            HWCCHECK(eCheckHwcProt);
            if (contrib.GetBuf()->IsReallyProtected())
            {
                HWCERROR(eCheckHwcProt, "Encrypted buffer %s passed to HWC Composition",
                    buf->IdStr(strbuf));
            }
        }

        // Attach the list of input transforms to the output buffer
        buf->SetAllCombinedFrom(contributors);

        HWCCHECK(eCheckHwcProt);
        if (buf->HasMediaDetailsEncrypted())
        {
            HWCERROR(eCheckHwcProt, "HWC Composition used to attempt to create encrypted buffer %s",
                buf->IdStr(strbuf));
        }

        if (mState->IsCheckEnabled(eCheckHwcCompMatchesRef) && sourcesSuitable)
        {
            // Remember which frame this composition was started in
            buf->SetLastHwcFrame(mFN);

            Hwcval::Hwc1Layer tgt(&layer, buf);

            // Initiate composition content check
            mCompVal->Compose(buf, mCompSources, tgt);
        }
    }
    else
    {
        HWCERROR(eCheckAllocFail, "%s composition buffer allocation failure", description);
        return;
    }
    HWCCHECK(eCheckAllocFail);
}
#endif //HWCVAL_ABSTRACTCOMPOSITIONCHECKER_EXISTS
...
void HwcTestKernel::SetHwcOption(android::String8& optionName, android::String8& value)
{
    HWCLOGD_COND(eLogParse, "Parsed HWC option %s: %s", optionName.string(), value.string());
...
int HwcTestKernel::GetHwcOptionInt(const char* optionName)
{
    ...
    {
        int ret = atoi(str);
        HWCLOGD_COND(eLogOptionParse, "GetHwcOptionInt(%s) atoi(%s)=%d", optionName, str, ret);
        return ret;
    }
    else
    {
...
void HwcTestKernel::ParseDmConfig(const char* str)
{
    ...
    while (*p)
    {
        ...
        if (strncmpinc(p, "MOSAIC") == 0)
        {
            ...
            {
                HWCLOGV_COND(eLogParse, "ParseDmConfig: display type not matched: %s", p);
                return;
            }
            ...
            while (strncmpinc(p, "(P:") == 0)
            {
                ...
                skipws(p);
                HWCLOGV_COND(eLogParse, "Matched source crop %d,%d %dx%d leaving %s", sx ,sy, sw, sh, p);
                ...
                skipws(p);
                HWCLOGV_COND(eLogParse, "Matched dest offset %d,%d leaving %s", dx, dy, p);

                // Finally the physical display size and rate we need to match
                uint32_t phx = atoiinc(p);
                HWCLOGV_COND(eLogParse, "Matched physical display width %d leaving %s", phx, p);
                if (!ExpectChar(p,'x')) return;
                uint32_t phy = atoiinc(p);
                HWCLOGV_COND(eLogParse, "Matched physical display height %d leaving %s", phy, p);
                if (!ExpectChar(p,'@')) return;
                uint32_t phrate = atoiinc(p);
                HWCLOGV_COND(eLogParse, "Matched physical display refresh %d leaving %s", phrate, p);
                if (!ExpectChar(p,')')) return;
                skipws(p);

                if ((disp >= HWCVAL_MAX_CRTCS) || (logDisp > HWCVAL_MAX_LOG_DISPLAYS))
                {
                    HWCLOGW("Invalid display config: bad display index SF:%d P:%d", logDisp, disp);
                    return;
                }
                Hwcval::LogDisplayMapping ldm(logDisp, disp, flags, sx, sy, sw, sh, dx, dy, sw, sh);
                ...
                if (crtc == 0)
                {
                    HWCLOGD_COND(eLogMosaic, "ParseDmConfig: No P%d => No mapping will be set", disp);
                    return;
                }
        ...
        else if (strncmpinc(p, "PASSTHROUGH") == 0)
        {
            ...
            // For now, if we find a passthrough display, we assume that it will match to default operation
...
void HwcTestKernel::MapLogDisplays(android::Vector<Hwcval::LogDisplayMapping> mappings)
{
...
{
    mWorkQueue.Push(new Hwcval::Work::GemOpenItem(fd, gemOpen->name, gemOpen->handle));
}
...
{
    mWorkQueue.Push(new Hwcval::Work::GemFlinkItem(fd, flink->name, flink->handle));
}

void HwcTestKernel::CheckIoctlGemWait(int fd, struct drm_i915_gem_wait* gemWait, int status, int64_t delayNs)
{
    mWorkQueue.Push(new Hwcval::Work::GemWaitItem(fd, gemWait->bo_handle, status, delayNs));

    // Optionally stall the gem thread
    DoStall(Hwcval::eStallGemWait);
}

void HwcTestKernel::CheckIoctlGemCreate(int fd, struct drm_i915_gem_create* gemCreate)
{
...
{
    char strbuf[HWCVAL_DEFAULT_STRLEN];
    HWCLOGD_COND(eLogBuffer, "Processing buffer free item for handle %p", item.mHandle);
    ...
    {
        HWCLOGI("Attempt to remove unknown buffer handle %p from mBuffers", item.mHandle);
    }
}
    ...
    if (item.mBoHandle)
    {
        ...
        if (boIx >= 0)
        {
            ...
            if (oldBuf.get())
            {
                ...
                {
                    // Yes - nothing to do
                    HWCLOGD_COND(eLogBuffer, "IOCTL_GEM_OPEN: %s already associated with buf %s",
                ...
                {
                    // No - so (a) the old DrmShimBuffer no longer maps to this bo (buffer object)
                    HWCLOGD_COND(eLogBuffer, "IOCTL_GEM_OPEN: %s presently associated with buf %s (new " BUFIDSTR " 0x%x), removing",
                        bo->IdStr(str), oldBuf->IdStr(str2), item.mId);
                    oldBuf->RemoveBo(bo);

                    //          (b) the DrmShimBuffer for our global ID does now map to this bo.
                    if (buf.get())
                    {
                        buf->AddBo(bo);
                        bo->mBuf = buf;
        ...
        // If we get this far, it's normally because we don't know anything about this bo handle.
        if (ix >= 0)
        {
            // We know the global ID, so create the bo record and attach it to the DrmShimBuffer
            // for that global ID.
            if (bo.get() == 0)
            {
            ...
            HWCLOGD_COND(eLogBuffer, "IOCTL_GEM_OPEN " BUFIDSTR " 0x%x -> fd %d boHandle 0x%x %s",
        ...
        else
        {
            ...
            // We are recording the opening of a buffer we haven't seen before, we expect to
            ...
            HWCLOGD_COND(eLogBuffer, "IOCTL_GEM_OPEN " BUFIDSTR " 0x%x -> fd %d boHandle 0x%x handle NOT YET ALLOCATED",
...
void HwcTestKernel::DoGem(const Hwcval::Work::GemFlinkItem& item)
{
    ...
    if (boHandleIx >= 0)
    {
        ...
        if (buf.get() == 0)
        {
            ...
            if (ix >= 0)
            {
                ...
                {
                    HWCLOGD_COND(eLogBuffer, "Flink: @%p boHandle 0x%x already associated to %s",
            ...
            buf->AddBo(bo);
            bo->mBuf = buf;
            HWCLOGD_COND(eLogBuffer, "IOCTL_GEM_FLINK: Linked " BUFIDSTR " 0x%x in new buf@%p to fd %d boHandle 0x%x handle %p",
                item.mId, buf.get(), item.mFd, item.mBoHandle, buf->GetHandle());
        }
        else if (buf->GetGlobalId() == 0)
        {
            // Normal case: We are linking an open buffer object to a name when it previously had none.
            MapGlobalId(item.mId, buf);
            HWCLOGD_COND(eLogBuffer, "IOCTL_GEM_FLINK: Linked " BUFIDSTR " 0x%x to fd %d boHandle 0x%x handle %p",
                item.mId, item.mFd, item.mBoHandle, buf->GetHandle());
        }
        else if (buf->GetGlobalId() != item.mId)
        {
            // We are requested to link an open buffer object to a name even though it prevously had one.
            // Let's assume that the old bo should be closed and a new one should be opened.
            buf->RemoveBo(bo);
            buf = new DrmShimBuffer(0);
            ...
            MapGlobalId(item.mId, buf);
            buf->SetBlanking(true);
            HWCLOGD_COND(eLogBuffer, "IOCTL_GEM_FLINK: boHandle 0x%x id has changed so we have created it and linked it to " BUFIDSTR " 0x%x",
    ...
    else
    {
        ...
        // Reading the struct definition, it appears that this call is invoked for the system to give a name to an already open buffer.
        // But we find that in some cases, the boHandle is one that has not been opened.

        if (ix >= 0)
        {
            android::sp<DrmShimBuffer> oldBuf = mBuffersByGlobalId.valueAt(ix);

            HWCLOGD_COND(eLogBuffer, "IOCTL_GEM_FLINK: Name 0x%x is new buffer, remove old association to handle %p (now linked to fd %d boHandle 0x%x)",
        ...
        {
            HWCLOGD_COND(eLogBuffer, "IOCTL_GEM_FLINK: fd %d boHandle 0x%x and name 0x%x not known",
        ...
        android::sp<HwcTestBufferObject> bo = CreateBufferObject(item.mFd, item.mBoHandle);
        buf->AddBo(bo);
        bo->mBuf = buf;
        ...
        BoKey k = {item.mFd, item.mBoHandle};
        mBosByBoHandle.add(k, bo);
        HWCLOGV_COND(eLogBuffer, "IOCTL_GEM_FLINK name 0x%x -> fd %d boHandle 0x%x handle NOT YET ALLOCATED",
            item.mId, item.mFd, item.mBoHandle);
        HWCLOGV_COND(eLogBuffer, "IOCTL_GEM_FLINK: %s", buf->IdStr(strbuf));
    }
}
...
void HwcTestKernel::DoGem(const Hwcval::Work::GemCreateItem& item)
{
    ...
    else
    {
        ...
        HWCLOGD_COND(eLogBuffer, "IOCTL_I915_GEM_CREATE bo already exists: %s", bo->IdStr(str));
    }
}

// Process the work item for a TIMEOUT of a GEM WAIT.
// No record is made of a successful GEM WAIT, as we don't require this information.
//
// A GEM WAIT is an IOCTL that is requesting to wait on a target buffer until composition is complete.
// So a timeout on one of these is likely to mean that iVP (usually) or perhaps partitioned composer
// has locked up.
void HwcTestKernel::DoGem(const Hwcval::Work::GemWaitItem& item)
{
    ...
    if (item.mBoHandle)
    {
        ...
        if (boHandleIx >= 0)
        {
            ...
            if (buf.get())
            {
                HWCERROR(eCheckDrmIoctlGemWaitLatency, "Timeout %fs waiting for %s boHandle 0x%x buffer %s (error %d)",
                    double(item.mDelayNs) / 1000000000.0,
                    buf->GetSourceName(),
                    item.mBoHandle, buf->IdStr(str), item.mStatus);
            }
            else
            {
                HWCERROR(eCheckDrmIoctlGemWaitLatency, "Timeout %fs waiting for buffer object %s (unknown buffer, error %d)",
                    double(item.mDelayNs) / 1000000000.0, bo->IdStr(str), item.mStatus);
            }
        }
        else
        {
            HWCERROR(eCheckDrmIoctlGemWaitLatency, "Timeout %fs waiting for unknown boHandle 0x%x",
                double(item.mDelayNs) / 1000000000.0, item.mBoHandle);
        }
    }
}

// Remove a buffer object from our internal tracking and update the data structures accordingly.
    ...
    if (ix >= 0)
    {
        ...
        if (buf.get())
        {
            ...
            if (buf->GetBos().size() == 0)
            {
                ...
                HWCLOGD_COND(eLogBuffer, "IOCTL_GEM_CLOSE: removed association from " BUFIDSTR " 0x%x to buf@%p handle %p",
        ...
        HWCLOGD_COND(eLogBuffer, "%s: Closed fd %d boHandle 0x%x %s",
            str, k.fd, k.h, buf.get() ? buf->IdStr(strbuf) : "");
        ValidateBo(buf, str);

    }
    else
    {
        // This does happen - boHandles get closed when they have already been closed.
...
android::sp<DrmShimBuffer> HwcTestKernel::UpdateIvpBufferState(buffer_handle_t handle)
{
    ATRACE_CALL();
    mWorkQueue.Process();
    ...
    if (ix >= 0)
    {
        ...
        {
            HWCERROR(eCheckIvpChangeTgtEncryption, "iVP has changed the encrypted state on %s to NOT ENCRYPTED", buf->IdStr(strbuf));
        }
        else if (!wasProtected && buf->HasMediaDetailsEncrypted())
        {
            HWCERROR(eCheckIvpChangeTgtEncryption, "iVP has changed the encrypted state on %s to ENCRYPTED", buf->IdStr(strbuf));
        }
    }
    else
    {
        HWCERROR(eCheckInternalError, "Buffer handle %p has disappeared from mBuffers between entrance and exit from iVP_exec",
            handle);
    }
...
// and should only be used if this is going wrong - for example there appears to be confusion
// as to which buffer is which, or SEGVs are occurring in this area.
//
// This should not be necessary unless there is a spec change in gralloc, GEM, or the way
// these are used.
void HwcTestKernel::ValidateBo(android::sp<DrmShimBuffer> buf, const char* str)
{
    char strbuf[HWCVAL_DEFAULT_STRLEN];
    ...
    if (buf.get())
    {
        ...
        if (id > 0)
        {
            ...
            if (ix < 0)
            {
                HWCERROR(eCheckInternalError, "Name not indexed: %s", buf->IdStr(strbuf));
                BoValidationAssert();
            }
        }
...
void HwcTestKernel::ValidateBo(android::sp<HwcTestBufferObject> bo, android::sp<DrmShimBuffer> buf, const char* str)
{
    if (bo.get())
    {
        char strbuf[HWCVAL_DEFAULT_STRLEN];
        ...
        if (parentBuf.get())
        {
            ...
            if (boIx < 0)
            {
                HWCERROR(eCheckTestBufferAlloc, "%s: BO %s missing ref from %s",
                    str, bo->IdStr(strbuf), parentBuf->IdStr(strbuf2));
                BoValidationAssert();
            }
        }
        ...
        if (ix < 0)
        {
            HWCERROR(eCheckTestBufferAlloc, "%s: BO %s not indexed", str, bo->IdStr(strbuf));
            if (buf.get())
            {
        ...
        else
        {
            ...
            if (buf.get())
            {
                if (parentBuf != buf)
                {
                    HWCERROR(eCheckTestBufferAlloc, "%s: BO %s does not point to parent %s",
                        str, bo->IdStr(strbuf), buf->IdStr(strbuf2));
                    if (parentBuf.get())
                    {
                        HWCLOGV_COND(eLogBuffer, "  -- instead %s", parentBuf->IdStr(strbuf));
                        BoValidationAssert();
                    }
                }
            }
...
// This is the main function for recording the state of a buffer where we know its gralloc handle.
// This includes its interception from SF/the harness (i.e. CheckSetEnter) and also interception of compositions.
//
// The caller indicates the source of the buffer so we can distinguish between SF inputs, composition and other cases.
android::sp<DrmShimBuffer> HwcTestKernel::RecordBufferState(buffer_handle_t handle, Hwcval::BufferSourceType bufferSource, char* notes)
{
    ATRACE_CALL();
    mWorkQueue.Process();
    notes[0] = '\0';
    ...
    HWCLOGD_COND(eLogBuffer, "HwcTestKernel::RecordBufferState Enter handle %p", handle);
    ...
    if (DrmShimBuffer::GetBufferInfo(handle, &bi))
    {
        HWCERROR(eCheckGrallocDetails, "HwcTestKernel::RecordBufferState Error obtaining gralloc details handle %p", handle);
    }
    ...
    HWCLOGD_COND(eLogBuffer, "HwcTestKernel::RecordBufferState got bo@%p handle %p", bo.get(), handle);
    if (bo.get())
    {
        boBuf = bo->mBuf.promote();
        HWCLOGD_COND(eLogBuffer, "HwcTestKernel::RecordBufferState got buf@%p bo@%p handle %p", boBuf.get(), bo.get(), handle);
    }
#endif

    // For buffers appearing in the SF layer list only, we need to consider if this is the
    // re-appearance of a buffer that was in the previous frame's layer list.
    // If so, we assume it is the same content and hence can be represented by the same
    // DrmShimBuffer. Otherwise (even if it is the same buffer handle as a buffer that
    // appeared in the layer list 2 frames ago) it is assumed to be different content and
    // hence will be represented by a different DrmShimBuffer.
    //
    // For buffers created by iVP on the other hand, we assume that each time there is a composition
    // we are creating new content as HWC will not repeat a composition that it has already
    // done.
    ssize_t ix = mBuffers.indexOfKey(handle);
    ...
    if (ix >= 0)
    {
        ...
        HWCLOGD_COND(eLogBuffer, "RecordBufferState: Existing buf@%p found in mBuffers, handle %s", buf.get(), handle);
            ...
            (bufferSource == Hwcval::BufferSourceType::SfComp))
        {
            ...
            if (buf->IsCurrent(mFN) &&
               // ... and was it in the OnSet (buffer can't transition from being a composition target
               // to being in the OnSet layer list - if it appears to, it is a different buffer with
               // the same handle
               ( (buf->GetSource() == Hwcval::BufferSourceType::Input) ||
                 (buf->GetSource() == Hwcval::BufferSourceType::SfComp) ))
            {
                notesLen += snprintf(notes+notesLen, HWCVAL_DEFAULT_STRLEN-notesLen, "LASTFRAME ");

                // If the buffer has the same name as the one we know
#if INTEL_UFO_GRALLOC_HAVE_PRIME
                if (((bi.NAME_OR_PRIME == buf->GetGlobalId()) || (buf->GetGlobalId() == 0)) &&
                    ((boBuf == buf) || (boBuf == 0)))
#else
                if (bi.NAME_OR_PRIME == buf->GetGlobalId())
#endif
                {
                    notesLen += snprintf(notes+notesLen, HWCVAL_DEFAULT_STRLEN-notesLen, "UNCHANGED" BUFIDSTR);
                    ...
                    if (bo.get())
                    {
                        bo->mBuf = buf;
                        buf->AddBo(bo);
                        HWCLOGD_COND(eLogBuffer, "HwcTestKernel::RecordBufferState adding bo@%p to buf@ handle %p", bo.get(), buf.get(), handle);
                    }
#endif
                    // Hence, we are returning the existing DrmShimBuffer record.
                    HWCLOGD_COND(eLogBuffer, "RecordBufferState: Exit buf@%p IdStr: " BUFIDSTR " %s", buf.get(), buf->IdStr(strbuf));
                    return buf;
                }
                else
                {
                    notesLen += snprintf(notes+notesLen, HWCVAL_DEFAULT_STRLEN-notesLen, "DIFFERENT" BUFIDSTR);
                }
            }
            else
            {
                // Because this buffer has not been seen sufficiently recently, we have decided to treat this as new
                // content.
                // Therefore, we will drop through to the code that will create a new DrmShimBuffer.
                // We remember what the existing DrmShimBuffer is so we can transfer the FB IDs and buffer objects
                // to the new DrmShimBuffer.
                notesLen += snprintf(notes+notesLen, HWCVAL_DEFAULT_STRLEN-notesLen, "NEWCONTENT (%s open %d new %d)",
                    buf->GetHwcFrameStr(strbuf), buf->GetOpenCount(), buf->IsNew());
                buf->SetBlanking(false);
                existingBuf = buf;
                HWCLOGD_COND(eLogBuffer, "RecordBufferState: Existing buf@%p is too old for handle %s", buf.get(), handle);
            }
        }
        else // buffer is a composition target
        {
            ...
            HWCLOGD_COND(eLogBuffer, "RecordBufferState: Existing buf@%p is composition target %s", buf.get(), handle);
        }
    ...
    if (nameIx >= 0)
    {
        // We found the (global) buffer name/prime. So a GEM OPEN must have been called...
        android::sp<DrmShimBuffer> altbuf = mBuffersByGlobalId.valueAt(nameIx);
        altbuf->ResetAppearanceCount(); // Reset consecutive appearance count to zero.
        HWCLOGD_COND(eLogBuffer, "RecordBufferState: Existing buf@%p found in mBuffersByGlobalId, handle %s", altbuf.get(), handle);

        if (altbuf->GetHandle() == 0)
        {
            // The buffer name/prime has not yet been associated with a handle, so we can do that now.
            ...
            HWCLOGD_COND(eLogBuffer, "RecordBufferState: Exit buf@%p IdStr: " BUFIDSTR " %s", altbuf.get(), altbuf->IdStr(strbuf));

            return altbuf;
        }
        else if (altbuf.get() != existingBuf.get())
        {
            HWCLOGD_COND(eLogBuffer, "RecordBufferState: existing buf@%p handle %p: " BUFIDSTR " 0x%x presently associated with DIFFERENT %s",
    ...
    HWCLOGD_COND(eLogBuffer, "RecordBufferState: new buf@%p handle %p: ", buf.get(), handle);


#if INTEL_UFO_GRALLOC_HAVE_PRIME
    boBuf = bo->mBuf.promote();
    HWCLOGD_COND(eLogBuffer, "RecordBufferState: Checking boBuf buf@%p bo@%p vs buf@%p", boBuf.get(), bo.get(), buf.get());

    // Fix up links between buffer and buffer object in the case where these have changed.
    if (boBuf != buf)
    {
        if (boBuf.get() != 0)
        {
            boBuf->RemoveBo(bo);
            HWCLOGD_COND(eLogBuffer, "RecordBufferState: Removed bo@%p from buf@%p", bo.get(), boBuf.get());

            if ((existingBuf.get() == 0) ||
                (existingBuf->GetBos().size() == 0) ||
                ((existingBuf->NumFbIds() == 0) && (boBuf->NumFbIds() > 0)) )
            {
                existingBuf = boBuf;
                HWCLOGD_COND(eLogBuffer, "RecordBufferState: Using boBuf buf@%p as existing buf@%pIdStr", boBuf.get(), existingBuf.get());
            }
        }

        bo->mBuf = buf;
        buf->AddBo(bo);
        HWCLOGD_COND(eLogBuffer, "RecordBufferState: Added bo@%p from buf@%p", bo.get(), buf.get());
    }
#endif
...
#endif
    {
        ...
        // a copy.
        // The reason why the new buffer must have the original buffer objects is that it
        ...
        HWCLOGD_COND(eLogBuffer, "RecordBufferState: buf@%p takes bos from buf@%p", buf.get(), existingBuf.get());

        for (uint32_t i=0; i<buf->GetBos().size(); ++i)
        {
            android::sp<HwcTestBufferObject> existingBo = existingBuf->GetBos().itemAt(i);
            android::sp<HwcTestBufferObject> newBo = existingBo->Dup();
            bosForOldBuf.add(newBo);
            HWCLOGD_COND(eLogBuffer, "RecordBufferState: existing bo@%p copied to bo@%p", existingBo.get(), newBo.get());
        ...
        HWCLOGD_COND(eLogBuffer, "RecordBufferState: buf@%p takes copied bos", existingBuf.get());
    ...
    HWCLOGD_COND(eLogBuffer, "HwcTestKernel::RecordBufferState Exit buf@%p handle %p", buf.get(), handle);

    return buf;
}

void HwcTestKernel::AddZOrder(uint32_t order, uint32_t seq, uint32_t planeOffset)
{
    // order is the Z-order value passed to DRM
    // seq is the z-order logical sequence we want to add for plane (CRTC + planeOffset)
...
void HwcTestKernel::SendFrameCounts(bool clear)
{
    ...
    for (uint32_t i=0; i<HWCVAL_MAX_CRTCS; ++i)
    {
        if (mCrtcByDisplayIx[i])
        {
            HwcTestResult::PerDisplay* resultPerDisp = HwcGetTestResult()->mPerDisplay + i;
            uint32_t dropped;
            ...
            if (resultPerDisp->mFrameCount > 50)
            {
                ...
                {
                    HWCERROR(eCheckTooManyDroppedFrames, "Display %d had %d frames dropped out of %d (%d%%)",
                        i, dropped, resultPerDisp->mFrameCount, (100 * dropped) / resultPerDisp->mFrameCount);
                }

            }
            // For now, this is a very generous limit.
            // TODO: Something cleverer, by allowing dropped frames only when there is a good reason
            // such as being in the process of setting display resolution.
            HWCCHECK(eCheckTooManyConsecutiveDroppedFrames);
            if (consecutive > 120)
            {
                HWCERROR(eCheckTooManyConsecutiveDroppedFrames, "Display %d had %d consecutive dropped frames",
                    i, consecutive);
            }
...
void HwcTestKernel::UpdateVideoState(int64_t sessionId, bool isPrepared)
{
    ...
    if (isPrepared)
    {
        HWCLOGD_COND(eLogVideo, "HwcTestKernel::UpdateVideoState add session %d", sessionId);
        if (ix < 0)
        {
    ...
    {
        HWCLOGD_COND(eLogVideo, "HwcTestKernel::UpdateVideoState remove session %d", sessionId);
        if (ix >= 0)
        {
...
HwcTestCrtc* HwcTestKernel::GetHwcTestCrtcByDisplayIx(uint32_t displayIx, bool persistentCopy)
{
    ...
    if (displayIx < HWCVAL_MAX_CRTCS)
    {
        if (displayIx != HWCVAL_VD_WIDI_DISPLAY_INDEX)
        {
            ...
            if (ll)
            {
                ...
                // there was a final frame that should have been displayed. We call that dropped too.
                ++droppedFrames;
            }
        }
        else if (mWidiState == eWidiEnabled) // Widi / Virtual display
        {
            ...
            //   current frame number - last frame number seen in Widi shim - 1 (potentially)
            droppedFrames = GetHwcFrame(HWCVAL_VD_WIDI_DISPLAY_INDEX) - GetWidiLastFrame();
            if (droppedFrames >= 1)
            {
                --droppedFrames;
            }
...
{
    // Wait 3.1 seconds for stability - this is longer than the 3 second page flip timeout, but still no guarantee.
    usleep(3100000);
    ...
    if ((mSnapshotsRestored.GetValue() > 10) && (mSnapshotsRestored.GetValue() > frameCount / 1000))
    {
        HWCERROR(eCheckTooManySnapshotsRestored, "%d snaphots restored", mSnapshotsRestored.GetValue());
    }
}
...
void HwcTestKernel::CheckInvalidSessionsDisplayed()
{
    ...
    if ((systemTime(SYSTEM_TIME_MONOTONIC) - mLastOnPrepareTime) > 2000000000) // 2 seconds
    {
    ...
    for (uint32_t i=0; i<mPlanes.size(); ++i)
    {
        ...
        if (crtc)
        {
            ...
            {
                HWCLOGW("CheckInvalidSessionsDisplayed: Plane %d CRTC %p has no CRTC id", plane->GetPlaneId(), crtc);
                return;
            }
            ...
            if (buf.get())
            {
                Hwcval::ValidityType val = buf->ProtectedContentValidity(mProtChecker, hwcFrame);
                if ((val != ValidityType::Valid) && (val != ValidityType::ValidUntilModeChange) && (val != ValidityType::Invalidating))
                {
                    HWCERROR(eCheckInvProtDisp, "Buffer %s %s has invalid protected content and was still displayed on plane %d at end of DisableEncryptedSessions()",
                        buf->IdStr(strbuf), buf->EncryptionStr(strbuf2), plane->GetPlaneId());
                }
            }
        }
    }
...
uint32_t HwcTestKernel::GetMDSVideoRate()
{
    ...
    else
    {
        ...
        HWCLOGV_COND(eLogVideo, "GetMDSVideoRate: %d sessions, so no video rate", mVideoSessions.size());
    }
...
void HwcTestKernel::ValidateOptimizationMode(Hwcval::LayerList* ll)
{
    ...
    if (!expectLowDDR)
    {
        ...
        if (autoLowDDR)
        {
            HWCLOGV_COND(eLogVideo, "ValidateOptimizationMode: autolowddr set: clearing expectNormalDDR");
            ...
            if (!mState->HotPlugInProgress())
            {
                ...
                // Currently low DDR mode is ONLY used when there is one display active and it is the panel.
                // If the panel is disabled (extended mode) then low DDR should not be used.
                // If this ceases to be the case, the second part of the following if statement can be removed.
                if (numDisplays == 1 && mCrtcByDisplayIx[0] && mCrtcByDisplayIx[0]->IsDisplayEnabled())
                {
                    if (mCrtcByDisplayIx[0]->IsProtectedLayerRemoved())
                    {
                        HWCLOGV_COND(eLogVideo, "HWC replaced protected layer with black: skipping low DDR check");
                    }
                    else
                    {
                        expectLowDDR = (ll->GetVideoFlags().mFullScreenVideo == eTrue);
                        expectNormalDDR = (ll->GetVideoFlags().mFullScreenVideo == eFalse);
                        HWCLOGV_COND(eLogVideo, "ValidateOptimizationMode: auto: 1 display FS %s expectNormalDDR=%d expectLowDDR=%d",
                ...
                {
                    HWCLOGV_COND(eLogVideo, "ValidateOptimizationMode: auto: %d displays active, not expecting low DDR", numDisplays);
                }
            }
            else
            {
                HWCLOGV_COND(eLogVideo, "ValidateOptimizationMode: Hotplug in progress, no DDR expectation");
            }
        }
        else
        {
            HWCLOGV_COND(eLogVideo, "ValidateOptimizationMode: autovideoddr disabled");
        }
    }
    else
    {
    ...
    if (mDDRMode == mDDRModeLastFrame)
    {
        ...
        if (expectLowDDR)
        {
            ...
            {
                HWCERROR(eCheckDDRMode, "DDR mode is normal, we are expecting LOW. (force %d auto %d)", mForceLowDDRMode, autoLowDDR);
            }
            else
            {
...
bool HwcTestKernel::IsWidiVideoMode(Hwcval::LayerList::VideoFlags vf, uint32_t hwcFrame)
{
    if (!mState->IsAutoExtMode())
    {
    ...
    return  !mExtVideoModeDisabled &&
            ...
            // Rotation animation still in progress so we won't leave extended mode
...
void HwcTestKernel::SetSnapshot(buffer_handle_t handle, uint32_t keepCount)
{
    ...
    HWCLOGD("SetSnapshot: Buffer handle %p is snapshot. Rotation frame:%d-%d", handle, mRotationStartFrame, mRotationEndFrame);
    if (ix >= 0)
    {
...
{
    bool ret = ((hwcFrame >= mRotationStartFrame) && (hwcFrame <= mRotationEndFrame));
...
bool HwcTestKernel::IsSnapshot(buffer_handle_t handle, uint32_t hwcFrame)
{
    if (mSnapshots.size() > 0)
    {
        ...
        if (ix >= 0)
        {
            ...
            {
                HWCLOGD("IsSnapshot: Buffer handle %p is snapshot now and until frame %d", handle, expiryFrame);
                return true;
            }
            else
            {
                HWCLOGD("IsSnapshot: Deleting snapshot record for handle %p, now frame:%d, expired at %d",
