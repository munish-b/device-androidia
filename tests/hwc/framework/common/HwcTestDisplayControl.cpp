...
HwcTestDisplayControl::HwcTestDisplayControl(uint32_t disp,
                                             sp<IDisplayControl> real,
                                             HwcTestKernel *testKernel)
    : mDisplayIx(disp), mReal(real), mTestKernel(testKernel) {
...
HwcTestDisplayModeControl::HwcTestDisplayModeControl(
    uint32_t disp, sp<IDisplayModeControl> real, HwcTestKernel *testKernel)
    : mDisplayIx(disp), mReal(real), mTestKernel(testKernel) {
...
status_t HwcTestDisplayModeControl::restorePreferredMode() {
  ...
  if (mCrtc) {
    ...
    HWCERROR(eCheckInternalError,
             "Can't restore preferred mode for display %d as no CRTC defined",
             mDisplayIx);
  }
...
status_t HwcTestDisplayModeControl::getMode(uint32_t *width, uint32_t *height,
...
status_t HwcTestDisplayModeControl::setMode(uint32_t width, uint32_t height,
                                            uint32_t refresh, uint32_t flags,
                                            uint32_t ratio) {
  mCrtc = mTestKernel->GetHwcTestCrtcByDisplayIx(mDisplayIx, true);
  HWCLOGA(
      "HwcTestDisplayModeControl::setMode Enter %dx%d refresh %d flags %d %s "
      "ratio %d",
  ...
  HWCLOGA(
      "HwcTestDisplayModeControl::setMode Exit %dx%d refresh %d flags %d ratio "
      "%d",
