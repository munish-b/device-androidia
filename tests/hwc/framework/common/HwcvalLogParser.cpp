...
bool Hwcval::LogParser::ParseEnableEncryptedSessionEntry(pid_t pid,
                                                         int64_t timestamp,
                                                         const char* str) {
  HWCVAL_UNUSED(pid);
  ...
  if (MatchRegex(
          "HwcService_Video_EnableEncryptedSession sessionID:(\\d+) "
          "instanceID:(\\d+) -->",
          str, &num_fields_matched, &mParsedEncEnableSession,
          &mParsedEncEnableInstance)) {
    HWCCHECK(eCheckLogParserError);
    if (num_fields_matched == 2) {
      HWCLOGD_COND(
          eLogParse,
          "PARSED MATCHED %s - enabling encrypted State for session: %d "
          "instance %d",
          str, mParsedEncEnableSession, mParsedEncEnableInstance);
...
bool Hwcval::LogParser::ParseEnableEncryptedSessionExit(pid_t pid,
                                                        int64_t timestamp,
                                                        const char* str) {
  HWCVAL_UNUSED(pid);
  ...
  if (MatchRegex("HwcService_Video_EnableEncryptedSession .* <--", str)) {
    HWCLOGD_COND(eLogParse, "PARSED MATCHED %s - enable encrypted session exit",
                 str);
    ...
    if (ns > (15 * HWCVAL_MS_TO_NS)) {
      HWCERROR(eCheckProtEnableStall,
               "Enabling encrypted session %d instance %d took %fms",
               mParsedEncEnableSession, mParsedEncEnableInstance,
               double(ns) / double(HWCVAL_MS_TO_NS));
    }
...
bool Hwcval::LogParser::ParseDisableEncryptedSessionEntry(pid_t pid,
                                                          int64_t timestamp,
                                                          const char* str) {
  HWCVAL_UNUSED(pid);
  ...
  if (MatchRegex(
          "HwcService_Video_DisableEncryptedSession sessionID:(\\d+) -->", str,
          &num_fields_matched, &mParsedEncDisableSession)) {
    HWCCHECK(eCheckLogParserError);
    if (num_fields_matched == 1) {
      HWCVAL_LOCK(_l, mTestKernel->GetMutex());
      mParsedEncDisableStartTime = timestamp;
      HWCLOGD_COND(eLogParse,
                   "PARSED MATCHED %s - disabling encrypted state for "
                   "session: %d - current time is: %f",
                   str, mParsedEncDisableSession,
                   double(mParsedEncDisableStartTime) / HWCVAL_SEC_TO_NS);
...
bool Hwcval::LogParser::ParseDisableEncryptedSessionExit(pid_t pid,
                                                         int64_t timestamp,
                                                         const char* str) {
  HWCVAL_UNUSED(pid);
  ...
  if (MatchRegex("HwcService_Video_DisableEncryptedSession .* <--", str)) {
    HWCLOGD_COND(eLogParse,
                 "PARSED MATCHED %s - disabling encrypted session exit", str);
    HWCVAL_LOCK(_l, mTestKernel->GetMutex());
    mProtChecker.DisableEncryptedSessionExit(
        mParsedEncDisableSession, mParsedEncDisableStartTime, timestamp);
...
bool Hwcval::LogParser::ParseDisableAllEncryptedSessionsEntry(pid_t pid,
                                                              int64_t timestamp,
                                                              const char* str) {
  HWCVAL_UNUSED(pid);
  HWCVAL_UNUSED(timestamp);
  ...
  if (MatchRegex("HwcService_Video_DisableAllEncryptedSessions -->", str)) {
    HWCLOGD_COND(eLogParse,
                 "PARSED MATCHED %s - disable all encrypted session entry",
                 str);
    HWCVAL_LOCK(_l, mTestKernel->GetMutex());
...
bool Hwcval::LogParser::ParseDisableAllEncryptedSessionsExit(pid_t pid,
                                                             int64_t timestamp,
                                                             const char* str) {
  HWCVAL_UNUSED(pid);
  HWCVAL_UNUSED(timestamp);
  ...
  if (MatchRegex("HwcService_Video_DisableAllEncryptedSessions .* <--", str)) {
    HWCLOGD_COND(eLogParse,
                 "PARSED MATCHED %s - disable all encrypted session exit", str);
    HWCVAL_LOCK(_l, mTestKernel->GetMutex());
...
bool Hwcval::LogParser::ParseWidiSetSingleDisplayEntry(pid_t pid,
                                                       int64_t timestamp,
                                                       const char* str) {
  HWCVAL_UNUSED(pid);
  HWCVAL_UNUSED(timestamp);
  ...
  if (MatchRegex("HwcService_Widi_SetSingleDisplay (\\d+) -->", str,
                 &num_fields_matched, &mode)) {
    HWCLOGD_COND(eLogParse,
                 "PARSED MATCHED %s - setting Widi single display mode to: %d",
                 str, mode);
...
bool Hwcval::LogParser::ParseWidiSetSingleDisplayExit(pid_t pid,
                                                      int64_t timestamp,
                                                      const char* str) {
  HWCVAL_UNUSED(pid);
  HWCVAL_UNUSED(timestamp);
...
bool Hwcval::LogParser::ParseOption(const char*& p) {
  ...
  if (endOfValue) {
    HWCLOGV_COND(eLogOptionParse, "%s pWords[0] %p %s", optionName.string(),
                 pWords[0], pWords[0]);
    ...
    value = words[0];
    HWCLOGV_COND(eLogOptionParse, "ParseOption: %s = %s", optionName.string(),
                 value.string());
  }
