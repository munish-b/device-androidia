...
{
    // ALWAYS ensure this matches the values of the enumerate HwcTestComponentType.
    "None",
    "Test",
    "HWC",
    "Buffers",
...
#define DECLARE_CHECK(enumId, component, defaultPriority, description, category) description,
...
#define DECLARE_CHECK(enumId, component, defaultPriority, description, category) eComponent##component,
...
static android::String8 GetLongProp(const char* name)
{
    // We are combining together the properties "name", "name_1", "name_2" etc.
    android::String8 result;
    for (uint32_t pn = 0; pn<10; ++pn)
    {
        android::String8 propName;
        if (pn > 0)
        {
            propName = android::String8::format("%s_%d", name, pn);
            result += " ";
        }
        else
        {
            propName = android::String8(name);
        }

        char value[PROPERTY_VALUE_MAX + 1];

        if (!property_get(propName.string(), value, NULL))
        {
            break;
        }
    ...
    ALOGD("Long Property %s=\"%s\"",name,result.string());
    ...
    mStableModeExpect(eDontCare)
{
    ...
    if (property_get( "hwcval.default_log_priority", logPriorityStr, NULL ))
    {
        ...
        mMinLogPriority = (int)(strchr(priorities,  (int)logPriorityStr[0]) - priorities);
...
#define DECLARE_CHECK(enumId, component, defaultPriority, description, category) ANDROID_LOG_##defaultPriority,
    int priorities[] =
    {
#include "HwcTestCheckList.h"
        0
...
#define DECLARE_CHECK(enumId, component, defaultPriority, description, category) eCat##category,
    int categories[] =
    {
#include "HwcTestCheckList.h"
        0
...
#undef DECLARE_CHECK
    ...
    // N.B. Property value maximum length is 91 characters currently, so there is a limit to the
    // number of log enables/disables that can be specified using this method.
    // But it's enough for now.
    android::String8 logEnableStr = GetLongProp("hwcval.log.enable");
    android::String8 logDisableStr = GetLongProp("hwcval.log.disable");
    android::String8 logWarningStr = GetLongProp("hwcval.log.setwarning");
    ...
    for (uint32_t i=0; i<eHwcTestNumChecks; ++i)
    {
        ...
        if (strstr(logEnableStr.string(), name) != 0)
        {
            HwcValLog(ANDROID_LOG_VERBOSE, "HwcTestConfig::HwcTestConfig() enabling log string %s", formalNames[i]);
            mCheckConfigs[i].enable = true;
        }

        if (strstr(logDisableStr.string(), name) != 0)
        {
            HwcValLog(ANDROID_LOG_VERBOSE, "HwcTestConfig::HwcTestConfig() disabling log string %s", formalNames[i]);
            mCheckConfigs[i].forceDisable = true;
        }

        if (strstr(logWarningStr.string(), name) != 0)
        {
            if (mCheckConfigs[i].priority > ANDROID_LOG_WARN)
            {
                HwcValLog(ANDROID_LOG_VERBOSE, "HwcTestConfig::HwcTestConfig() Set warning log string %s", formalNames[i]);
...
void HwcTestConfig::Initialise(bool valHwc, bool valDisplays, bool valBufferAllocation, bool valIvp, bool valSf,
                               bool valHwcComposition, bool valIvpComposition)
{
    ...
    // At component level, we just configure which checks can cause test failure, not actually which are enabled
    ...
    // Opt category is not enabled by any of the above. We do that now for any checks we require.
...
bool HwcTestConfig::WriteToParcel(android::Parcel& parcel) const
{
    parcel.writeInt32(mMinLogPriority);
    parcel.writeInt32(mGlobalEnable);
    parcel.writeInt32(mBufferMonitorEnable);
    parcel.writeInt32(mDisplayCRCCheckDelay);

    for (uint32_t i=0; i<eHwcTestNumChecks; ++i)
    {
        parcel.writeInt32((int32_t)mCheckConfigs[i].value);
    }

    for (uint32_t i=0; i<eComponentMax; ++i)
    {
        parcel.writeInt32((int32_t)mComponentEnabled[i]);
    }

    return true;
};

bool HwcTestConfig::ReadFromParcel(const android::Parcel& parcel)
{
    mMinLogPriority = parcel.readInt32();
    mGlobalEnable = parcel.readInt32();

    // Buffer monitor enable can't be re-enabled once disabled
    bool bufferMonitorEnable = parcel.readInt32();

    if (mBufferMonitorEnable)
    {
        mBufferMonitorEnable = bufferMonitorEnable;
    }

    mDisplayCRCCheckDelay = parcel.readInt32();

    for (uint32_t i=0; i<eHwcTestNumChecks; ++i)
    {
        parcel.readInt32((int32_t*)&(mCheckConfigs[i].value));
    }

    for (uint32_t i=0; i<eComponentMax; ++i)
    {
        mComponentEnabled[i] = (bool) parcel.readInt32();
    }
...
void HwcTestConfig::SetCheck(HwcTestCheckType check, bool enable, bool causesTestFail)
{
...
void HwcTestConfig::SetComponentEnabled(uint32_t component, bool enable, bool causesTestFail)
{
...
bool HwcTestResult::WriteToParcel(android::Parcel& parcel) const
{
    for (uint32_t i=0; i<eHwcTestNumChecks; ++i)
    {
        parcel.writeInt32((int32_t)mCheckFailCount[i]);
        parcel.writeInt32((int32_t)mCheckEvalCount[i]);
    }

    for (uint32_t i=0; i<HWCVAL_MAX_CRTCS; ++i)
    {
        parcel.writeInt32((int32_t) mPerDisplay[i].mMaxConsecutiveDroppedFrameCount);
        parcel.writeInt32((int32_t) mPerDisplay[i].mDroppedFrameCount);
        parcel.writeInt32((int32_t) mPerDisplay[i].mFrameCount);
    }

    parcel.writeInt32((int32_t) mHwcCompValSkipped);
    parcel.writeInt32((int32_t) mHwcCompValCount);
    parcel.writeInt32((int32_t) mSfCompValSkipped);
    parcel.writeInt32((int32_t) mSfCompValCount);

    parcel.writeInt64(mStartTime);
    parcel.writeInt64(mEndTime);

    return true;
}

bool HwcTestResult::ReadFromParcel(const android::Parcel& parcel)
{
    for (uint32_t i=0; i<eHwcTestNumChecks; ++i)
    {
        parcel.readInt32((int32_t*)&mCheckFailCount[i]);
        parcel.readInt32((int32_t*)&mCheckEvalCount[i]);
    }

    for (uint32_t i=0; i<HWCVAL_MAX_CRTCS; ++i)
    {
        parcel.readInt32((int32_t*) &mPerDisplay[i].mMaxConsecutiveDroppedFrameCount);
        parcel.readInt32((int32_t*) &mPerDisplay[i].mDroppedFrameCount);
        parcel.readInt32((int32_t*) &mPerDisplay[i].mFrameCount);
    }

    parcel.readInt32((int32_t*) &mHwcCompValSkipped);
    parcel.readInt32((int32_t*) &mHwcCompValCount);
    parcel.readInt32((int32_t*) &mSfCompValSkipped);
    parcel.readInt32((int32_t*) &mSfCompValCount);

    mStartTime = parcel.readInt64();
    mEndTime = parcel.readInt64();
...
HwcTestResult& HwcTestResult::operator+=(HwcTestResult& rhs)
{
    ...
    for (uint32_t i=0; i<HWCVAL_MAX_CRTCS; ++i)
    {
        mPerDisplay[i].mMaxConsecutiveDroppedFrameCount += rhs.mPerDisplay[i].mMaxConsecutiveDroppedFrameCount;
...
void HwcTestResult::ConditionalDropPriority(HwcTestCheckType check, uint32_t maxNormCount, int reducedPriority)
{
...
void HwcTestResult::ConditionalRevertPriority(HwcTestConfig& config, HwcTestCheckType check, uint32_t maxNormCount)
{
    if (mCheckFailCount[check] > maxNormCount)
    {
...
bool HwcTestResult::IsGlobalFail()
{
    for (uint32_t i=0; i<eHwcTestNumChecks; ++i)
    {
        if ((mCheckFailCount[i] > 0) && (mFinalPriority[i] >= ANDROID_LOG_ERROR) && mCausesTestFail[i])
        {
            return true;
        }
    }
...
void HwcTestResult::Reset(HwcTestConfig* config)
{
    for (uint32_t i=0; i<eHwcTestNumChecks; ++i)
    {
        if ((config == 0) || (config->mCheckConfigs[i].category != eCatStickyTest))
        {
            mCheckEvalCount[i] = 0;
            mCheckFailCount[i] = 0;
        }
...
void HwcTestResult::Log(HwcTestConfig& config, const char* testName, bool brief)
{
    ...
    for (uint32_t component=eComponentMin; component<eComponentMax; ++component)
    {
        ...
        {
            if ((HwcTestConfig::GetComponent(check) == component) && config.IsEnabled((HwcTestCheckType) check))
            {
                if (mCheckFailCount[check] > 0)
                {
                    componentTitleNeeded = true;
                    if (((mFinalPriority[check] >= ANDROID_LOG_ERROR) ||
                        (config.mCheckConfigs[check].category == eCatPriWarn)) // or this is a priority warning
        ...
        for (uint32_t priority = ANDROID_LOG_FATAL; priority >= ANDROID_LOG_INFO; --priority)
        {
            ...
            for (uint32_t check=0; check<eHwcTestNumChecks; ++check)
            {
                    ...
                    && (mFinalPriority[check] == priority))
                {
                    ...
                    if (mCheckFailCount[check] > 0)
                    {
                        ...
                        {
                            printf("%s    %s: %d/%d %s\n", checkPrefix, HwcTestConfig::GetDescription(check),
                                mCheckFailCount[check],
    ...
    for (uint32_t i=0; i<HWCVAL_MAX_CRTCS; ++i)
    {
        ...
        printf("D%d: %sFrames: %d in %3.1fs (%2.1ffps)\n", i, prefix, perDisp->mFrameCount, time > 0 ? time : 0.0, isnan(fps) ? 0.0 : fps);

        if (!brief && perDisp->mDroppedFrameCount > 0)
        {
            printf("D%d: %d dropped frames (max %d consecutive)\n", i,
                perDisp->mDroppedFrameCount, perDisp->mMaxConsecutiveDroppedFrameCount);
        }
