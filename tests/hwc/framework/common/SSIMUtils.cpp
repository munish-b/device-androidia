...
inline static laba convert_pixel(dssim_rgba px, int i, int j, uint32_t alphaMask)
{
    px.a |= alphaMask;
...
// [from http://www.gamedev.net/topic/307417-how-to-write-gaussian-blur-filter-in-c-tool/]
...
void gaussianBlur(float *src, float *tmp, float *dst, const int width, const int height)
{
...
static void transposing_1d_blur(float *src, float *dst, const int width, const int height)
{
...
static void regular_1d_blur(float *src, float *dst, const int width, const int height,
...
static void blur (float *src, float *tmp, float *dst, const int width, const int height,
                  rowcallback *const callback, const int blur_type)
{
...
void DoSSIMCalculations(dssim_info *inf,
                        ...
                        const int width, const int height, const int blur_type, bool hasAlpha)
{
    float * img2[CHANS];
    for(int ch=0; ch < CHANS; ch++)
    {
        ...
        inf->chan[ch].img1 = new float[inf->chan[ch].width * inf->chan[ch].height]();
        img2[ch] = new float[inf->chan[ch].width * inf->chan[ch].height]();
    }

    //int64_t startTime = ns2ms(systemTime(SYSTEM_TIME_MONOTONIC));
    convert_image(Bufrow_pointers, inf, inf->chan[0].img1, inf->chan[1].img1, inf->chan[2].img1, hasAlpha);
    ...
    for (int ch = 0; ch < CHANS; ch++)
    {
        ...
        blur(sigma1_tmp, tmp, inf->chan[ch].sigma1_sq, width, height, NULL, blur_type);
        ...
        blur(img2[ch], tmp, inf->chan[ch].sigma2_sq, width, height, square_row, blur_type);
