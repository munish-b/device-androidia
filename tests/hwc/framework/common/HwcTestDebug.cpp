...
uint32_t driverBufferOffset =
    MEMBER_OFFSET(Hack::intel::ufo::gralloc::Buffer::Allocation, mBo);
uint32_t boOffset =
...
int lockCCS(gralloc_module_t* pGralloc, buffer_handle_t handle, int offset,
  ...
  Hack::intel::ufo::gralloc::Module* pModule =
      reinterpret_cast<Hack::intel::ufo::gralloc::Module*>(pGralloc);
  ...
  if (index >= 0) {
    ...
    HWCERROR(eCheckInternalError,
             "lockCCS: gralloc does not know buffer handle %p", handle);
    *vaddr = 0;
    return -1;
  }

  uint8_t* hackedBuffer = reinterpret_cast<uint8_t*>(pBuffer);
  drm_intel_bo** ptr = reinterpret_cast<drm_intel_bo**>(
      hackedBuffer + bufOffset + driverBufferOffset + boOffset);
  ALOG_ASSERT(ptr);
  HWCLOGD("bo**=%p", ptr);
  drm_intel_bo* bo = *ptr;
  HWCLOGD("bo*=%p", bo);
  ...
  HWCLOGD(
      "lockCCS: bo %p size %d align 0x%x offset %d virt %p bufmgr %p handle %d",
  ...
  HwcTestState::FPdrm_intel_bo_map fpdrm_intel_bo_map =
      HwcTestState::getInstance()->GetDrmIntelBoMap();
  ...
  if (err) {
    ALOGE("%s() failed to map bo for handle %p, %d/%s", __FUNCTION__, handle,
          err, strerror(-err));
    return err;
  }
  HWCLOGD_COND(eLogDebugDebug, "bo for handle %p mapped to %p", handle,
               bo->virt);
...
int unlockCCS(gralloc_module_t* pGralloc, buffer_handle_t handle) {
  ...
  if (index >= 0) {
    ...
    HWCERROR(eCheckInternalError,
             "unlockCCS: gralloc does not know buffer handle %p", handle);
    return -1;
  }
  ...
  HWCLOGD(
      "unlockCCS: bo %p size %d align 0x%x offset %d virt %p bufmgr %p handle "
      "%d",
      bo, bo->size, bo->align, bo->offset, bo->virt, bo->bufmgr, bo->handle);
  ...
  HwcTestState::FPdrm_intel_bo_unmap fpdrm_intel_bo_unmap =
      HwcTestState::getInstance()->GetDrmIntelBoUnmap();
  ...
  if (err) {
    ALOGE("%s() failed to unmap bo for handle %p, %d/%s", __FUNCTION__, handle,
          err, strerror(-err));
    return err;
  }
...
#endif  // INTEL_UFO_GRALLOC_HAVE_BUFFER_DETAILS_1 &&
        // UFO_GRALLOC_ENABLE_AUX_ALLOCATIONS
...
#define MAKE_ARGB_FROM_YCrCb(Y, Cb, Cr)                                      \
  ((0xFF000000) |                                                            \
   ...
   (CLAMP((298 * ((Y)-16) - 100 * ((Cb)-128) - 208 * ((Cr)-128) + 128) >> 8) \
    << 8) |                                                                  \
...
bool HwcTestDumpAuxBufferToDisk(const char* pchFilename, uint32_t num,
  ...
  if ((stride == 0) || (bufferInfo.rc.aux_offset >= size)) {
    HWCLOGD(
        "HwcTestDumpAuxBufferToDisk: handle %p has no aux buffer (stride %d "
        "offset %d size %d used size %d)",
        grallocHandle, stride, bufferInfo.rc.aux_offset, size,
        bufferInfo.pitch * height);
  ...
  lockCCS(pGralloc, grallocHandle, bufferInfo.rc.aux_offset, (void**)&pAuxData);
  ...
  *((uint32_t*)&strFormat) = bufferInfo.drmformat;
  ...
  const uint32_t colourMap[] = {
      MAKE_ARGB(0xff, 0, 0, 0), MAKE_ARGB(0xff, 0xff, 0, 0),
      MAKE_ARGB(0xff, 0, 0xff, 0), MAKE_ARGB(0xff, 0, 0, 0xff)};

  android::String8 filename = android::String8::format(
      DEBUG_DUMP_DISK_ROOT "%s%05d.tga", pchFilename, num);
...
bool HwcTestDumpMemBufferToDisk(const char* pchFilename, uint32_t num,
  ...
  if (outputDumpMask & DUMP_BUFFER_TO_RAW) {
    ...
    // Generate filename
    android::String8 filename = android::String8::format(
        DEBUG_DUMP_DISK_ROOT "%s%05d.raw", pchFilename, num);
  ...
  if (outputDumpMask & DUMP_BUFFER_TO_TGA) {
    ...
    android::String8 filename = android::String8::format(
        DEBUG_DUMP_DISK_ROOT "%s%05d.tga", pchFilename, num);
    HWCLOGD_COND(eLogDebugDebug, "Dumping %p to %s [fmt:%u/%s]\n", handle,
                 filename.string(), bufferInfo.format,
                 FormatToStr(bufferInfo.format));
    ...
    if (fp) {
      ...
      if (ftell(fp) <= (long)szTGAHeader) {
        HWCLOGE("Failed to dump DRM format 0x%x %s to %s", bufferInfo.format,
                FormatToStr(bufferInfo.format), filename.string());
        bRet = false;
      }
