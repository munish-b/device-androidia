...
class DrmShimTransform {
 ...
 public:
  DrmShimTransform();
  DrmShimTransform(android::sp<DrmShimBuffer>& buf, double width = 0.0,
                   double height = 0.0);
  ...
  void SetBlend(Hwcval::BlendingType blend, bool hasPixelAlpha,
                float planeAlpha);
  uint32_t GetBlend();
...
class DrmShimCroppedLayerTransform : public DrmShimTransform {
 public:
  DrmShimCroppedLayerTransform(android::sp<DrmShimBuffer>& buf,
...
inline void DrmShimTransform::SetBlend(Hwcval::BlendingType blend,
