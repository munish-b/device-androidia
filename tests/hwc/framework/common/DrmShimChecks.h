...
File Name:      DrmShimChecks.h

Description:    Class definition for DRMShimChecks class.
...
class EXPORT_API DrmShimChecks
  : public HwcTestKernel
{
protected:
    int mShimDrmFd;

    // All CRTCs, by CRTC id
    android::DefaultKeyedVector<uint32_t, DrmShimCrtc* > mCrtcs;
    ...
    android::KeyedVector<uint32_t, Connector> mConnectors;

    // gralloc buffer object tracking by fb ID
    android::DefaultKeyedVector<uint32_t, android::sp<DrmShimBuffer> > mBuffersByFbId;

    // List of all hot-pluggable connectors
    android::SortedVector<uint32_t> mHotPluggableConnectors;

    // Per-encoder data
    android::DefaultKeyedVector<uint32_t, uint32_t> mConnectorForEncoder;
    android::DefaultKeyedVector<uint32_t, uint32_t> mPossibleCrtcsForEncoder;

    // CRTCs indexed by pipe index - NOT the same as display index
    DrmShimCrtc* mCrtcByPipe[HWCVAL_MAX_PIPES];

    // Frame number currently processing in HWC's DRM thread (according to log entries)
    ...
    // Setdisplay cache for nuclear to SetDisplay conversion
    drm_mode_set_display mSetDisplay[HWCVAL_MAX_CRTCS];
...
public:
    ...
    EXPORT_API void checkPageFlipEnter(int fd, uint32_t crtc_id,
                                       uint32_t fb_id, uint32_t flags,
                                       void *&user_data);

    EXPORT_API void checkPageFlipExit (int fd, uint32_t crtc_id,
                                       uint32_t fb_id, uint32_t flags,
                                       void *user_data,
                                       int ret);
    ...
    EXPORT_API void checkSetPlaneExit ( int fd, uint32_t plane_id,
                                        uint32_t crtc_id,
                                        uint32_t fb_id, uint32_t flags,
                                        uint32_t crtc_x, uint32_t crtc_y,
                                        uint32_t crtc_w, uint32_t crtc_h,
    ...
    EXPORT_API int CheckSetDisplayEnter ( drm_mode_set_display* drmDisp, DrmShimCrtc*& crtc);
    ...
    EXPORT_API void CheckGetConnectorExit(int fd, uint32_t connId, drmModeConnectorPtr& pConn);

    EXPORT_API void CheckGetEncoder(uint32_t encoder_id, drmModeEncoderPtr pEncoder);

    EXPORT_API void CheckSetCrtcEnter ( int fd, uint32_t crtcId,
                                        uint32_t bufferId,
    ...
    EXPORT_API void CheckIoctlI915SetPlaneZOrder(struct drm_i915_set_plane_zorder* ord);

    EXPORT_API void CheckIoctlI915SetPlane180Rotation(struct drm_i915_plane_180_rotation* rot);

#ifdef BUILD_I915_DISP_SCREEN_CONTROL
    EXPORT_API void CheckIoctlI915DispScreenControl(struct drm_i915_disp_screen_control* ctrl, int status);
#endif

    EXPORT_API void CheckIoctlI915SetDecrypt(struct drm_i915_reserved_reg_bit_2* decrypt);

    EXPORT_API void CheckSetDPMS(uint32_t conn_id, uint64_t value, HwcTestEventHandler* eventHandler, HwcTestCrtc*& crtc, bool& reenable);
    EXPORT_API void CheckSetDPMSExit(uint32_t fd, HwcTestCrtc* crtc, bool reenable, HwcTestEventHandler* eventHandler, uint32_t status);
    EXPORT_API void CheckSetPanelFitter(uint32_t conn_id, uint64_t value);
    EXPORT_API void CheckSetPanelFitterSourceSize(uint32_t conn_id, uint32_t sw, uint32_t sh);
    ...
    virtual uint32_t GetDisplayProperty(uint32_t displayIx, HwcTestState::DisplayPropertyType prop);
    ...
    // Set hardware blending into transform
    void SetPlaneBlend(DrmShimPlane* plane, drm_mode_set_display_plane* drmPlane);

    // Convert DRM (nuclear on Broxton) transform values to HAL transform values.
    uint32_t DrmTransformToHalTransform(HwcTestState::DeviceType deviceType, uint32_t drmTransform);

    /// Broxton plane scaler validation
    static uint32_t BroxtonPlaneValidation(HwcTestCrtc* crtc, android::sp<DrmShimBuffer> buf, const char* str, uint32_t id,
        double srcW, double srcH,
        uint32_t dstW, uint32_t dstH,
        uint32_t transform);

    // Simulate hotplug on any suitable display
    virtual EXPORT_API bool SimulateHotPlug(uint32_t displayTypes, bool connected);
...
protected:
    ...
    android::sp<DrmShimBuffer> UpdateBufferPlane(uint32_t fbId, DrmShimCrtc* crtc, DrmShimPlane* plane);
    ...
    virtual android::sp<HwcTestBufferObject> GetBufferObject(uint32_t boHandle);

    /// Move device-specific ids from old to new buffer
    virtual void MoveDsIds(android::sp<DrmShimBuffer> existingBuf, android::sp<DrmShimBuffer> buf);

    // Change the number and order of modes in a mode list (from drmModeGetConnector)
...
private:
    ...
    // (for nuclear spoofing when we don't have universal plane support, this is more complex
    // than it appears)
