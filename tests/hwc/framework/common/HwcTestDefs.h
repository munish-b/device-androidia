...
File Name:      HwcTestDefs.h

Description:    Common types, enum and structs fro hwc and drm shims.
...
// The default of the default! This is overriden at run time by the environment variable HWCVAL_LOG_DEFAULT_PRIORITY
#define HWCVAL_DEFAULT_LOG_PRIORITY ANDROID_LOG_WARN

// Window on cropf and display frame size matching
// Needs to be at least 2 to allow for adjustment to even values on 422/NV12 layers
...
// At what interval in number of frames do we perform DrmShimBuffer garbage collection?
#define HWCVAL_BUFFER_GARBAGECOLLECT_FREQUENCY 100

// What is the minimum length of time after last seeing a DrmShimBuffer that we should retain reference to it?
#define HWCVAL_BUFFER_LIFETIME 100

// Number of current buffers that can be tracked before we generate an error message
#define CURRENT_BUFFER_COUNT_WARNING_LEVEL 1000

// Number of current and historical buffers that can be tracked before we generate an error mesage
...
// Multiplication factor to convert floating point [0,1] alpha values to integer [0,255].
...
// Composition validation ensures caller does not destroy input buffers using fences
...
// This is the CRTC id (and array / display index) for the virtual / Widi display (if present)
...
// Uncomment to enable resource leak checking including memory leak checking with stack trace
...
// Number of frames after a hot plug before HWC should respond by issuing a teardown
// message in the log
//
// This can take a surprisingly long time, because HWC is busy carrying out the
// drmModeGetConnector stuff - and it has to do that before it can decide if a display
...
/*
 * The following section has been copied from the HWC Drm.c/ivpg_drm.c file. In it,
