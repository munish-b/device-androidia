...
// circumstances.
// This is not a requirement for nuclear DRM. Hence in our translation layer we
// must allocate one ourselves.
//
// Here we allocate it and give it an FbId, just as HWC does when it uses
// SetDisplay.
uint32_t DrmShimCrtc::GetBlankingFb(
  ...
  if (mBlankingFb == 0) {
    ...
    mBlankingBuffer = new android::GraphicBuffer(
        mWidth, mHeight, HAL_PIXEL_FORMAT_RGBA_8888,
        GRALLOC_USAGE_HW_COMPOSER | GRALLOC_USAGE_HW_RENDER |
            GRALLOC_USAGE_HW_FB);

    ALOG_ASSERT(mBlankingBuffer.get());
    HWCLOGD("DrmShimCrtc::GetBlankingFb: Allocated blanking buffer %p",
            mBlankingBuffer->handle);
    ...
    pitches[2] = stride;
    pitches[3] = stride;
    int result = addFb2Func(fd, mWidth, mHeight, DRM_FORMAT_ABGR8888, handles,
                            pitches, offsets, &mBlankingFb, 0);
    if (result != 0) {
      HWCERROR(eCheckDrmShimFail,
               "Failed to allocate FB ID to blanking buffer %dx%d boHandle "
               "0x%x stride %d",
               mWidth, mHeight, boHandle, stride);
    } else {
      HWCLOGD("DrmShimCrtc::GetBlankingFb: Blanking buffer has FB %d",
              mBlankingFb);
    }
  }
