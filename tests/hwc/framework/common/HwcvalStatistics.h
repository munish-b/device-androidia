...
namespace Hwcval
{
    ...
    public:
        ...
        // Numeric statistic, gathering min, max and count and reporting all these plus mean.
        template<class T>
        ...
        private:
            ...
            // So bucket 0 counts instances of values in the range mCollectionMin <= value < mCollectionMin * 2
            // bucket 1 counts instances of values in the range mCollectionMin * 2 <= value < mCollectionMin * 4
            // etc.
            T mCollectionMin;
    ...
    void Statistics::Aggregate<T>::Dump(FILE* file, const char* prefix)
    {
        ...
        fprintf(file, fmt.string(), prefix, GetName(), double(mSum) / mCounter.GetValue());
    }

    // Template implementation: Statistic::Aggregate<T>
    template<class T>
    Statistics::CumFreqLog<T>::CumFreqLog(const char* name, T collectionMin, const char* fmt)
      : Aggregate<T>(name, fmt),
        mCollectionMin(collectionMin)
    {
    }
