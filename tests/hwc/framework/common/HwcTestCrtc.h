...
File Name:      HwcTestCrtc.h

Description:    Class definition for an HwcTestCrtc class. This models the
                notion of a 'pipe' (in GEN hardware parlance).
...
class HwcTestCrtc : public android::RefBase
{
public:

    typedef android::Vector<uint32_t> SeqVector;
    ...
    HwcTestCrtc(uint32_t crtcId, uint32_t width, uint32_t height, uint32_t clock, uint32_t vrefresh);
    ...
    void SetDimensions(uint32_t width, uint32_t height, uint32_t clock, uint32_t vrefresh);
    void ResetOutDimensions();
    void SetOutDimensions(uint32_t width, uint32_t height);

    // These functions are telling the destination display which SF display the layers come from
    // and how they will be cropped.
    void SetMosaicTransform(uint32_t srcDisp,
        double srcLeft, double srcTop, double width, double height,
        double dstLeft, double dstTop);

    // This function additionally supports scaling which is not currently a HWC feature.
    void SetMosaicTransform(uint32_t srcDisp,
    ...
    void SetVideoLayerIndex(int layerIndex, const hwc_rect_t& rect = {0, 0, 0, 0});
    int GetVideoLayerIndex();

    // Current power state
    PowerState GetPower();

    // Record main plane disable - this is effectively an event, it only persists for the
    ...
    void GetDroppedFrameCounts(uint32_t& droppedFrameCount, uint32_t& maxConsecutiveDroppedFrameCount, bool clear);
    void SetDroppedFrame();
    bool IsDroppedFrame();

    // Call when some error has been detected on this display.
    // Returns true if the caller should continue to report the error, false if it has been trapped at a higher level.
    ...
    // Add to dropped frames having first considered if the display was turned off.
    ...
    // Clear the list of transforms showing what's been drawn, and associated stuff.
    void ClearDrawnList();
    ...
    // So returns true if SF/the harness will see this as a connected display when it is physically connected
    bool IsBehavingAsConnected();

    // Combined connected flag
    // Returns true if and only if the display is actually connected, and logically connected from a hot plug
    // spoof point of view.
    bool IsConnected();
    ...
    void SetUserModeFinish(android::status_t st, uint32_t width, uint32_t height, uint32_t refresh, uint32_t flags, uint32_t ratio);
    ...
    // Checks
    void Checks(Hwcval::LayerList* ll, HwcTestKernel* testKernel, uint32_t hwcFrame);
    void FlickerClassify(DrmShimPlane* plane, android::sp<DrmShimBuffer>& buf);
    void DetectCloneOptimization(DrmShimPlane* plane, android::sp<DrmShimBuffer>& buf);
    void FlickerChecks();
...
inline DrmShimPlane* HwcTestCrtc::GetPlane(uint32_t planeIx)
{
    ...
    {
        return mPlanes.valueAt(planeIx);
    }
    else
    {
        return 0;
...
inline void HwcTestCrtc::SetVideoLayerIndex(int layerIndex, const hwc_rect_t& rect)
{
...
inline bool HwcTestCrtc::EsdStateTransition(EsdRecoveryStateType from, EsdRecoveryStateType to)
{
...
{
    HWCLOGD_COND(eLogDrm, "HwcTestCrtc::SetDisplayType Crtc=%d %s",
        mCrtcId, HwcTestState::DisplayTypeStr(displayType));
...
inline void HwcTestCrtc::SetRealDisplayType(HwcTestState::DisplayType displayType)
{
    HWCLOGD_COND(eLogDrm, "HwcTestCrtc::SetRealDisplayType Crtc=%d %s",
        mCrtcId, HwcTestState::DisplayTypeStr(displayType));
