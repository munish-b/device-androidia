...
BxService::BxService() : mRealBinder(0), mRealService(0) {
}
...
void BxService::GetRealService() {
  // A service may be requested (particularly diagnostic) before the real HWC
  // service has started.
  ...
  while ((mRealService.get() == 0) && (++tries < 50)) {
    ...
    if ((tries % 10) == 1) {
      HWCLOGI("BxService: retrying after attempt %d to get service %s.", tries,
              IA_HWCREAL_SERVICE_NAME);
    }
  ...
  switch (code) {
    ...
    default: {
      if ((code >> 24) == '_') {
        ...
        // This method should maximise binary compatibility and avoid any
        // problems
