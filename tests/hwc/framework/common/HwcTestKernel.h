...
protected:
    ...
    android::DefaultKeyedVector<buffer_handle_t, android::sp<DrmShimBuffer> > mBuffers;

    // buffer tracking by buffer "name" (this is actually the global handle of the buffer)
    android::DefaultKeyedVector<int, android::sp<DrmShimBuffer> > mBuffersByGlobalId;

    // buffer tracking by buffer object handle (NOT same as gralloc handle)
    android::DefaultKeyedVector<BoKey, android::sp<HwcTestBufferObject> > mBosByBoHandle;
    ...
    //   mNumSetResolutionsReceived = the number of received "Set Resolution" messages
    ...
    // HWC does strange things with snapshots so we need to know this is going on in the validation
...
public:
    ...
    EXPORT_API android::sp<DrmShimBuffer> lookupDrmShimBuffer(buffer_handle_t handle);
    ...
    EXPORT_API void setExtVideoModeDisable(bool disableExtVideoMode) { mExtVideoModeDisabled = disableExtVideoMode; }

    /// Do we meet the conditions for widi video mode validation?
    EXPORT_API bool IsWidiVideoMode(Hwcval::LayerList::VideoFlags vf, uint32_t hwcFrame);
    ...
    void NotifyPageFlipHandlerExit(HwcTestCrtc* crtc, int firstUnsignalledRetireFence);

    // Throw away the next frame we were going to validate
    void SkipFrameValidation(HwcTestCrtc* crtc);

    // Read anything outstanding in the LLQ to work out how many frames were dropped at the end of the test
    void FinaliseTest();
    ...
    void CheckIoctlGemWait(int fd, struct drm_i915_gem_wait* gemWait, int status, int64_t delayNs);
    ...
    virtual EXPORT_API bool SimulateHotPlug(uint32_t displayTypes, bool connected) = 0;
    ...
    // Processing function for Prime call (associate bo handle with DMA buffer handle)
    ...
    HwcTestCrtc* GetHwcTestCrtcByDisplayIx(uint32_t displayIx, bool persistentCopy = false);
    ...
    void SetExtendedModeExpectation(bool singleFullScreenVideo, bool haveSecondDisplay, uint32_t hwcFrame);

    // Display property query
    virtual uint32_t GetDisplayProperty(uint32_t displayIx, HwcTestState::DisplayPropertyType prop) = 0;
    ...
    void DetermineFullScreenVideo(uint32_t displayIx, uint32_t i, Hwcval::ValLayer& vl, const char* notes);
    ...
    // (We already know this is the DmConfig option value so no need to return true/false)
    ...
    // Harness can call this regularly to avoid the work queue getting too big when
    ...
    android::sp<DrmShimBuffer> RecordBufferState(buffer_handle_t handle, Hwcval::BufferSourceType bufferSource, char* notes);

    /// Validate layer display frame is inside Framebuffer target
    void ValidateHwcDisplayFrame(const hwc_rect_t& layerDf, const hwc_rect_t& fbtDf, uint32_t displayIx, uint32_t layerIx);
...
protected:
    ...
    virtual HwcTestBufferObject* CreateBufferObject(int fd, uint32_t boHandle) = 0;

    /// Find or create and index a tracking object for a bo
    virtual android::sp<HwcTestBufferObject> GetBufferObject(uint32_t boHandle) = 0;

    /// Move device-specific ids from old to new buffer
    virtual void MoveDsIds(android::sp<DrmShimBuffer> existingBuf, android::sp<DrmShimBuffer> buf) = 0;

    // Internal validation
    void ValidateBo(android::sp<DrmShimBuffer> buf, const char* str="");
    void ValidateBo(android::sp<HwcTestBufferObject> bo, android::sp<DrmShimBuffer> buf, const char* str="");

    /// Make an educated guess as to whether this is an empty buffer used by HWC to replace prohibited content
...
inline void HwcTestKernel::ValidateBo(android::sp<DrmShimBuffer> buf, const char* str)
{
    HWCVAL_UNUSED(buf);
    HWCVAL_UNUSED(str);
}

inline void HwcTestKernel::ValidateBo(android::sp<HwcTestBufferObject> bo, android::sp<DrmShimBuffer> buf, const char* str)
{
    HWCVAL_UNUSED(bo);
    HWCVAL_UNUSED(buf);
    HWCVAL_UNUSED(str);
...
inline void HwcTestKernel::SetHdmiPreferredMode(uint32_t width, uint32_t height, uint32_t refresh)
{
...
// Widi disabled state excludes the transitional one after Widi disconnect commences
...
// Nonmember operators
inline bool operator<(const HwcTestKernel::BoKey& lhs, const HwcTestKernel::BoKey& rhs)
{
    union BoKeyCmp
    {
