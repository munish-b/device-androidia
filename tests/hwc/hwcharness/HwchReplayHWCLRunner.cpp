...
bool Hwch::ReplayHWCLRunner::AddLayers(Hwch::Frame& frame,
    uint32_t display,
    ...
    int32_t msecs)
{
    ...
    {
        Hwch::eRed, Hwch::eGreen, Hwch::eBlue, Hwch::eYellow, Hwch::eCyan,
        Hwch::ePurple, Hwch::eGrey, Hwch::eLightRed, Hwch::eLightGreen,
        Hwch::eLightCyan, Hwch::eLightPurple, Hwch::eLightGrey,
    ...
    while (std::getline(mFile, line))
    {
        ...
        if (mParser->ParseEncrypted(line, session, instance))
        {
            ...
            {
                HWCLOGW("Saw protected layer in replay, but protected content is disabled!");
            }
        ...
        else if (mParser->IsHwclLayerUnsupported(line))
        {
            ...
            HWCLOGW_IF(REPLAY_HWCL_DEBUG, "Skipping layer with unsupported "
                "colour space: %s", line.c_str());
            continue;
        }
        ...
        {
            for (size_t i = 0; i < prev_layer_cache.size(); ++i)
            {
                // Get a pointer to the layer
                Hwch::ReplayLayer& layer = *prev_layer_cache.editValueAt(i);
                ...
                else if (!layer.IsAClone())
                {
                    ...
                    android::sp<Hwch::ReplayLayer> test_layer = new Hwch::ReplayLayer("Replay", 0, 0);
                ...
                if (match)
                {
                    ...
                    HWCLOGI_IF(REPLAY_HWCL_DEBUG, "Frame (%5d) : %ds %3dms : "
                        ...
                        handle, display, layer_cache.size(),
                        prev_layer_cache.size(), key);
                }
            }
        }

        // See if the layer is in the cache i.e. was it present in the last frame
        if (layer_cache.indexOfKey(key) < 0)
        {
            ...
            if (display > 0)
            {
                ...
                if ((layer_cache.indexOfKey(test_key) >= 0) &&
                    (layer_cache.valueFor(test_key)->IsKnownBuffer(handle)))
                {
                    // Found a layer on display 0 - clone it
                    new_or_cloned_layer = layer_cache.valueFor(test_key)->Dup();
                    mParser->ParseHwclLayer(line, *new_or_cloned_layer);
                    layer_cache.add(key, new_or_cloned_layer);

                    // Add some debug information to the HWC log
                    HWCLOGI_IF(REPLAY_HWCL_DEBUG, "Frame (%5d) : %ds %3dms : %s : "
                        ...
                        key, layer_cache.indexOfKey(key));
                }
            }

            // See if we created a clone
            if ((display == 0) || (layer_cache.indexOfKey(key) < 0))
            {
                // No clone - so allocate a new layer. Note that layer takes
                // ownership of the pattern through a strong pointer.
                HWCLOGI_IF(REPLAY_HWCL_DEBUG, "Frame (%5d) : %ds %3dms : %s : Allocating "
                ...
                if (is_skip_layer)
                {
                    new_or_cloned_layer->SetSkip(true, !mParser->IsHwclLayerUnsupported(line));
                }
                ...
                // to white. Otherwise, iterate through the colour palette (but do not use
                // black and white).
                if ((layer_index == 0) && new_or_cloned_layer->IsFullScreen(display))
                {
                    new_or_cloned_layer->SetPattern(new Hwch::ReplayPattern(Hwch::eWhite));
                }
                else
                {
                    new_or_cloned_layer->SetPattern(new Hwch::ReplayPattern(
                        Hwch::Alpha(colour_lut[colour], mAlpha)));
            ...
            Hwch::System& system = Hwch::System::getInstance();
            frame.Add(*new_or_cloned_layer, system.IsWirelessDisplayCloningEnabled() ? -1 : display);
        }
        else
        {
            // The layer was present in the last frame - get a pointer to it
            Hwch::ReplayLayer& layer = *layer_cache.editValueFor(key);

            // Add some debug to the log
            HWCLOGI_IF(REPLAY_HWCL_DEBUG, "Frame (%5d) : %ds %3dms : %s : Rotating for "
...
void Hwch::ReplayHWCLRunner::PrintStatistics(void)
{
    ...
    std::printf("HWC log replay complete. Statistics are as follows:\n"
...
int Hwch::ReplayHWCLRunner::RunScenario(void)
{
    ...
    while (std::getline(mFile, line))
    {
        ...
        else if (mParser->ParseProtectedEnable(line, session, instance))
        {
            if (!mReplayNoProtected)
            {
                HWCLOGD("Parsed a protected session enable, but protected " \
                        "content disabled on command line - skipping.");
                continue;
            }

            HWCLOGD("Enabling protected session (%d) and instance (%d)", session, instance);

            // Find a new protected session descriptor. Note the following:
            //
            // Sessions are created by libPAVP and refer to protected audio video paths in the
            // hardware. When these are torn down, it is perfectly feasible (and highly likely)
            // that the next created protected session will re-use the id from the previous.
            //
            // Instances are created by CoreU and are never re-used. The instance number is
            // effectively a counter that tells you how many tear downs have occurred.
            //
            // Because of this, we should never see a request to add a new instance on an existing
            // session. This would imply that we have missed a session 'disable' event.
            int32_t index;
            ...
            if (index == HWCVAL_MAX_PROT_SESSIONS)
            {
                ...
                HWCLOGE("No protected sessions available for parsed session %d (instance: %d)! "
                        "Skipping pavp_session_creation.", session, instance);
                continue;
            }
            ...
            android::sp<Hwch::AbstractPavpSession> pavpSession = Hwch::System::getInstance().GetPavpSession();

            // This should probably be done on a thread
            mProtectedSessions[index].active_session = pavpSession->StartPavpSession();
            mProtectedSessions[index].active_instance = pavpSession->GetInstanceId();
            HWCLOGD("Created new protected session %d with (instance: %d) from parsed session %d (instance: %d)",
                mProtectedSessions[index].active_session, mProtectedSessions[index].active_instance,
                mProtectedSessions[index].parsed_session, mProtectedSessions[index].parsed_instance);
        }
        else if (mParser->ParseProtectedDisableSession(line, session))
        {
            if (!mReplayNoProtected)
            {
                HWCLOGD("Parsed a protected session disable, but protected " \
                        "content disabled on command line - skipping.");
                continue;
            }
        ...
        else if (mParser->ParseProtectedDisableAll(line))
        {
            if (!mReplayNoProtected)
            {
                HWCLOGD("Parsed a protected session disable all, but protected " \
                        "content disabled on command line - skipping.");
                continue;
            }
        ...
        // Look for OnSets
        else if (mParser->ParseHwclOnSet(line, secs, msecs, nsecs, frame_id, display, flags))
        {
            // Update the statistics
            mStats.parsed_onset_count++;

            // Cache the display so that we know when we have processed the whole frame
            last_display_processed = display;

            // Check if this is the start of a new frame. If there is no frame_id, then check to see
            // if the frame has a different timestamp. If so, calculate the interframe time spacing and
            // update the previous timestamps.
            if ((mReplayNoTiming == false) && (display == 0) &&
                (((frame_id != -1) && (frame_id != prev_frame_id)) ||
                 ((secs != prev_secs) || (msecs != prev_msecs) || (nsecs != prev_nsecs))))
            {
                if (mStats.parsed_onset_count > 1) // skip the first frame
                {
                    // Calculate the interframe time spacing.
                    uint64_t prev_time =
                        ((prev_secs * mNanosPerSec) + (prev_msecs * mMillisPerSec) + prev_nsecs);
                    uint64_t curr_time = ((secs * mNanosPerSec) + (msecs * mMillisPerSec) + nsecs);
                    uint64_t spacing = curr_time - prev_time;

                    // If the inter-frame spacing is greater than the vsync period for this
                    // display, then adjust for it. If spacing is less than or equal to the
                    ...
                    if (spacing > vsync_period)
                    {
                        ...
                        interframe_spacing.tv_nsec = (spacing - vsync_period) % mNanosPerSec;
                    }

                    // Skip the inter-frame spacing if the delay is greater than the bound.
                    // This can occur (for example) if the user has combined log files i.e.
                    ...
                    HWCLOGD("Calculated interframe spacing as: %ds %dms %dns (%llu) - "
                        "%ds %dms %dns (%llu) - %u = %lld.%.9lds",
                        prev_secs, prev_msecs, prev_nsecs, prev_time,
                        secs, msecs, nsecs, curr_time,
                        vsync_period,
                        (long long)interframe_spacing.tv_sec, interframe_spacing.tv_nsec);
                }
            ...
            send_frame = AddLayers(frame, display, layer_cache, prev_layer_cache, secs, msecs);
        }
        else
        {
            // There are no blanking events, hot plugs or further 'onSet' lines. IF the frame
            // contains valid layers, then send it to the HWC and clean up.
            if (send_frame && frame.NumLayers(0) && (display <= last_display_processed))
            {
    ...
    HWCLOGI_IF(REPLAY_HWCL_DEBUG,
        ...
        ((double)mStats.match_count / (double)mStats.processed_layer_count) * 100.0,
        mStats.allocation_count);
