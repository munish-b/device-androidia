...
*
* File Name:            HwchBufferFormatConfig.cpp
*
...
namespace Hwch
{
BufferFormatConfig::BufferFormatConfig (uint32_t minDfWidth, uint32_t minDfHeight,
                            uint32_t minBufferWidth, uint32_t minBufferHeight,
                            uint32_t bufferWidthAlignment, uint32_t bufferHeightAlignment,
                            float cropAlignment, float minCropWidth, float minCropHeight,
                            uint32_t dfXMask, uint32_t dfYMask)
...
void BufferFormatConfig::AdjustDisplayFrame(Rect& r, uint32_t displayWidth, uint32_t displayHeight) const
{
    HWCLOGV_COND(eLogHarness, "AdjustDisplayFrame entry (%d, %d, %d, %d) %dx%d", r.left, r.top, r.right, r.bottom, displayWidth, displayHeight);

    if (HwcTestState::getInstance()->IsOptionEnabled(eOptDispFrameAlwaysInsideScreen))
    {
        if ((r.right > int(displayWidth)) || (r.bottom > int(displayHeight)))
        {
            HWCLOGD_COND(eLogHarness, "Adjusting to %dx%d", displayWidth, displayHeight);
        }
    ...
    HWCLOGV_COND(eLogHarness, "AdjustDisplayFrame exit (%d, %d, %d, %d)", r.left, r.top, r.right, r.bottom);
    ALOG_ASSERT(r.right > r.left);
    ALOG_ASSERT(r.bottom > r.top);
}

// Adjust buffer size to comply with the min width & height, and alignment (i.e. whether odd values are permitted)
...
void BufferFormatConfig::AdjustCropSize(uint32_t bw, uint32_t bh, float& w, float& h) const
{
...
// Adjust crop rectangle to comply with crop size and alignment restrictions
void BufferFormatConfig::AdjustCrop(uint32_t bw, uint32_t bh, float& l, float& t, float& w, float& h) const
{
    if (mCropAlignment > 0.0)
    {
...
// Adjust display frame to comply with the min width & height
void BufferFormatConfigManager::AdjustDisplayFrame(uint32_t format, Hwch::Rect& r, uint32_t displayWidth, uint32_t displayHeight)
{
    ssize_t ix = indexOfKey(format);

    if (ix >= 0)
    {
        const BufferFormatConfig& cfg = valueAt(ix);
...
// Adjust buffer size to comply with the min width & height, and alignment (i.e. whether odd values are permitted)
void BufferFormatConfigManager::AdjustBufferSize(uint32_t format, uint32_t& w, uint32_t& h)
{
    ssize_t ix = indexOfKey(format);
...
void BufferFormatConfigManager::AdjustCropSize(uint32_t format, uint32_t bw, uint32_t bh, float& w, float& h)
{
    ssize_t ix = indexOfKey(format);
...
void BufferFormatConfigManager::AdjustCrop(uint32_t format, uint32_t bw, uint32_t bh, float& l, float& t, float& w, float& h)
{
    ssize_t ix = indexOfKey(format);
