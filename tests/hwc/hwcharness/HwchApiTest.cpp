...
void Hwch::ApiTest::SetLayerCrop(Hwch::Layer* layer, uint32_t format, uint32_t bufferWidth, uint32_t bufferHeight)
{
    float cropWidth = BufferSizeChoice(mScreenLogWidth, mMinCropWidth, bufferWidth).Get();
    float cropHeight = BufferSizeChoice(mScreenLogHeight, mMinCropHeight, bufferHeight).Get();
    ...
    SetLayerCropInsideBuffer(layer, cropX, cropY, cropWidth, cropHeight, bufferWidth, bufferHeight);
}

void Hwch::ApiTest::SetLayerCropInsideBuffer(Hwch::Layer* layer, float cropX, float cropY,
    float cropWidth, float cropHeight, uint32_t bufferWidth, uint32_t bufferHeight)
{
    ...
    HWCLOGD_COND(eLogHarness, "SetLayerCropInsideBuffer (%f,%f) %fx%f -> (%f,%f,%f,%f)",
...
void Hwch::ApiTest::SetLayerDisplayFrame(Hwch::Layer* layer)
{
    ...
    float cropWidth = layer->GetCrop().Width(); // NB this means that absolute co-ordinates must be used
    ...
    {
        uint32_t minWidth = min<uint32_t>(mScreenLogWidth, max<uint32_t>(mMinDisplayFrameWidth, mMinLayerScale * lsCropWidth));
        uint32_t maxWidth = min<uint32_t>(mScreenLogWidth, mMaxLayerScale * lsCropWidth);
        uint32_t minHeight = min<uint32_t>(mScreenLogHeight, max<uint32_t>(mMinDisplayFrameHeight, mMinLayerScale * lsCropHeight));
        uint32_t maxHeight = min<uint32_t>(mScreenLogHeight, mMaxLayerScale * lsCropHeight);
        OnScreenDisplayFrameChoice dfXChoice(mScreenLogWidth, lsCropWidth, minWidth, maxWidth);
        OnScreenDisplayFrameChoice dfYChoice(mScreenLogHeight, lsCropHeight, minHeight, maxHeight);
    ...
    {
        FullDisplayFrameChoice dfXChoice(mScreenLogWidth, lsCropWidth, mMinDisplayFrameWidth, mMaxBufWidth);
        FullDisplayFrameChoice dfYChoice(mScreenLogHeight, lsCropHeight, mMinDisplayFrameHeight, mMaxBufHeight);
...
Hwch::Layer* Hwch::ApiTest::CreatePFLayerInternal(const char* name, uint32_t format, uint32_t layerIndex)
{
    uint32_t transform = mTransformChoice.Get();
    bool reallyRotated90 = ((transform & eTransformRot90) != 0);
    ...
    HWCLOGD_COND(eLogHarness, "screenIsRotated90 %d transform %d reallyRotated90 %d", mScreenIsRotated90, transform, reallyRotated90);
    ...
    HWCLOGD_COND(eLogHarness, "DisplayFrameBounds (%d, %d, %d, %d)", minX, minY, maxX, maxY);
    ...
    if ((layerIndex == 0) && ((minX >= 0) || (minY >= 0)))
    {
        ...
        dfWidth = Scaled((minX >= 0) ? (maxX - minX) : mScreenLogWidth, mScreenLogWidth);
        dfHeight = Scaled((minY >= 0) ? (maxY - minY) : mScreenLogHeight, mScreenLogHeight);
    }
    else
    {
        if (minX < 0)
        {
            // Auto mode
            OnScreenDisplayFrameChoice dfXChoice(mScreenLogWidth, 0, mMinPFDisplayFrameWidth, mScreenLogWidth);
            dfWidth = dfXChoice.Get();
            dfX = dfXChoice.GetOffset();
        }
        else
        {
            ...
            HWCLOGD_COND(eLogHarness, "mMinPFDisplayFrameWidth %d minX %d maxX %d", mMinPFDisplayFrameWidth, minX, maxX);
            int32_t w = Choice(mMinPFDisplayFrameWidth, maxX - minX, "letter/pillar w").Get();
            dfWidth = Scaled(w, mScreenLogWidth);
            HWCLOGD_COND(eLogHarness, "dfWidth=%d", w);
            dfX = Scaled(Choice(minX, maxX - w, "letter/pillar x").Get(), mScreenLogWidth);
        }

        if (minY < 0)
        {
            OnScreenDisplayFrameChoice dfYChoice(mScreenLogHeight, 0, mMinPFDisplayFrameHeight, mScreenLogHeight);
            dfHeight = dfYChoice.Get();
            dfY = dfYChoice.GetOffset();
        }
        else
        {
            HWCLOGD_COND(eLogHarness, "mMinPFDisplayFrameHeight %d minY %d maxY %d", mMinPFDisplayFrameHeight, minY, maxY);
            int32_t h = Choice(mMinPFDisplayFrameHeight, maxY - minY, "letter/pillar h").Get();
            dfHeight = Scaled(h, mScreenLogHeight);
            HWCLOGD_COND(eLogHarness, "dfHeight=%d", h);
            dfY = Scaled(Choice(minY, maxY - h, "letter/pillar y").Get(), mScreenLogHeight);
        }
    }
    ...
    *p++='x';
    ...
    HWCLOGD_COND(eLogHarness, "Format %s transform %d screen %dx%d df %s dfInSrc %dx%d scale %fx%f",
        FormatToStr(format),
        transform, mScreenWidth, mScreenHeight, strbuf, dfWidthInSrc, dfHeightInSrc, (double) xscale, (double) yscale);
    ...
    // Now as we've adjusted the crop, we need to work the other way and regenerate the display frame
    if (transform & eTransformRot90)
    {
    ...
    *p++='x';
    dfHeight.WriteStr(p,"%d");
    HWCLOGD_COND(eLogHarness, "CreatePfLayerInternal: crop (%f,%f) screenLogInSrc (%d,%d) df %s", double(cropWidth), double(cropHeight),
        screenLogWidthInSrc, screenLogHeightInSrc, strbuf);

    uint32_t bufferWidth = Choice(cropWidth, screenLogWidthInSrc * 2, "bufferWidth").Get();        // TODO: more advanced way of determining a buffer size
    uint32_t bufferHeight = Choice(cropHeight, screenLogHeightInSrc * 2, "bufferHeight").Get();

    RoundSizes(format, bufferWidth, bufferHeight);
    HWCLOGV_COND(eLogHarness, "New buffer format 0x%x %s %dx%d", format, FormatToStr(format), bufferWidth, bufferHeight);
    ...
    float cropX = CropAlignmentChoice(bufferWidth, cropWidth).Get();
    float cropY = CropAlignmentChoice(bufferHeight, cropHeight).Get();
    EnforceMinCrop(format, bufferWidth-cropWidth, bufferHeight-cropHeight, cropX, cropY);
...
void Hwch::ApiTest::EnforceMinCrop(uint32_t format, uint32_t bufferWidth, uint32_t bufferHeight, float& w, float& h)
{
    mSystem.GetBufferFormatConfigManager().AdjustCropSize(format, bufferWidth, bufferHeight, w, h);
}
...
Hwch::Layer* Hwch::ApiTest::CreateLayer(const char* name)
{
    ...
    uint32_t height = BufferSizeChoice(mScreenHeight, mMinBufHeight,
                                       min(HWCH_MAX_PIXELS_PER_BUFFER / width, (uint32_t) mMaxBufHeight)).Get();
    RoundSizes(format, width, height);

    HWCLOGV_COND(eLogHarness, "New buffer format 0x%x %s %dx%d", format, FormatToStr(format), width, height);
...
// This creates a layer with the same dimensions and location as the one underneath.
// It is RGBA and may or may not be transparent.
Hwch::Layer* Hwch::ApiTest::CreateOverlayLayer(const char* name, Hwch::Layer* inLayer, uint32_t colour)
{
    uint32_t format = HAL_PIXEL_FORMAT_RGBA_8888;

    HWCLOGV_COND(eLogHarness, "New overlay buffer format 0x%x %s colour 0x%x", format, FormatToStr(format), colour);

    Layer* layer = new Layer(name, inLayer->GetWidth(), inLayer->GetHeight(), format);
...
Hwch::Layer* Hwch::ApiTest::CreatePanelFitterLayer(const char* name, uint32_t layerIndex)
{
    uint32_t format = mPFFormatChoice.Get();

    // With panel fitter we decide the displayframe first, then work out the crop using the scale factor
...
void Hwch::ApiTest::ChoosePattern(Hwch::Layer* layer)
{
    ...
    if (fg == 0)
    {
        // Let's make half of all layers with transparent background transparent all over
    ...
    {
        layer->SetPattern(mPatternMgr.CreateHorizontalLinePtn(layer->GetFormat(), mUpdateRateChoice.Get(), fg, bg));
    }
}
...
int Hwch::ApiTest::RunScenario()
{
    ...
    mDisplayFrameInsideScreen = (GetParam("display_frame_not_inside_screen") == 0);

    uint32_t maxAcquireDelay = GetTimeParamUs("max_acquire_delay", 15000) / HWCVAL_MS_TO_US;    // maximum acquire delay given to layers (in ms)

    // Whether to disable adding NV12 layers
    mNoNV12 = (GetParam("no_NV12") != 0);
    bool noLinearNV12 = (GetParam("no_linear_NV12") != 0);
    bool noXTiledNV12 = (GetParam("no_x_tiled_NV12") != 0);
    bool noYTiledNV12 = (GetParam("no_y_tiled_NV12") != 0);
    bool noYUY2 = (GetParam("no_YUY2") != 0);

    // Maximum buffer size. Use smaller numbers to encourage the test to generate more layers.
    ...
    uint32_t protectedSessionCreatePeriod = GetIntParam("protected_session_create_period", 200);

    bool panelFitter = (GetParam("panel_fitter") != 0); // create layer lists which encourage the use of panel fitter

    // For panel fitter tests, minimum scale factor the tests will generate
    float minScaleFactor = GetFloatParam("panel_fitter_validation_min_scale_factor", HWCH_PANELFITVAL_MIN_SCALE_FACTOR);

    // For panel fitter tests, minimum scale factor HWC will use panel fitter for
    float minPFScaleFactor = GetFloatParam("panel_fitter_validation_min_supported_scale_factor", HWCH_PANELFITVAL_MIN_PF_SCALE_FACTOR);

    // For panel fitter tests, maximum scale factor HWC will use panel fitter for
    float maxPFScaleFactor = GetFloatParam("panel_fitter_validation_min_supported_scale_factor", HWCH_PANELFITVAL_MAX_PF_SCALE_FACTOR);

    // For panel fitter tests, maximjm scale factor the tests will generate
    float maxScaleFactor = GetFloatParam("panel_fitter_validation_max_scale_factor", HWCH_PANELFITVAL_MAX_SCALE_FACTOR);
    ...
    // Small number of iterations by default, so "valhwch -all" does not take too long.
    ...
    // Minimum and maximum scale factors. Defaults mean that other limitations will be more
    // significant.
    ...
    // Api test creates large number of layers with crazy and unrealistic combinations of
    // buffer and layer sizes, parameters and alignments.
    // As a result, PartitionedComposer has not been optimized to cope with these parameters
    // and so sometimes composition is extremely slow.
    // Alternatively, it may be that our current practice of filling the buffers from the CPU
    // rather than the GPU causes locking issues in the kernel which can make it very slow
    // for GL to access the buffers in partitioned composer.
    //
    // The result of this is that OnSet may exceed the normal 200ms time limit we have applied.
    // To prevent spurious test failure, we therefore downgrade this error to a warning, in
    // this test only.
    ...
    HWCLOGD_COND(eLogHarness, "ApiTest: Screen %dx%d", mScreenWidth, mScreenHeight);
    ...
    //mFormatChoice.Add(HAL_PIXEL_FORMAT_RGBA_4444_INTEL);
    ...
    else
    {
        ...
        if (!noXTiledNV12)
        {
            mFormatChoice.Add(HAL_PIXEL_FORMAT_NV12_X_TILED_INTEL);
            mFormatChoice.Add(HAL_PIXEL_FORMAT_NV12_X_TILED_INTEL);
            mFormatChoice.Add(HAL_PIXEL_FORMAT_NV12_X_TILED_INTEL);
            mFormatChoice.Add(HAL_PIXEL_FORMAT_NV12_X_TILED_INTEL);
        }
        ...
        if (!noLinearNV12)
        {
            mFormatChoice.Add(HAL_PIXEL_FORMAT_NV12_LINEAR_INTEL);
            mFormatChoice.Add(HAL_PIXEL_FORMAT_NV12_LINEAR_PACKED_INTEL);
            mFormatChoice.Add(HAL_PIXEL_FORMAT_NV12_LINEAR_CAMERA_INTEL);
        }
    }

    //mFormatChoice.Add(HAL_PIXEL_FORMAT_YUV420PackedSemiPlanar_INTEL);
    //mFormatChoice.Add(HAL_PIXEL_FORMAT_YUV420PackedSemiPlanar_Tiled_INTEL);

    if (!noYUY2)
    {
        mFormatChoice.Add(HAL_PIXEL_FORMAT_YCbCr_422_I);
        //mFormatChoice.Add(HAL_PIXEL_FORMAT_YCbCr_420_H_INTEL
        //mFormatChoice.Add(HAL_PIXEL_FORMAT_YCbCr_411_INTEL
        //mFormatChoice.Add(HAL_PIXEL_FORMAT_YCbCr_422_V_INTEL
        //mFormatChoice.Add(HAL_PIXEL_FORMAT_YCbCr_422_H_INTEL
        //mFormatChoice.Add(HAL_PIXEL_FORMAT_YCbCr_444_INTEL
    }
    ...
    mWidthChoice = new BufferSizeChoice(mScreenWidth, mMinBufWidth, HWCH_MAX_BUFFER_WIDTH);
    mBlendingChoice.Add(HWC_BLENDING_PREMULT);
    mBlendingChoice.Add(HWC_BLENDING_NONE);
    // Gary says COVERAGE is not used and is not well defined so no point in testing it.
    ...
    {
        mProtectionValidityChoice.Add(Hwch::Layer::eEncrypted);
        mProtectionValidityChoice.Add(Hwch::Layer::eEncrypted | Hwch::Layer::eInvalidSessionId);
        mProtectionValidityChoice.Add(Hwch::Layer::eEncrypted | Hwch::Layer::eInvalidInstanceId);
        mProtectionValidityChoice.Add(Hwch::Layer::eEncrypted | Hwch::Layer::eInvalidInstanceId | Hwch::Layer::eInvalidSessionId);
    }
    ...
    mPanelFitterScaleChoice.SetLimits(minScaleFactor, minPFScaleFactor, maxScaleFactor, maxPFScaleFactor);
    ...
    Choice numFramesChoice(1, maxFramesPerIteration); // Number of frames to send between each layout update
    ...
    uint32_t layerCreateCount = 100; // Start from 101 to avoid confusion with other numbering schemes

    // Ensure we don't go into extended mode and create spurious errors
    UpdateInputState(true, false);

    // If we intend to create protected layers, warm up HWC so we can create sessions straight away
    // if needed.
    ...
    for (uint32_t i=0; i<testIterations; ++i)
    {
        ...
        // Remove all existing layers every mClearLayersPeriod iterations, so behaviour is consistent.
        ...
        // Choose whether to use presentation mode and share out the ram allowance accordingly
        ...
        if (!presentationMode)
        {
            ...
            // We are in clone mode, so we can't have any presentation mode layers on D1 or D2
        ...
        for (uint32_t d = minDisp; d < maxDisp; ++d)
        {
            ...
            HWCLOGV_COND(eLogHarness, "D%d: Deleting layers till we have %d/%d", d, layersToKeep, layersRemaining);
            ...
            HWCLOGV_COND(eLogHarness, "D%d: Assessing RAM used by remaining %d layers", d, layersRemaining);
            uint32_t ram=0;
            ...
            HWCLOGD_COND(eLogHarness, "D%d Mode: %s remaining layers: %d required: %d",
                d, presentationMode ? "presentation" : "clone", frame.NumLayers(d), requiredLayerCount);
            ...
            {
                HWCLOGI_COND(eLogHarness, "No RAM available to create one layer on D%d. ram=%d, maxRamSoFar=%d, requiredLayerCount=%d",
            ...
            while ((layersRemaining < requiredLayerCount) && (ram < maxRamSoFar))
            {
                ...
                HWCLOGV_COND(eLogHarness, "D%d: Adding layer at Z=%d disp=%d", d, layerZOrder, (presentationMode ? d : -1));
        ...
        for (uint32_t f=0; f<numFrames; ++f)
        {
            ...
            uint32_t d = presentationMode ? Choice(0, mInterface.NumDisplays() - 1, "presentation mode display").Get() : 0;
            int layersRemaining = frame.NumLayers(d);
            ...
            if (layersRemaining > 0)
            {
                ...
                {
                    HWCLOGD_COND(eLogHarness, "Modifying layer crop %d (%s)", l, layer->GetName());
                ...
                {
                    HWCLOGD_COND(eLogHarness, "Modifying layer display frame and transform %d (%s)", l, layer->GetName());
                    SetLayerDisplayFrame(layer);
                }

                if (updateBlendingChooser.Get() == 0)
                {
                    HWCLOGD_COND(eLogHarness, "Modifying layer blending %d (%s)", l, layer->GetName());
                    SetLayerBlending(layer);
                }
            }
            else
    ...
    HWCLOGV_COND(eLogHarness, "Api test complete, reporting statistics and restoring state");
    ...
    if (allowedTransformErrors > 0)
    {
        ...
        ConditionalDropPriority(eCheckPlaneTransform, allowedTransformErrors, ANDROID_LOG_WARN);
    }
...
void Hwch::ApiTest::ReportStatistics()
{
    ...
    if (mRCEnabled)
    {
        ...
        printf("RC layers with 'RC':        %6d RC layers with 'CC_RC':     %6d RC layers with 'Hint':      %6d\n",
            mNumRCLayersRC, mNumRCLayersCC_RC, mNumRCLayersHint);
    }

    printf("Protected sessions started: %6d Protected layers created:   %6d Skip layers created:        %6d\n",
        mNumProtectedSessionsStarted, mNumProtectedLayersCreated, mNumSkipLayersCreated);
    printf("Suspends:                   %6d Mode changes:               %6d Video opt mode changes:     %6d\n",
