...
*
* File Name:            HwchRandomModesTest.cpp
*
...
int Hwch::RandomModesTest::RunScenario()
{
    ...
    // With multiple simultaneous random events going on, HWC does not always leave the panel
    ...
    mExtendedModeChooser.SetMax(extendedModePeriod - 1, (extendedModePeriod == 0));

    // Set workaround flag
    mAvoidVideoOnResumeOrModeChange = (GetParam("avoid_video_on_resume_or_mode_change") != 0);
...
void Hwch::RandomModesTest::ChooseExtendedMode()
{
    ...
    {
        if ((HwcTestState::getInstance()->GetHwcFrame(0) >= mDontStartExtendedModeBefore)
            && (mExtendedModeChooser.Get() == 0))
        {
            // Perform an extended mode state change
            // Decide if we want to be in extended mode, at what video rate, and whether there is input timeout
...
void Hwch::RandomModesTest::DetermineExtendedModeExpectation()
{
    ...
    {
        HWCLOGV_COND(eLogHarness, "Panel disable expected: NV12 present, video playing, input timed out");
    ...
    {
        HWCLOGV_COND(eLogHarness, "Panel disable NOT expected: NV12 %spresent, video %splaying, input %s",
                mVideoPresent ? "" : "NOT ",
                mVideoPlaying ? "" : "NOT ",
...
void Hwch::RandomModesTest::ClearVideo()
{
    if (mAvoidVideoOnResumeOrModeChange)
    {
        ...
        mDontStartExtendedModeBefore = HwcTestState::getInstance()->GetHwcFrame(0) + HWCVAL_EXTENDED_MODE_CHANGE_WINDOW;
    }
}
