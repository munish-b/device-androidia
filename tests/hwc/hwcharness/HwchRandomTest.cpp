...
void Hwch::RandomTest::ParseOptions() {
  ...
  // Mean period (in frames) for entry/exit of wireless docking mode. Note, this
  // only works if
  ...
  if (continuousKernelEvent) {
    AsyncEvent::RepeatData* repeatData =
        new AsyncEvent::RepeatData(burstLength, burstInterval);
  ...
  mWirelessDockingChooser.SetMax(wirelessDockingPeriod - 1,
                                 (wirelessDockingPeriod == 0));
