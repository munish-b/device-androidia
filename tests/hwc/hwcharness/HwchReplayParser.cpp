 ...
 *
 * @file    Hwch::ReplayParser.cpp
 * @author  James Pascoe (james.pascoe@intel.com)
 * @date    12th June 2014
 * @brief   Implementation of the parser.
 *
 * @details This file implements the parsing API. See Hwch::ReplayParser.h for
    ...
    protected_disable_all_regex(protected_disable_all_string)
{
    ...
    if (!hwcl_onset_regex.ok() || !hwcl_onset_1533_regex.ok() ||
        ...
        !protected_disable_session_regex.ok() || !protected_disable_all_regex.ok())
    {
...
bool Hwch::ReplayParser::ParseHwclLayerIndex(const std::string& str, uint32_t& val)
{
    return RE2::PartialMatch(str, hwcl_layer_regex_hdr, &val) ||
           RE2::PartialMatch(str, hwcl_layer_1533_regex_hdr, &val);
}

bool Hwch::ReplayParser::ParseHwclLayerHandle(const std::string& str, uint64_t& val)
{
    void *np = NULL;

    return RE2::PartialMatch(str, hwcl_layer_regex_hdr, np, np, RE2::Hex(&val)) ||
           RE2::PartialMatch(str, hwcl_layer_1533_regex_hdr, np, np, RE2::Hex(&val));
}

bool Hwch::ReplayParser::ParseHwclLayerTransform(const std::string& str, uint32_t& val)
{
    void *np = NULL;

    return RE2::PartialMatch(str, hwcl_layer_regex_hdr, np, np, np, np, &val) ||
           RE2::PartialMatch(str, hwcl_layer_1533_regex_hdr, np, np, np, np, &val);
}

bool Hwch::ReplayParser::ParseHwclLayerBlending(const std::string& str, uint32_t& val)
{
    if (str == "OP" || str == "BL" || str == "CV")
    {
        val = str == "OP" ? HWC_BLENDING_NONE :
              str == "BL" ? HWC_BLENDING_PREMULT :
...
bool Hwch::ReplayParser::ParseHwclLayerColourSpace(const std::string& str, uint32_t& val)
{
    if (str == "RGBA"  || str == "RGBX"  || str == "RGB"   ||
        str == "565"   || str == "BGRA"  || str == "5551"  ||
        str == "4444"  || str == "YV12"  || str == "RAW"   ||
        str == "BLOB"  || str == "IMPL"  || str == "422s"  ||
        str == "420s"  || str == "422i"  || str == "NV12X" ||
        str == "NV12"  || str == "NV12Y" || str == "NV12L" ||
        str == "NV12T" || str == "???")
    {
        val =
          str == "RGBA"  ? HAL_PIXEL_FORMAT_RGBA_8888                          :
          str == "RGBX"  ? HAL_PIXEL_FORMAT_RGBX_8888                          :
          str == "RGB"   ? HAL_PIXEL_FORMAT_RGB_888                            :
          str == "565"   ? HAL_PIXEL_FORMAT_RGB_565                            :
          str == "BGRA"  ? HAL_PIXEL_FORMAT_BGRA_8888                          :
#if ANDROID_VERSION < 440
          str == "5551"  ? HAL_PIXEL_FORMAT_RGBA_5551                          :
          str == "4444"  ? HAL_PIXEL_FORMAT_RGBA_4444                          :
#endif
          str == "YV12"  ? HAL_PIXEL_FORMAT_YV12                               :
#ifdef HAL_PIXEL_FORMAT_RAW_SENSOR
          str == "RAW"   ? HAL_PIXEL_FORMAT_RAW_SENSOR                         :
#endif
          str == "BLOB"  ? HAL_PIXEL_FORMAT_BLOB                               :
          str == "IMPL"  ? HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED             :
          str == "422s"  ? HAL_PIXEL_FORMAT_YCbCr_422_SP                       :
          str == "420s"  ? HAL_PIXEL_FORMAT_YCrCb_420_SP                       :
          str == "422i"  ? HAL_PIXEL_FORMAT_YCbCr_422_I                        :
          str == "NV12X" ? HAL_PIXEL_FORMAT_NV12_X_TILED_INTEL                 :
          str == "NV12"  ? HAL_PIXEL_FORMAT_NV12_Y_TILED_INTEL                 :
          str == "NV12Y" ? HAL_PIXEL_FORMAT_NV12_Y_TILED_INTEL                 :
          str == "NV12L" ? HAL_PIXEL_FORMAT_NV12_LINEAR_INTEL                  :
          str == "NV12T" ? HAL_PIXEL_FORMAT_YUV420PackedSemiPlanar_Tiled_INTEL :
          0x0;

          /* Note: HAL_PIXEL_FORMAT_YUV420PackedSemiPlanar_INTEL also matches NV12L
           * in the 15.33 HWC logger. HAL_PIXEL_FORMAT_YUV420PackedSemiPlanar_INTEL
           * and HAL_PIXEL_FORMAT_NV12_LINEAR_INTEL are equivalent, so its ok to
           * return HAL_PIXEL_FORMAT_NV12_LINEAR_INTEL for NV12L. */
...
bool Hwch::ReplayParser::IsHwclLayerSkip(const std::string& line)
{
    ...
    return (RE2::PartialMatch(line, hwcl_layer_regex_trl, np, np, np,
            &flags, np) && (flags & HWC_SKIP_LAYER));
}
...
bool Hwch::ReplayParser::IsHwclLayerUnsupported(const std::string& line)
{
    ...
    return ((RE2::PartialMatch(line, hwcl_layer_regex_hdr, np, np, np, np, np,
             np, np, &colour_space) ||
             RE2::PartialMatch(line, hwcl_layer_1533_regex_hdr, np, np, np, np,
             np, np, np, &colour_space)) && (colour_space == "???"));
}

bool Hwch::ReplayParser::ParseHotPlug(const std::string& line, bool& connected)
{
...
bool Hwch::ReplayParser::ParseBlanking(const std::string& line, bool& blank, int32_t& disp)
{
...
bool Hwch::ReplayParser::ParseEncrypted(const std::string& line, int32_t& session, int32_t& instance)
{
...
bool Hwch::ReplayParser::ParseProtectedEnable(const std::string& line, int32_t& session, int32_t& instance)
{
...
bool Hwch::ReplayParser::ParseProtectedDisableSession(const std::string& line, int32_t& session)
{
...
bool Hwch::ReplayParser::ParseHwclOnSet(const std::string& line, int32_t& secs,
    int32_t& msecs, int32_t& nsecs, int32_t& frame, int32_t& disp, int32_t& flags)
{
    ...
    if (RE2::FullMatch(line, hwcl_onset_regex, &secs, &msecs, &nsecs_sp, &tid_sp, &disp,
        &frame_sp, &ret_fence, RE2::Hex(&buf), &acq_fence, &flags, &text))
    {
        ...
        HWCLOGV_IF(PARSER_DEBUG, "Parser output: %ds %03dms %06dns TID:%04d D%d onSet"
                   " Entry frame:%d Fd:%d outBuf:0x%x outFd:%d flags:%d%s",secs, msecs, nsecs,
                   tid, disp, frame, ret_fence, buf, acq_fence, flags, text.c_str());
        ...
        &ret_fence, RE2::Hex(&buf), &acq_fence, &flags, &text))
    {
        HWCLOGV_IF(PARSER_DEBUG, "Parser output: %ds %03dms %06dns D%d onSet Entry Fd:%d"
                   " outBuf:0x%x outFd:%d Flags:%d%s",secs, msecs, nsecs, disp, ret_fence,
                   buf, acq_fence, flags, text.c_str());
...
bool Hwch::ReplayParser::ParseHwclLayer(const std::string& line, Hwch::Layer& layer)
{
    // Use a 'StringPiece' so that RE2 can match incrementally
    re2::StringPiece input(line);

    int32_t layer_num = 0, fb = 0, transform = 0, width = 0, height = 0, refresh = 0;
    ...
    if (RE2::Consume(&input, hwcl_layer_regex_hdr,
        &layer_num, &comp, RE2::Hex(&handle), &fb_s, &transform, &refresh,
        &blend, RE2::Hex(&plane_alpha), &colour_space, &width, &height,
        &sc_left_f, &sc_top_f, &sc_right_f, &sc_bot_f,
        &df_left, &df_top, &df_right, &df_bot,
        &acq_fence, &rel_fence,
...
#if PARSER_DEBUG
        // re2::StringAppendF does not appear to support the '#' format specifier (i.e.
        // to include the '0x' with hex prints). Use a string stream to format the handle
        // for printing so that it matches the original input.
        std::stringstream padded_handle;
        padded_handle << std::showbase << std::setw(mHexPrintWidth) << std::hex << handle;

        re2::StringAppendF(&debug, "  %d %2s %s:%2s:%d %d %2s:%02X %-4s "
            "%4dx%-4d %6.1f,%6.1f,%6.1f,%6.1f %4d,%4d,%4d,%4d %d %d "
            "V:%4d,%4d,%4d,%4d ", layer_num, comp.c_str(),
            handle ? padded_handle.str().c_str() : "       0x0",
            fb_s.c_str(), transform, refresh, blend.c_str(), plane_alpha,
            colour_space.c_str(), width, height, sc_left_f, sc_top_f,
            sc_right_f, sc_bot_f, df_left, df_top, df_right, df_bot,
    ...
    if (RE2::Consume(&input, hwcl_layer_1533_regex_hdr,
        &layer_num, &comp, RE2::Hex(&handle), &fb, &transform,
        &blend, RE2::Hex(&plane_alpha), &colour_space, &width, &height,
        &sc_left, &sc_top, &sc_right, &sc_bot,
        &df_left, &df_top, &df_right, &df_bot,
        &acq_fence, &rel_fence,
        &vr_left, &vr_top, &vr_right, &vr_bot))
    {
        ...
        re2::StringAppendF(&debug, "    %d %2s %s:%2d:%d %2s:%2X %-5s "
           "%4dx%-4d %4d,%4d,%4d,%4d->%4d,%4d,%4d,%4d %d %d "
           "V:%4d,%4d,%4d,%4d ", layer_num, comp.c_str(),
           handle ? padded_handle.str().c_str() : "       0x0",
           fb, transform, blend.c_str(), plane_alpha,
    ...
    if (match_hwcnext || match_1533)
    {
        ...
        {
            HWCLOGE_IF(PARSER_DEBUG,"Layer header matched, but blending or "
        ...
        layer.SetLogicalDisplayFrame(Hwch::LogDisplayRect(df_left, df_top, df_right, df_bot));
        ...
        if (match_hwcnext)
        {
            layer.SetCrop(Hwch::LogCropRect(sc_left_f, sc_top_f, sc_right_f, sc_bot_f));
        }
        else
        {
        ...
        //
        // 1882,0,1920,1200
        while (RE2::Consume(&input, hwcl_layer_regex_vbr,
               &vr_left, &vr_top, &vr_right, &vr_bot))
        {
#if PARSER_DEBUG
            re2::StringAppendF(&debug, "%4d,%4d,%4d,%4d ", vr_left, vr_top,
                               vr_right, vr_bot);
#endif
        }
        ...
        if (RE2::Consume(&input, hwcl_layer_regex_trl,
            RE2::Hex(&usage), &hints, &hints_s, RE2::Hex(&flags), &flags_s))
        {
            layer.SetFlags(flags);

#if PARSER_DEBUG
            re2::StringAppendF(&debug, "U:%08x Hi:%d%s Fl:%x%s", usage,
                        hints, hints_s.c_str(), flags, flags_s.c_str());

            if ((flags & HWC_SKIP_LAYER) == 0)
            {
                HWCLOGE_IF(debug != line, "Replay parser output does not match "
                           "original input:\nParser output: %s\nOriginal line: %s",
        ...
        {
            HWCLOGE_IF(PARSER_DEBUG,"Layer header matched, but trailer is "
                                    "malformed: %s", line.c_str());
        }
    }
...
bool Hwch::ReplayParser::ParseDSProfile(const std::string& line, std::string& profile)
{
    void *np = NULL;

    return RE2::FullMatch(line, ds_layer_regex, np, np, np, np, np, np, np,
                          np, np, np, np, np, np, np, np, np, np, &profile);
}
                                      ...
                                      int32_t& width, int32_t& height)
{
    ...
    if (RE2::FullMatch(line, ds_display_regex, &disp, &width, &height,
        &xdpi_str, &ydpi_str, &refresh))
    {
        HWCLOGV_IF(PARSER_DEBUG, "Parser output:  Display[%d] : %dx%d, xdpi=%s,"
            " ydpi=%s, refresh=%d", disp, width, height, xdpi_str.c_str(),
            ydpi_str.c_str(), refresh);
...
bool Hwch::ReplayParser::ParseDSLayer(const std::string& line, Hwch::Layer& layer)
{
    std::string comp_type, name, profile, refresh_str;
    int32_t hints = 0, flags = 0, tr = 0;
    uint32_t buf_handle = 0, blend = 0, format = 0;
    float sc_top = 0.0, sc_left = 0.0, sc_bot = 0.0, sc_right = 0.0, refresh = 0.0;
                       ...
                       &df_right, &df_bot, &name, &refresh_str, &profile))
    {
        ...
        layer.SetCrop(Hwch::LogCropRect(sc_left, sc_top, sc_right, sc_bot));
        layer.SetLogicalDisplayFrame(Hwch::LogDisplayRect(df_left, df_top,
                                     df_right, df_bot));
        ...
        if (layer.mPattern != NULL)
        {
            ...
            layer.GetPattern().SetUpdateFreq(refresh ? refresh : mDefaultDSUpdateFreq);
        }

        HWCLOGV_IF(PARSER_DEBUG, "Parser output: %11s | %8x | %08x | %08x | %02x "
                                 "| %05x | %08x | [ %6.1f, %6.1f, %6.1f, %6.1f] |"
                                 "[ %4d, %4d, %4d, %4d] %s: %.3f\n", comp_type.c_str(),
                                 buf_handle, hints, flags, tr, blend, format, sc_left,
                                 sc_top, sc_right, sc_bot, df_left, df_top, df_right,
 ...
 * Unit-Test Framework
 *
 * This function provides a unit-testing framework that is designed to prevent regressions
 * when modifying the regular expressions. The intention is that test-cases can be added
 * easily as parsing mismatches are encountered in practice. Then, by running the tests from
 * the command-line (using the -replay_test argument), the developer can ensure that they have
...
bool Hwch::ReplayParser::RunParserUnitTests()
{
    const char *tests[] =
    {
        // PreETM 'onSet' string (from DualDisplay-BasicPlayback-H264-CloneMode.log)
        ...
        "0,  79,1920,1159 -1 -1 V:   0,  79,1920,1159 U:00002900 Hi:0 Fl:20000000",

        // Parse failure identified by Srinivas Kulkarni (email: 18/12/14 @ 11:16)
        "  0 OV 0xf690d1a0:20:060 OP:FF RGBX 1920x1280    0.0,  40.0,1920.0,1240.0"
        "    0,   0,1920,1200 116 -1 V:   0,   0,1920,1200 U:00000900 Hi:0 Fl:0 SO SC",
        ...
        "        HWC | b83db7a0 | 00000000 | 00000000 | 00 | 00100 | 7fa00f00 | [    0.0,   "
        "40.0, 1920.0, 1240.0] | [    0,    0, 1920, 1200] com.android.systemui.ImageWallpaper",

        // New notation adopted after the logging was migrated into HWC
        "  1 TG        0x0:--:0 60 BL:FF ???      0x0       0.0,   0.0,1920.0,1200.0    0,   "
        "0,1920,1200 -1 -1 V:   0,   0,1920,1200 U:00000000 Hi:0 Fl:0 OP DISABLE",

        "  1 TG        0x0:20:0 60 BL:FF ???      0x0       0.0,   0.0,1920.0,1200.0    0,   "
        "0,1920,1200 -1 -1 V:   0,   0,1920,1200 U:00000000 Hi:0 Fl:0 OP DISABLE",

        // Log sent to me by Gary (29/04/15) with 64 bit handles
        "  0 OV 0x7f72edc2d060: 0:0 60 OP:FF NV12Y  864x480     0.0,   0.0, 854.0, 480.0    0,"
        "  40,1280, 759 -1 -1 V:   0,  40,1280, 759 U:00006900 Hi:0 Fl:0 OP V ENCRYPT(S:0, I:5) S SC",
        ...
        "  0 FB 0x7f6e3f7e91a0: 0:0 60 BL:FF NV12Y 1920x1200    0.0,   0.0,1920.0,1200.0    "
        "0,   0,1920,1200 -1 -1 V:   0,   0,1920,1200 U:00004900 Hi:0 Fl:0 OP V ENCRYPT(S:0, I:1) OV",

        "  1 RT 0x7f6e3f7e9330:47:0 31 BL:FF 422i  1920x1200    0.0,   0.0,1920.0,1200.0    "
        "0,   0,1920,1200 -1 -3 V:   0,   0,1920,1200 U:00000a00 Hi:0 Fl:0 OP V ENCRYPT(S:0, I:1) CO VppComposer",
        ...
        "     5 FB 0x0: 0:0 BL:FF ???      0x0       0,   0,   0,   0->   0, 983,1920,1200 -1 -1 "
        "V:   0, 983,1920,1200 U:00000000 Hi:0 Fl:1:SKIP"
    };

    // Dummy variables for the parsing functions to write into
    int32_t secs = 0, msecs = 0, nsecs = 0, frame = 0, flags = 0, disp = 0, width = 0, height = 0;
    int32_t session = 0, instance = 0;
    ...
    for (int32_t i = 0; i < sz; ++i)
    {
        if (!ParseHwclOnSet(tests[i], secs, msecs, nsecs, frame, disp, flags) &&
            ...
            !ParseProtectedDisableAll(tests[i]) &&
