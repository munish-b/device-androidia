...
void Hwch::Frame::Add(Layer& layer, int disp) {
  if (layer.mFrame) {
    HWCERROR(eCheckFrameworkProgError,
             "Layer %s is already attached to a frame.", layer.mName.string());
    return;
  }
...
void Hwch::Frame::RotateTo(Hwch::RotationType rot, bool animate,
                           uint32_t disp) {
  RotationType relativeRotation =
      SubtractRotation(rot, mSystem.GetDisplay(disp).GetRotation());
  RotateBy(relativeRotation, animate, disp);
}

void Hwch::Frame::RotateBy(Hwch::RotationType rot, bool animate,
                           uint32_t disp) {
  if (rot == eRotateNone) {
    return;
  }
...
void Hwch::Frame::Free() {
  ...
  sw_sync_timeline_inc(mTimelineThread->Get(),
                       mTimelineThread->GetTimelineTime() + 1000);
