...
const char gTextureFragmentShader[] =
    ...
    "   vec4 eps = vec4(0.009, 0.009, 0.009, 0.009);\n"
    "   vec4 pix = texture2D(u_TextureUnit, v_TextureCoordinates);\n"
    "   if ( u_useDiscard > 0.5f && all(greaterThanEqual(pix, u_ignoreColour - eps)) && all(lessThanEqual(pix, u_ignoreColour + eps)) )\n"
    "   {\n"
    "       discard;\n"
    "   }\n"
    "   else\n"
...
{
    for (int error = (int)eglGetError(); error != EGL_SUCCESS ; error = (int)eglGetError())
    {
        if ( error != EGL_SUCCESS )
        {
            HWCERROR(eCheckGlFail, "after %s() eglError (0x%x) => %d", op, error, error);
        }
    }
}
...
bool GlTargetPlatform::StartFrame(void)
{
    ...
    HWCLOGD_COND(eLogGl, "GlTargetPlatform::StartFrame: x = %d y = %d w = %d h = %d", m_tX, m_tY, m_tW, m_tH);
...
bool GlTargetPlatform::InitEGl(uint32_t screenWidth, uint32_t screenHeight)
{
    ...
    HWCLOGD_COND(eLogGl, "eglCreateContext geteglerror = %d Context=%p", eglGetError(), m_context);
    ...
    HWCLOGD_COND(eLogGl, "GlTargetPlatform::InitEGL width = %d height = %d", w, h);
...
//audi:
    HWCLOGD_COND(eLogGl, "******: GlTargetPlatform::InitEGL - Exit ( rv = %d)", (int)rv);
...
bool GlTargetPlatform::InitTarget(android::sp<android::GraphicBuffer> buf)
{
    ...
    if ( clientBufferAddress )
    {
        ...
        {
            EGL_WIDTH, (EGLint)buf->getWidth(),
            EGL_HEIGHT, (EGLint)buf->getHeight(),
            EGL_MATCH_FORMAT_KHR, EGL_FORMAT_RGBA_8888_KHR,
            EGL_IMAGE_PRESERVED_KHR, EGL_TRUE,
            EGL_NONE
        };*/

        m_eglImage = eglCreateImageKHR(m_display, EGL_NO_CONTEXT, EGL_NATIVE_BUFFER_ANDROID, clientBufferAddress, NULL);
...
void GlTargetPlatform::Clear(float r, float g, float b, float a, int x, int y, int w, int h)
{
    HWCLOGD_COND(eLogGl, "GlTargetPlatform::Clear r=%f g=%f b=%f a=%f x=%d y=%d w=%d h=%d",
...
TexturePtr GlTargetPlatform::LoadTexture(PngImage& pngImage, TextureMode aMode)
{
    ...
    if ( texture )
    {
        ...
        HWCLOGD_COND(eLogGl, "W = %d H = %d ct = %d bt = %d", width, height, texture->color_type, texture->bit_depth);

        texture->glWidth = texture->imWidth = width;
        texture->glHeight = texture->imHeight = height;

        HWCLOGD_COND(eLogGl, "Texture width = %d height = %d", texture->glWidth, texture->glHeight);

        texture->data = (char*)(pngImage.GetDataBlob());

        texture->textureHandle = CreateTexture(texture->glWidth, texture->glHeight, GetGlColorFormat(texture->color_type), texture->data, aMode);
        texture->data = 0;
...
bool GlTargetPlatform::ApplyTexture(TexturePtr aTexture, int x, int y, int w, int h, bool bIgnore, float ignorer, float ignoreg, float ignoreb, float ignorea)
{
    bool rv = false;
    ...
    if ( aTexture )
    {
        ...
        if ( aTexture->bDoneInit == false )
        {
            ...
            HWCLOGD_COND(eLogGl, "GlTargetPlatform::ApplyTexture initializing vbo and attributes");
        ...
        if ( m_imageProgram == 0)
        {
            m_imageProgram = CreateProgram(gTextureVertexShader, gTextureFragmentShader);
            if ( m_imageProgram )
            {
        ...
        aTexture->aPositionLocation = glGetAttribLocation(m_imageProgram, "vPosition");
        checkGlError("glGetAttribLocation1");
        if (aTexture->aPositionLocation < 0)
        {
            HWCERROR(eCheckGlFail, "ApplyTexture: No location for vPosition attribute");
            return false;
        }

        aTexture->aTextureCoordinateLocation = glGetAttribLocation(m_imageProgram, "a_TextureCoordinates");
        checkGlError("glGetAttribLocation2");
        if (aTexture->aTextureCoordinateLocation < 0)
        {
            HWCERROR(eCheckGlFail, "ApplyTexture: No location for a_TextureCoordinates attribute");
            return false;
        }

        aTexture->aTextureUnitLocation = glGetUniformLocation(m_imageProgram, "u_TextureUnit");
        ...
        aTexture->uProjMatrix = glGetUniformLocation(m_imageProgram, "uProjectionMatrix");
        ...
        aTexture->aIgnoreColourLocation = glGetUniformLocation(m_imageProgram, "u_ignoreColour");
        checkGlError("glGetUniformLocation");
        ...
        aTexture->aUseDiscardLocation = glGetUniformLocation(m_imageProgram, "u_useDiscard");
        checkGlError("glGetUniformLocation");
        ...
        HWCLOGD_COND(eLogGl, "aTextureCoordinateLocation = %d", aTexture->aTextureCoordinateLocation);
        HWCLOGD_COND(eLogGl, "aTextureUnitLocation = %d", aTexture->aTextureUnitLocation);
        ...
        glUniform4f(aTexture->aIgnoreColourLocation, ignorer, ignoreg, ignoreb, ignorea);
        ...
        glVertexAttribPointer(aTexture->aPositionLocation, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GL_FLOAT), BUFFER_OFFSET(0));
        checkGlError("glVertexAttribPointer1");

        glVertexAttribPointer(aTexture->aTextureCoordinateLocation, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GL_FLOAT), BUFFER_OFFSET(2 * sizeof(GL_FLOAT)));
...
GLuint GlTargetPlatform::CreateProgram(const char* pVertexSource, const char* pFragmentSource)
{
...
void GlTargetPlatform::FreeProgram(GLuint& program, GLuint vertexShader, GLuint pixelShader)
{
    if (vertexShader)
    {
...
void GlTargetPlatform::DrawLine(float x1, float y1, float x2, float y2, int lineWidth, float drawr, float drawg, float drawb, float drawa)
{
    HWCLOGD_COND(eLogGl, "GlTargetPlatform::DrawLine (%f, %f) -> (%f, %f): width=%d color = (%f, %f, %f, %f)",
    ...
    GLuint uDrawColourLocation = glGetUniformLocation(m_lineProgram, "u_drawColour");
    checkGlError("glGetUniformLocation");
...
GLuint GlTargetPlatform::CreateTexture(const GLsizei width, const GLsizei height, const GLenum type, const GLvoid* pixels, TextureMode aMode)
{
    GLuint texture_object_id;

    HWCLOGD_COND(eLogGl, "BindTexture: width=%d height=%d type=%d pixels=%p", width, height, type, pixels);
...
GLuint GlTargetPlatform::CreateVbo(const GLsizeiptr size, const GLvoid* data, const GLenum usage)
{
    GLuint vboBuffer;
...
void GlTargetPlatform::loadOrthof(GLfloat *m, GLfloat l, GLfloat r, GLfloat b, GLfloat t,
                GLfloat n, GLfloat f)
{
   m[ 0] = 2.0f / (r - l);
   m[ 1] = 0.0f;
...
void GlTargetPlatform::loadOrtho2Df(GLfloat *m, GLfloat l, GLfloat r, GLfloat b, GLfloat t)
{
...
bool GlTargetPlatform::CopySurface(void *data, uint32_t /*stride*/)
{
    ...
    glReadPixels(0, 0, m_surfaceWidth, m_surfaceHeight, GL_RGBA, GL_UNSIGNED_BYTE, data);
