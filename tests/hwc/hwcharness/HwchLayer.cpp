...
{
    static char buf[100];
    sprintf(buf, "(%f, %f, %f, %f)", (double) left, (double) top, (double) right, (double) bottom);
    return buf;
}
...
static void IncLayerCount()
{
    if (++sLayerCount > 100)
    {
        if (sLayerCount > 500)
        {
            HWCERROR(eCheckInternalError, "Layer leak: %d layers allocated", sLayerCount);
        }
        else
        {
            HWCLOGD_COND(eLogLayerAlloc, "Currently %d layers allocated (+)", sLayerCount);
        }
    }
}
...
Hwch::Layer::Layer(const char* name,
  ...
  : mCompType(HWC2_COMPOSITION_CLIENT),
    ...
    mNeedBuffer(true),
    mPattern(0),
    mLogicalCropf(0.0, 0.0, CoordUnassigned<float>(), CoordUnassigned<float>()),
    mSourceCropf(0.0, 0.0, 0.0, 0.0),
    ...
    mSystem(Hwch::System::getInstance())
{
    ...
    HWCLOGI("Constructing layer %s %dx%d pixelFormat=%d numBuffers=%d usage=0x%x",
        mName.string(),mWidth.mValue, mHeight.mValue, mFormat, numBuffers, usage);
    memset(mClonedLayers, 0, sizeof(mClonedLayers));
...
Hwch::Layer::Layer(const Layer& rhs, bool clone)
  : mCompType(HWC2_COMPOSITION_CLIENT),
    ...
    mGeometryChanged(true),
    mIsForCloning(false),
    mUpdatedSinceFBComp(clone ? HWCH_ALL_DISPLAYS_UPDATED : rhs.mUpdatedSinceFBComp),
...
buffer_handle_t Hwch::Layer::Send() {
    ...
    bool buffersFilledAtLeastOnce = mBufs.get() && mBufs->BuffersFilledAtLeastOnce();

    if ((mBufs.get() && mPattern.get() && mBufs->NeedsUpdating()) && (patternNeedsUpdate || !buffersFilledAtLeastOnce))
    {
        HWCLOGD_COND(eLogHarness, "Layer %s setting updatedSinceLastFBComp", mName.string());
        ...
        if (!mSystem.IsFillDisabled())
        {
            ...
            if (buf.get())
            {
                ...
                UpdateRCResolve();
                mPattern->Fill(buf, Rect(0, 0, buf->getWidth(), buf->getHeight()), mBufs->GetInstanceParam());
            }
            else
            {
...
void Hwch::Layer::UpdateRCResolve()
{
    ...
    if (mCompressionType != eCompressionAuto)
    {
        ...
        if (mBufs.get())
        {
            ...
            {
                // Found an Aux buffer - set the Render Compression flag in the buffer details
                intel_ufo_buffer_resolve_details_t rd;
                rd.magic = sizeof( intel_ufo_buffer_resolve_details_t );
                HWCLOGV_COND(eLogRenderCompression, "UpdateRCResolve: Buffer %p has aux, compression type %s",
                ...
                if (gc.getBufferResolveDetails(mBufs->GetHandle(), &rd) == 0)
                {
                    ...
                    else if (mCompressionType == eCompressionHint)
                    {
                        ...
                        {
                            // Set the state based on the RC hint. If we deliberately want to ignore
                            // the hint, then use the opposite value.
                            switch (rd.hint)
                            {
                                case INTEL_UFO_BUFFER_HINT_RC_FULL_RESOLVE:
                                {
                                    rd.state = mSystem.IsRCHintToBeIgnored() ?
                                          INTEL_UFO_BUFFER_STATE_COMPRESSED /* i.e. not resolved */
                                        : INTEL_UFO_BUFFER_STATE_NO_CONTENT;
                                    HWCLOGD_COND(eLogRenderCompression, "Hint FULL RESOLVE, state -> %s", AuxBufferStateStr(rd.state));
                                    break;
                                }
                                ...
                                case INTEL_UFO_BUFFER_HINT_RC_DISABLE_RESOLVE:
                                {
                                    ...
                                    HWCLOGD_COND(eLogRenderCompression, "Hint PARTIAL/DISABLE RESOLVE, state -> %s", AuxBufferStateStr(rd.state));
                                    break;
                                }
                                default:
                                {
                                    HWCLOGW("Saw unhandled RC hint 0x%x - not changing RC state", rd.hint);
                                }
                            };
                        }
                        else
                        {
                            HWCLOGD_COND(eLogRenderCompression, "Buffer %p: RC hint ignored because %s %s",
                                mBufs->GetHandle(),
    ...
    if (IsForCloning())
    {
        ...
        for (uint32_t disp=1; disp<MAX_DISPLAYS; ++disp)
        {
            if (mSystem.GetDisplay(disp).IsConnected())
            {
                ...
                {
                    HWCLOGD_COND(eLogCloning, "Layer %s no change to cloning",mName.string());
                }
            ...
            else if (mClonedLayers[disp])
            {
                ...
                HWCLOGD_COND(eLogLayerAlloc, "Layer %s delete clone D%d", mName.string(), disp);
                delete mClonedLayers[disp];
                mClonedLayers[disp] = 0;

                // The chain of events started by the deletion of the cloned layer will have reset our mIsForCloning flag
                // So fix this.
                mIsForCloning = true;
            }
        }
...
void Hwch::Layer::CalculateSourceCrop(Display& display)
{
    ...
    if (!mIgnoreScreenRotation)
    {
        ...
        else
        {
            ...
            mSourceCropf.bottom = mLogicalCropf.bottom.Phys(display.GetLogicalHeight());
        }
    }
    else
    {
    ...
    mSystem.GetBufferFormatConfigManager().AdjustCrop(mFormat, bw, bh, mSourceCropf.left, mSourceCropf.top, w, h);
...
void Hwch::Layer::CalculateRects(Display& display)
{
    ...
    HWCLOGI_COND(eLogHarness, "CalculateRects(%s): LogCrop %s Crop %s", mName.string(), mLogicalCropf.Str("%f"),mSourceCropf.Str());
    HWCLOGI_COND(eLogHarness, "CalculateRects(%s): LogDF   %s DF   %s", mName.string(), mLogicalDisplayFrame.Str("%d"),mDisplayFrame.Str());

    // Create buffers if new, or size has changed (old buffers will be dereferenced)
    uint32_t width = mWidth.Phys(display.GetLogicalWidth());
    uint32_t height = mHeight.Phys(display.GetLogicalHeight());

    if (mNeedBuffer && ((mBufs.get() == 0) ||
        (mBufs->GetWidth() != width) || (mBufs->GetHeight() != height)))
    {
        // Overriding option values in this way does not work a way of creating X-tiled buffers when we want them
        // What we really need is for the y-tiling to be turned off at the moment when HWC calls drmModeAddFb for
        // the buffer. This will be some time later and is not under our control.
        //
        // The effect of this code when we were using it was therefore to create some X-tiled buffers at random
        // depending on what HWC threads happen to be doing when we turn the fbytile option off. This can cause
        // assertions in HWC because for example you can end up with some members of a buffer set X tiled while
        // others are Y tiled. The X tiled ones don't support RC and therefore a "geometry change required" assertion
        ...
        {
            HWCERROR(eCheckTestBufferAlloc, "Failed to create buffers for layer %s", mName.string());
        }
    ...
    if (!(
        ...
        (mSourceCropf.right <= width) &&
        (mSourceCropf.bottom <= height) ) &&
        ((mFlags & HWC_SKIP_LAYER) == 0))               // Don't check crop for SKIP layers, these are (0,0,0,0).
    {
...
// Used to fill black around the bottom layer as this is likely to be most or all of the screen.

void Hwch::Layer::FillExcluding(const hwc_rect_t& rect, const hwc_rect_t& exclRect)
{
    ...
    else
    {
        ...
        {
            t = exclRect.top;
            mPattern->Fill(mBufs->Get(), Rect(rect.left, rect.top, rect.right, exclRect.top), bufferParam);
        }

        if (exclRect.bottom < rect.bottom)
        {
            b = exclRect.bottom;
            mPattern->Fill(mBufs->Get(), Rect(rect.left, exclRect.bottom, rect.right, rect.bottom), bufferParam);
        }

        if (exclRect.left > rect.left)
        {
            mPattern->Fill(mBufs->Get(), Rect(rect.left, t, exclRect.left, b), bufferParam);
        }

        if (exclRect.right < rect.right)
        {
            mPattern->Fill(mBufs->Get(), Rect(exclRect.right, t, rect.right, b), bufferParam);
        }
    }
}
