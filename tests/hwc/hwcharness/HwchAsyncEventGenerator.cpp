...
bool Hwch::AsyncEventGenerator::Add(uint32_t eventType, int32_t delayUs) {
  if (Size() + 8 < MaxSize()) {
    ...
    HWCLOGD("AsyncEventGenerator: queue full, dropped event %s",
            AsyncEvent::EventName(eventType).string());
    return false;
  }
}

bool Hwch::AsyncEventGenerator::Add(uint32_t eventType,
                                    android::sp<Hwch::AsyncEvent::Data> data,
                                    int32_t delayUs) {
  if (delayUs < 0) {
    ...
    HWCLOGD("AsyncEventGenerator: queued %s at delay %fms",
            AsyncEvent::EventName(eventType).string(), double(delayUs) / 1000);
...
bool Hwch::AsyncEventGenerator::WirelessDocking(bool entry) {
  ...
  status_t st = HwcService_Widi_SetSingleDisplay(
      mHwcsHandle, entry ? HWCS_TRUE : HWCS_FALSE);

  return (st == 0);
}

bool Hwch::AsyncEventGenerator::Do(uint32_t eventType,
                                   android::sp<AsyncEvent::Data> data) {
...
void Hwch::AsyncEventGenerator::Do(AsyncEvent& ev) {
  ...
  // HWCLOGV("%s scheduled for %f", AsyncEvent::EventName(ev.mType).string(),
  ...
  if (!Do(ev.mType, ev.mData)) {
    HWCLOGI("ASync event generation failure: %s",
            AsyncEvent::EventName(ev.mType).string());
  }
}
...
bool Hwch::KernelEventGenerator::Add(
    uint32_t eventType, android::sp<AsyncEvent::Data> data, int32_t delayUs,
...
bool Hwch::KernelEventGenerator::Do(
    uint32_t eventType, android::sp<AsyncEvent::Data> data,
    android::sp<AsyncEvent::RepeatData> repeatData) {
  bool success = true;
