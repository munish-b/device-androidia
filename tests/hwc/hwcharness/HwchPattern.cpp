...
Hwch::SPixelWord::SPixelWord(uint32_t colour, uint32_t format)
{
    ...
    HWCLOGD_COND(eLogHarness, "Colour: %08x mBytesPerPixel: %d Pixel Word: %08x", colour, mBytesPerPixel, mWord32);
...
Hwch::FramebufferTargetPtn::FramebufferTargetPtn()
  : SolidColourPtn(eBlack)      // Background colour before overwriting with composition
{
}
...
int Hwch::SolidColourPtn::Fill(android::sp<android::GraphicBuffer> buf, const hwc_rect_t& rect, uint32_t& bufferParam)
{
...
////////////////////////////////////////////// HorizontalLinePtn ////////////////////////////////////////
Hwch::HorizontalLinePtn::HorizontalLinePtn(float mUpdateFreq, uint32_t fgColour, uint32_t bgColour)
...
void Hwch::HorizontalLinePtn::FillLumaLine(unsigned char* data, uint32_t row, uint32_t stride, uint32_t left, uint32_t width, SPixelWord pixel)
{
...
void Hwch::HorizontalLinePtn::FillChromaLineNV12(unsigned char* chromaData, uint32_t row, uint32_t stride,
...
void Hwch::HorizontalLinePtn::FillChromaULineYV12(unsigned char* chromaData, uint32_t row, uint32_t stride,
    uint32_t left, uint32_t width, SPixelWord pixel)
{
    unsigned char* lineStart = chromaData + stride * (row/2);
...
void Hwch::HorizontalLinePtn::FillChromaVLineYV12(unsigned char* chromaData, uint32_t row, uint32_t stride,
    uint32_t left, uint32_t width, SPixelWord pixel)
{
    unsigned char* lineStart = chromaData + stride * (row/2);
...
int Hwch::HorizontalLinePtn::Fill(android::sp<android::GraphicBuffer> buf, const hwc_rect_t& rect, uint32_t& bufferParam)
{
    void *data = 0;
    unsigned char* chromaStart = 0;
    ...
    if ((height == 0) || (width == 0))
    {
        HWCLOGD_COND(eLogHarness, "HorizontalLinePtn::Fill aborted %p %dx%d", buf->handle, width, height);
        return 0;
    }
    ...
    if ((format == HAL_PIXEL_FORMAT_YV12)) {
        ...
        HWCLOGV_COND(eLogHarness, "Starting %s fill, handle %p %dx%d, ustart=%p, vstart=%p, stride=%d, cstride=%d",
            (format == HAL_PIXEL_FORMAT_YV12) ? "YV12" : "NV12",
            buf->handle, width, height,
            ustart, vstart, stride, cstride);
    }
    else
    {
        err = buf->lock(GRALLOC_USAGE_SW_WRITE_OFTEN, &data);
    ...
    if (bufferParam == HWCH_BUFFERPARAM_UNDEFINED)
    {
        ...
        for (uint32_t row=0; row<height; ++row)
        {
            ...
            FillLumaLine((unsigned char*) data, row, stride, left, width, currentPixel);
        }
...
void Hwch::PngPtn::Set(android::sp<Hwch::PngImage> spImage)
{
    mspImage = spImage;
    Set(*spImage);
}

void Hwch::PngPtn::FillLineFromImage(unsigned char* data, uint32_t row, uint32_t stride, uint32_t left, uint32_t width)
{
...
int Hwch::PngPtn::Fill(android::sp<android::GraphicBuffer> buf, const hwc_rect_t& rect, uint32_t& bufferParam)
{
