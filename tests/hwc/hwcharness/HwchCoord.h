...
*
* File Name:            HwchCoord.h
*
* Description:          Smart co-ordinate class definition
*
* Environment:
*
* Notes:
...
namespace Hwch
{
    ...
    // The last value it will return will be eMaxRotate, which isn't really a rotation.
    inline RotationType operator++(RotationType& rot)
    {
    ...
    // Add rotations
    // This always returns a valid rotation from eRotateNone to eRotate270.
    inline RotationType operator+(RotationType rot1, RotationType rot2)
    {
        uint32_t r1 = (uint32_t) rot1;
        uint32_t r2 = (uint32_t) rot2;

        return (RotationType) ((r1 + r2) % (eMaxRotate));
    }
    ...
    enum CoordType
    {
        ...
        eCoordScaled,           // Calculate by scaling relative to screen size from original frame of reference
        eCoordWallpaper,        // Special for wallpaper layers: Give the X or Y size of D0, whichever is greater
        ...
        public:
            ...
            T Phys(T screenMax) const
            {
                switch (mType)
                {
                    ...
                    case eCoordScaled:
                        return RoundIfNeeded(screenMax, ((static_cast<double>(mValue) + 0.5) / 65536.0) * screenMax);
                ...
                switch (mType)
                {
                    ...
                    case eCoordMaxRel:
                        *buf++='M';
                        break;
                    case eCoordScaled:
                        buf += sprintf(buf, "S%f", ((static_cast<double>(mValue) + 0.5) / 65536.0));
                        return buf;
                        break;
                    case eCoordWallpaper:
                        *buf++='W';
    ...
    {
        public:
            CtrRelative<T>(const int32_t& value)
              : Coord<T>(value, eCoordCentreRel)
            {
            }

            CtrRelative<T>(const uint32_t& value)
              : Coord<T>(value, eCoordCentreRel)
            {
            }

            CtrRelative<T>(const float& value)
              : Coord<T>(value, eCoordCentreRel)
            {
            }

            CtrRelative<T>(const double& value)
              : Coord<T>(value, eCoordCentreRel)
            {
            }
    };
    ...
    {
        public:
            MaxRelative<T>(const int32_t& value)
              : Coord<T>(value, eCoordMaxRel)
            {
            }

            MaxRelative<T>(const uint32_t& value)
              : Coord<T>(value, eCoordMaxRel)
            {
            }

            MaxRelative<T>(const float& value)
              : Coord<T>(value, eCoordMaxRel)
            {
            }

            MaxRelative<T>(const double& value)
              : Coord<T>(value, eCoordMaxRel)
            {
            }
    };
    ...
    class LogicalRect
    {
        public:
            ...
            LogicalRect<T>(const Coord<T>& l, const Coord<T>& t, const Coord<T>& r, const Coord<T>& b)
