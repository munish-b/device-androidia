...
const int Hwch::Display::mRotationTable[eMaxRotate][eMaxTransform] = {
    ...
    // RotationType) to each of the 8 possible transforms.
...
void Hwch::Display::CloneTransform(Layer& panelLayer, Layer& layer) {
  ...
  if (mDisplayIx > 0) {
    ...
    layer.mName =
        android::String8("%s_%d").format(panelLayer.mName, mDisplayIx);
    ...
    HWCLOGI("Cloned layer %s (%d,%d,%d,%d) to (%d,%d,%d,%d) scale (%f, %f)",
            layer.mName.string(), panelLayer.mDisplayFrame.left,
