...
bool Hwch::System::AddEvent(
    uint32_t eventType, android::sp<Hwch::AsyncEvent::Data> data,
    int32_t delayUs, android::sp<Hwch::AsyncEvent::RepeatData> repeatData) {
...
void Hwch::System::SetHwcOption(android::String8& option,
...
void Hwch::System::HwcOptionState::Override(bool enable) {
  if (!mDefaultSet) {
    const char* str =
        HwcTestState::getInstance()->GetHwcOptionStr(mName.string());
  ...
  if (desiredValue != mCurrentValue) {
    HWCLOGV_COND(eLogHarness, "Setting %s option to %s", mName.string(),
                 desiredValue.string());
...
void Hwch::System::HwcOptionState::Reset() {
  if (mDefaultSet) {
    if (mCurrentValue != mDefaultValue) {
      HWCLOGV_COND(eLogHarness, "Resetting %s option to %s", mName.string(),
