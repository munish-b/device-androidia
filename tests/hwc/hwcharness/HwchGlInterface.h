...
typedef void (GLAPIENTRYP PFNGLEGLIMAGERESOLVEOESINTEL)(GLenum target, GLeglImageOES eglImage);
...
namespace Hwch
{
    ...
    class GlTargetPlatform
    {
        public:
            ...
            bool ReleaseTarget();
            void Clear(float r, float g, float b, float a, int x = -1, int y = -1, int w = -1, int h = -1);
            TexturePtr LoadTexture(PngImage& pngImage, TextureMode aMode = Nearest);
            void FreeTexture(TexturePtr &aTexture);
            bool ApplyTexture(TexturePtr aTexture, int x, int y, int w, int h, bool bIgnore, float ignorer, float ignoreg, float ignoreb, float ignorea);
            ...
            void DrawLine(float x1, float y1, float x2, float y2, int lineWidth, float drawr, float drawg, float drawb, float drawa);
        ...
        private:
            ...
            GLuint CreateTexture(const GLsizei width, const GLsizei height, const GLenum type, const GLvoid* pixels, TextureMode aMode);
    ...
    {
        public:
            // create render target of specified size, or use the screen size if no values supplied
            GlInterface()
            {
            ...
            {
                m_aGlTargetPlatform.Clear(m_clearR, m_clearG, m_clearB, m_clearA, x, y, w, h);
            }
            ...
            {
                m_aGlTargetPlatform.DrawLine(x1, y1, x2, y2, lineWidth, m_drawR, m_drawG, m_drawB, m_drawA);
            }
            ...
            // Applies the specified texture at the specified location, optionally can ignore 'IgnoreColour' pixels
            bool ApplyTexture(TexturePtr aTexture, int x, int y, int w, int h, bool bIgnore = false)
            {
                return (m_aGlTargetPlatform.ApplyTexture(aTexture, x, y, w, h, bIgnore, m_ignoreR, m_ignoreG, m_ignoreB, m_ignoreA));
            }
            ...
            // Return the handle of the currently in use buffer (ie the one we have been or will be drawing to)
