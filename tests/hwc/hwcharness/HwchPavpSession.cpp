...
int32_t Hwch::PavpSession::GetPavpSession(pavp_lib_session *pLibInstance,
                                          uint32_t &id) {
...
int32_t Hwch::PavpSession::StartCoreUSession(int32_t pavpSessionId) {
  ...
  // This will allow us to create protected buffers which can then be used by
  // HWC.
  //
  // Note - I'm using ALOG rather than HWCLOG here because CoreU puts debug in
  // the
  // Android log, and it's helpful to see which call generates which log
  // entries.
  //
  ...
  HWCCHECK(eCheckPavpFail);
  if (!CoreUPavpSetMDRMMode(mCoreu, bMultiPavpSessionSupport)) {
  ...
  if (CoreUDoPavpAction(mCoreu, &PavpInfo, sizeof(PavpInfo)) &&
    ...
    ALOGI("    Current CoreU power checking is %s",
          (checkPowerState) ? "on" : "off");
...
uint32_t Hwch::PavpSession::GetSessionInstance(int32_t session) {
  ...
  CoreUPavpQuerySessionStatus(mCoreu, sessionId, &sessionStatus, &pavpMode,
                              &instanceId);
...
PAVP_SESSION_STATUS Hwch::PavpSession::GetPavpSessionStatus(
    int32_t pavpSessionId) {
  ...
  if (!CoreUPavpQuerySessionStatus(mCoreu, sessionId, &status, &pavpMode,
                                   &instanceId)) {
...
void Hwch::PavpSession::SetPavpSession(int session, PAVP_SESSION_STATUS status,
                                       const char *str) {
  ...
  if (!CoreUPavpChangeSession(mCoreu, KM_PAVP_SESSION_TYPE_DECODE,
#ifdef HWCVAL_PAVP_REFACTORING
                              KM_PAVP_SESSION_MODE_HEAVY,
#endif
                              &sessionId, &status, &previousStatus)) {
...
bool Hwch::PavpSession::StartProtectedContent() {
  ...
  return true;
#else
  HWCERROR(eCheckFeatureNotInBuild,
           "PAVP not supported, so this test can't be run.");
  return false;
#endif
}
...
bool Hwch::PavpSession::ProtectedContentStarted() {
  ...
  if (!mProtectedContentStarted) {
    ...
    HWCERROR(eCheckPavpFail,
             "New PAVP instance has not been created. Still instance %d",
             mPavpInstance);
  }
#else
  HWCERROR(eCheckFeatureNotInBuild,
...
bool Hwch::PavpSession::threadLoop() {
  ...
  for (uint32_t tries = 1; tries <= 5; ++tries) {
    ...
    if (mSessionInstance[sessionToKill] >= 0) {
      ...
      SetPavpSession(sessionToKill, PAVP_SESSION_TERMINATE_IN_PROGRESS,
                     "TERMINATE_IN_PROGRESS");
      SetPavpSession(sessionToKill, PAVP_SESSION_UNINITIALIZED,
                     "UNINITIALIZED");
    ...
    if (pavpSessionId >= 0) {
      ...
      if (pavpInstanceId > 0) {
        ...
        HWCLOGI("Protected Content Session %d Instance %d", mPavpSessionId,
                mPavpInstance);
        return false;
      }
    }
