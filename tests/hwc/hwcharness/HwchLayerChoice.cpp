...
Hwch::BufferSizeChoice::BufferSizeChoice(uint32_t screenSize, uint32_t minSize, uint32_t maxSize)
  : mScreenSize(screenSize),
    mMaxSize(maxSize),
    mBufferSizeClassChoice("mBufferSizeClassChoice"),
    mSmallerChoice(minSize, min(screenSize - 1, maxSize), "BufferSize mSmaller"),
    mBiggerChoice(screenSize + 1, mMaxSize, "BufferSize mBigger")
{
    HWCLOGV_COND(eLogHarness, "BufferSizeChoice screen %d max %d", screenSize, maxSize);
...
uint32_t Hwch::BufferSizeChoice::Get()
{
    ...
    // TODO: Specific cases for panel fitter which supports up to 2048x2047 buffer size.
}
...
Hwch::CropAlignmentChoice::CropAlignmentChoice(uint32_t bufferSize, float cropSize)
  : mBufferSize(bufferSize),
    mCropSize(cropSize),
    mAlignmentChoice(eMinAligned, eAlignmentMax, "mAlignmentChoice"),
    mOffsetChoice(delta, ((float)bufferSize) - cropSize - delta, "mOffsetChoice")
{
    HWCLOGV_COND(eLogHarness, "CropAlignmentChoice bufferSize %d crop %d", bufferSize, cropSize);
}
...
// DisplayFrameChoice
Hwch::DisplayFrameChoice::DisplayFrameChoice(uint32_t screenSize, float cropSize, uint32_t minSize, uint32_t maxSize)
  : mScreenSize(screenSize),
...
{
    HWCLOGV_COND(eLogHarness, "DisplayFrameChoice screen %d crop %d max %d", mScreenSize, mCropSize, mMaxSize);
...
Hwch::FullDisplayFrameChoice::FullDisplayFrameChoice(uint32_t screenSize, float cropSize, uint32_t minSize, uint32_t maxSize)
...
Hwch::OnScreenDisplayFrameChoice::OnScreenDisplayFrameChoice(uint32_t screenSize, float cropSize, uint32_t minSize, uint32_t maxSize)
  : DisplayFrameChoice(screenSize, cropSize, minSize, maxSize)
{
...
Hwch::Coord<int32_t> Hwch::DisplayFrameChoice::Get()
{
    ...
    switch(scaleType)
    {
        case eNotScaled:
        {
            HWCLOGV_COND(eLogHarness, "Hwch::DisplayFrameChoice::Get NotScaled crop %d", mCropSize);
            dfSize = mCropSize;
            break;
        }
        case eScaledToMin:
        ...
        {
            HWCLOGV_COND(eLogHarness, "Hwch::DisplayFrameChoice::Get ScaledFullScreen %d", mScreenSize);
            dfSize = mScreenSize;
            break;
        }
        case eScaledSmaller:
        {
            HWCLOGV_COND(eLogHarness, "Hwch::DisplayFrameChoice::Get ScaledSmaller crop %d min %d max %d", mCropSize, mMinSize, mMaxSize);
            dfSize = Choice(mMinSize + 1, min(mCropSize - 1, mMaxSize)).Get();
            break;
        }
        case eScaledBigger:
        {
            HWCLOGV_COND(eLogHarness, "Hwch::DisplayFrameChoice::Get ScaledBigger crop %d max %d", mCropSize, mMaxSize);
            dfSize = Choice(mCropSize + 1, mMaxSize).Get();
            break;
        }
        case eScaledHuge:
        {
            HWCLOGV_COND(eLogHarness, "Hwch::DisplayFrameChoice::Get ScaledHuge %d", mMaxSize);
            dfSize = mMaxSize;
            break;
        }
        default:
...
Hwch::Coord<int32_t> Hwch::DisplayFrameChoice::GetOffset()
{
    ...
    switch(overlap.Get())
    {
        ...
        {
            HWCLOGV_COND(eLogHarness, "Hwch::DisplayFrameChoice OverlappingBothSides df %d screen %d", mDfSize, mScreenSize);
            int32_t overlapRange = mDfSize - mScreenSize;
            return Scaled(Choice(-overlapRange, -1, "Scaled eOverlappingBothSides").Get(), mScreenSize);
        }
        case eOverlappingMinOnly:
        {
            HWCLOGV_COND(eLogHarness, "Hwch::DisplayFrameChoice OverlappingMinOnly df %d screen %d", mDfSize, mScreenSize);
            ...
            return Scaled(Choice(-mDfSize + 1, -smallestOffset, "Scaled eOverlappingMinOnly").Get(), mScreenSize);
        }
        case eOverlappingMaxOnly:
        {
            HWCLOGV_COND(eLogHarness, "Hwch::DisplayFrameChoice OverlappingMaxOnly df %d screen %d", mDfSize, mScreenSize);
            int32_t smallestOffset = 1;
            ...
            return Scaled(Choice(mScreenSize - mDfSize + 1, mScreenSize - smallestOffset, "Scaled eOverlappingMaxOnly").Get(), mScreenSize);
        }
        case eAlignedMin:
        {
            HWCLOGV_COND(eLogHarness, "Hwch::DisplayFrameChoice AlignedMin df %d screen %d", mDfSize, mScreenSize);
            return Scaled(0, mScreenSize);
        }
        case eAlignedMax:
        {
            HWCLOGV_COND(eLogHarness, "Hwch::DisplayFrameChoice AlignedMax df %d screen %d", mDfSize, mScreenSize);
            return Scaled(mScreenSize - mDfSize, mScreenSize);
        }
        case eNotOverlapping:
        {
            HWCLOGV_COND(eLogHarness, "Hwch::DisplayFrameChoice NotOverlapping df %d screen %d", mDfSize, mScreenSize);
            return Scaled(Choice(1, mScreenSize - mDfSize - 1, "Scaled eNotOverlapping").Get(), mScreenSize);
        }
        default:
        {
            ALOG_ASSERT(0);
...
void Hwch::PanelFitterScaleChoice::SetLimits(float minScaleFactor, float minPFScaleFactor, float maxScaleFactor, float maxPFScaleFactor)
{
    ...
    HWCLOGD("PanelFitterScaleChoice: factor limits set to %f,%f,%f,%f",
        (double) minScaleFactor, (double) minPFScaleFactor, (double) maxScaleFactor, (double) maxPFScaleFactor);
}
...
float Hwch::PanelFitterScaleChoice::GetAValue()
{
    ...
    switch (mScalingChoice.Get())
    {
        case eMuchTooSmall:
        {
            ...
            HWCLOGD_COND(eLogHarness, "PanelFitterScaleChoice eMuchTooSmall %f", scaleFactor);
            break;
        }

        case eTooSmall:
        {
            scaleFactor = FloatChoice(mMinScaleFactor + scalingDelta, mMinPFScaleFactor - scalingDelta, "PanelFitterScale eTooSmall").Get();
            HWCLOGD_COND(eLogHarness, "PanelFitterScaleChoice eTooSmall %f", scaleFactor);
            break;
        }

        case eSmallestSupported:
        {
            scaleFactor = mMinPFScaleFactor + scalingDelta; // If we choose mMinPfScaleFactor exactly, we are unlikely to get global scaling owing to rounding
            HWCLOGD_COND(eLogHarness, "PanelFitterScaleChoice eSmallestSupported %f", scaleFactor);
            break;
        }

        case eSmaller:
        {
            scaleFactor = FloatChoice(mMinPFScaleFactor + scalingDelta, 1.0 - scalingDelta, "PanelFitterScale eSmaller").Get();
            HWCLOGD_COND(eLogHarness, "PanelFitterScaleChoice eSmaller %f", scaleFactor);
        }

        case eUnity:
        {
            scaleFactor = 1.0;
            HWCLOGD_COND(eLogHarness, "PanelFitterScaleChoice eUnity %f", scaleFactor);
            break;
        }

        case eBigger:
        {
            scaleFactor = FloatChoice(1.0 + scalingDelta, mMaxPFScaleFactor - scalingDelta, "PanelFitterScale eBigger").Get();
            HWCLOGD_COND(eLogHarness, "PanelFitterScaleChoice eBigger %f", scaleFactor);
            break;
        }

        case eTooBig:
        {
            scaleFactor = FloatChoice(mMaxPFScaleFactor + scalingDelta, mMaxScaleFactor, "PanelFitterScale eTooBig").Get();
            HWCLOGD_COND(eLogHarness, "PanelFitterScaleChoice eTooBig %f", scaleFactor);
            break;
        }
...
void Hwch::PanelFitterScaleChoice::GetDisplayFrameBounds(int32_t& minX, int32_t& minY, int32_t& maxX, int32_t& maxY)
{
    minX = mPFMinX;
    minY = mPFMinY;
    maxX = mPFMaxX;
...
Hwch::AlphaChoice::AlphaChoice()
  : mPlaneAlphaClassChoice(eTransparent, ePlaneAlphaMax, "mPlaneAlphaClassChoice"),
