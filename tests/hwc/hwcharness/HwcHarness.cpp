...
void Wake(bool wake, int backlight)
{
    ...
    void *libPowerHandle = dll_open(HWCVAL_LIBPATH "/hw/power.default.so", RTLD_NOW);
    if (!libPowerHandle)
    {
    ...
    pPowerModule->powerHint(pPowerModule, wake ? POWER_HINT_INTERACTION : POWER_HINT_LOW_POWER, 0);
    sleep(1);
#endif
...
void WrapString(android::String8& str8, uint32_t width)
{
    int len = str8.length();
    ...
    for (char* p = str; *p; ++p)
    {
        if (*p == ' ')
        {
            ...
            {
                // Current line is longer than "width" and there was a space in it somewhere
...
int HwcTestRunner::getargs(int argc, char **argv)
{
    ...
    // TODO: change option processing so that all options with parameters are of the form
    // -option=<parameter>
    ...
    for (int i = 1; i < argc; i++)
    {
            ...
            (strcmp(argv[i], "-verbose_help") == 0))
        {
            printf("Usage: %s [OPTIONS]...\n"
                   ...
                   "\t-verbose_help             All help, including some very specialised options\n"
                   "\t-all                      Run all tests\n"
                   "\t-t <test name>            Choose test to run (this option can be repeated)\n"
                   "\t-avoid <test name>        Choose test to skip when '-all' is specified (this option can be repeated)\n"
                   "\n"
                   "Replay options:\n"
                   "\t-replay_hwcl <file>       Replay a test from a Hardware Composer log file\n"
                   "\t-replay_ds <file>         Recreate a HWC scenario from the output of 'dumpsys'\n"
                   "\t-replay_ds_frames <num>   Override the default number of frames (2000) for a dumpsys replay\n"
                   "\t-replay_match <num>       Adjusts the criteria used to track buffers. Match on:\n"
                   "\t  0 - buffer handles that are 'known' to a frame\n"
                   "\t  1 - buffer width/height and logical display frame coordinates\n"
                   "\t  2 - buffer width/height, crop frame width and logical display frame width\n"
                   "\t  3 - buffer width/height, crop frame width/height and display frame width/height\n"
                   "\t  4 - buffer width/height and either the crop frame or the display frame coordinates\n\n"
                   "\t-replay_no_timing         Run without inter-frame spacing i.e. send frames to the HWC as fast as possible\n"
                   "\t-replay_no_protected      Disable protected content support in the replay\n"
                   "\t-replay_alpha=n           Sets an alpha value for all replay layers\n"
                   "\t-replay_test              Runs the parser unit-tests and prints any regular expression mismatches\n"
                   "\t-crc                      Enable CRC-based flicker checking (requires Android build with CONFIG_DEBUG_FS=y)\n"
                   "\n"
                   "Harness configuration:\n"
                   "\t-default_num_buffers      Number of buffers each layer will have, unless overriden in the code.\n"
                   "\t-no_compose               Disable Reference Composer composition\n"
                   "\t-no_fill                  For maximum speed, don't fill any of the display buffers\n"
                   "\t-no_shims                 Run without installing the shims\n"
                   "\t-no_hdmi                  Don't use any HDMI display that is connected.\n"
                   "\t-hwc_config=<filename>    Configure hwc options via a registry file.\n"
                   "\t-async_buffer_destruction Destroy Gralloc buffers on a separate thread with random delays\n"
                   "\t-screen_disable_method=<list> Configure available methods for suspend and resume on random tests\n"
                   "\t\t<list> = all | [blank [,]] [power [,]] [both]\n"
                   "\t-force_setdisplay_fail=<Range>\n"
                   "\t                          SetDisplays in <Range> will fail.\n"
                   "\t-force_rotation_animation Forces rotations to emulate the Android rotation animation\n"
                   "\t-send_frames=<Range>      Send only frames in <Range> to onPrepare/onSet (to simplify and speed up testing)\n"
                   "\t-render_compression=<opt> Sets the render compression flag in Gralloc\n"
                   "\t  RC    - sets the render compression bit (only) for all buffers\n"
                   "\t  CC_RC - flags all buffers as containing Clear Compressed or Render Compressed content\n"
                   "\t  Hint  - follows the hint sent to GL by HWC (see render_compress_ignore_hint below)\n"
                   "\t-render_compression_ignore_hint=<Range> Sets a range for ignoring hints sent to GL by the HWC (ignored for 'RC' or 'CC_RC')\n\n"
                   ...
                   "\tWhere\t<p>=percentage of sample points where the stall will happen\n"
                   "\t\t<t>=duration of stall (units follow)\n"
                   "\t\t<time unit>=s|ms|us|ns\n"
                   "\t-force_setdisplay_stall=<StallConfig> Stall configuration for calls to drmModeSetDisplay\n"
                   "\t-force_dpms_stall=<StallConfig>       Stall configuration for calls to configure DPMS on/off\n"
                   "\t-force_setmode_stall=<StallConfig>    Stall configuration for setMode service calls\n"
                   "\t-force_blank_stall=<StallConfig>      Stall configuration for blank calls\n"
                   "\t-force_unblank_stall=<StallConfig>    Stall configuration for unblank calls\n"
                   "\t-force_hotplug_stall=<StallConfig>    Stall configuration for hotplugs\n"
                   "\t-force_hotunplug_stall=<StallConfig>  Stall configuraiton for hot unplugs\n"
                   "\t-force_gem_wait_stall=<StallConfig>   Stall configuration for GEM_WAIT calls\n"
                   "\n"
                   "Test configuration:\n"
                   "\t-val_hwc_composition      Enable validation of HWC composition against reference composer using SSIM\n"
                   "\t-val_ivp_composition      Enable validation of iVP composition against reference composer using SSIM\n"
                   "\t-val_buffer_allocation    Enable test failure from buffer allocation checks\n"
                   "\t-val_displays             Enable test failure from kernel displays specific checks\n"
                   "\t-no_val_hwc               Inhibit test failure from Hardware Composer specific checks\n"
                   "\t-no_val_ivp               Inhibit test failure from IVP specific checks\n"
                   "\t-val_sf                   Enable test failure from SurfaceFlinger specific checks\n"
                   "\n"
                   "\nVirtual display and WiDi options:\n"
                   "\t-virtual_display <w>x<h>  Enables virtual display emulation for a specified width and height\n"
                   "\t-widi <w>x<h> <sw>x<sh> r Enables Widi support. The parameters correspond to the following:\n"
                   "\t  <w>x<h>     - specifies the resolution of the underlying display (i.e. the size of the framebuffer target)\n"
                   "\t  <sw>x<sh>   - sets the scaled resolution that is (normally) returned by the Widi stack in 'setResolution'\n"
                   "\t  refresh (r) - refresh rate that is (normally) returned by the Widi stack in 'setResolution'\n"
                   ...
                   "\t-widi_retain_oldest=n     Retains the oldest fence for 'n' frames before releasing it\n"
                   "\t-widi_window <w>x<h>      Displays the widi visualisation window (with a specified width and height)\n"
                   "\n"
                   "Logging options:\n"
                   "\t-brief                    Provide minimal information in stdout, focus on pass/fail\n"
                   "\t-logname=<name>           Set name of results file to results_<name>.csv\n"
                   "\t-shortlog                 Suppress verbose flags in logcat and hwclog\n"
            ...
            if (strcmp(argv[i], "-verbose_help") == 0)
            {
                printf(
                   "More options:\n"
                   "\t-blank_after                      OnBlank should be called after each test\n"
                   "\t-delay_page_flip                  Delay every 5th page flip on D0 by 500ms to test out-of-order buffer release\n"
                   "\t-dump_frames=<Range>              Dump tga files for all the input buffers on frames with frame numbers in the range\n"
                   "\t-randomize_modes                  Randomize the video modes on each hotplug (and choose a random subset)\n"
                   "\t-vsync_delay=n<time unit>         Set delay offset for VSync synchronization, in us\n"
                   "\t-vsync_timeout=n<time unit>       Set timeout for VSync synchronization, in us (Default=50000)\n"
                   "\t-vsync_period=n<time unit>        Set frame period for when VSyncs don't come, in us (Default=16666)\n"
                   "\t-sync_to=compose|prepare|set      Set the event that is synchronized to the given delay from VSync\n"
                   "\t\tWhere: <time unit>=s|ms|us|ns\n"
                   "\n"
                   "Buffer/Crop/Display frame size control:\n"
                   "\t-no_adjust_sizes                  Inhibit all the following controls\n"
                   "\t-min_buf_size                     Default minimum value for buffer width/height (D=1 pixel)\n"
                   "\t-min_buf_width                    Minimum buffer width (D=min_buf_size)\n"
                   "\t-min_buf_height                   Minimum buffer height (D=min_buf_size\n"
                   "\t-min_crop_width                   Minimum crop width (D=min_buf_width)\n"
                   "\t-min_crop_height                  Minimum crop height (D=min_buf_height)\n"
                   "\t-min_display_frame_size           Minimum display frame width/height (D=2 pixels)\n"
                   "\t-min_display_frame_width          Minimum display frame width (D=min_display_frame_size)\n"
                   "\t-min_display_frame_height         Minimum display frame height (D=min_display_frame_size)\n"
                   "\t-min_NV12_crop_width              Minimum crop width for NV12 buffers (D=min_crop_width or 4 pixels, whichever is greater)\n"
                   "\t-min_NV12_crop_height             Minimum crop height for NV12 buffers (D=min_crop_height or 4 pixels, whichever is greater)\n"
                   "\t-NV12_display_frame_alignment     Alignment of source crop offset and size for YUY2 buffers.\n"
                   "\t-min_NV12_display_frame_width     Minimum display frame width for NV12 buffers (D=min_display_frame_width or 4 pixels, whichever is greater)\n"
                   "\t-min_NV12_display_frame_height    Minimum display frame height for NV12 buffers (D=min_display_frame_height or 4 pixels, whichever is greater)\n"
                   "\t-NV12_display_frame_alignment     Alignment of display frame offset and size for NV12 buffers.\n"
                   "\t-min_YUY2_crop_width              Minimum crop width for YUY2 buffers (D=min_crop_width or 4 pixels, whichever is greater)\n"
                   "\t-min_YUY2_crop_height             Minimum crop height for YUY2 buffers (D=min_crop_height or 4 pixels, whichever is greater)\n"
                   "\t-YUY2_crop_alignment              Alignment of souce crop offset and size for YUY2 buffers.\n"
                   "\t-min_YUY2_display_frame_width     Minimum display frame width for YUY2 buffers (D=min_display_frame_width or 4 pixels, whichever is greater)\n"
                   "\t-min_YUY2_display_frame_height    Minimum display frame height for YUY2 buffers (D=min_display_frame_height or 4 pixels, whichever is greater)\n"
                   "\t-YUY2_display_frame_alignment     Alignment of display frame offset and size for YUY2 buffers.\n"
                   "\n"
                   "Some tests have additional test-specific options.\n"
                   "\n");
            }

            android::String8 names;
            Hwch::BaseReg::mHead->AllNames(names);
            android::String8 testNamesString = android::String8("Tests: ") + names;
            WrapString(testNamesString, 100);
            printf ("%s\n", testNamesString.string());
            return 0;
        }
        else if (strcmp(argv[i], "-t") == 0)
        {
            ...
            if (i < argc)
            {
                mTestNames.add(android::String8(argv[i]));
            }
        }
        else if (strcmp(argv[i], "-avoid") == 0)
        {
            ...
            if (i < argc)
            {
                mAvoidNames.add(android::String8(argv[i]));
            }
        }
        else if (strcmp(argv[i], "-replay_hwcl") == 0)
        {
            ...
            if (++i < argc)
            {
              ...
              HWCLOGI(" HwcTestBase::SetArgs - replay requested for HWC log: %s", mReplayFileName);
            }
        }
        else if (strcmp(argv[i], "-replay_ds") == 0)
        {
            ...
            {
                mReplayFileName = argv[i];
                HWCLOGI(" HwcTestBase::SetArgs - replay requested for dumpsys file: %s", mReplayFileName);
            }
        }
        else if (strcmp(argv[i], "-replay_ds_frames") == 0)
        {
            if (++i < argc)
            {
                ...
                {
                    HWCERROR(eCheckCommandLineParam, " HwcTestBase::SetArgs - number of frames for dumpsys replay must be > 0 (not %d)", mDSReplayNumFrames);
                }
                else
                {
                    HWCLOGI(" HwcTestBase::SetArgs - dumpsys replay requested for: %d frames", mDSReplayNumFrames);
                }
            }
        }
        else if (strcmp(argv[i], "-replay_match") == 0)
        {
            if (++i < argc)
            {
                ...
                {
                    HWCERROR(eCheckCommandLineParam, " HwcTestBase::SetArgs - match selection must be between 0 and 4 (seen: %d)", mReplayMatch);
                }
                else
                {
                    HWCLOGI(" HwcTestBase::SetArgs - selected match algorithm: %d frames", mDSReplayNumFrames);
                }
            }
        }
        else if (strcmp(argv[i], "-virtual_display") == 0)
        {
            ...
            if ((++i >= argc) || std::sscanf(argv[i],"%dx%d", &vd_width, &vd_height) != 2)
            {
               std::fprintf(stderr,"Fatal: can not parse virtual display dimensions.\n"
                                   "Usage is: -virtual_display <w>x<h> (e.g. -virtual_display 1920x1280)\n");
               exit(-1);
            }
            else if ((vd_width <= 0) || (vd_height <= 0))
            {
               std::fprintf(stderr,"Fatal: invalid virtual display dimensions (%dx%d).\n"
        ...
        else if (argv[i][0] == '-')
        {
            ...
            android::String8 parname;
            android::String8 parval;

            if (eq)
            {
                parname = android::String8 (parstr, eq-parstr);
                parval = eq + 1;
            }
            else
            {
    ...
    // because the argument is intercepted by the script and converted to an environment variable.
    ...
    // Arguments ABOVE this are ignored, because they are about what is logged rather than
    // how the test is run.
    UsedArgs()="";
    ...
    // Do we want buffers destroyed on a separate thread, to test Gralloc/HWC handshake
    config.SetCheck(eOptAsyncBufferDestruction, (GetParam("async_buffer_destruction") != 0));
    ...
    android::String8 renderCompress = GetStrParamLower("render_compression");
    ...
    if (GetRangeParam("render_compression_ignore_hint", rcIgnoreHintRange))
    {
        if ((mSystem.GetGlobalRenderCompression() == Hwch::Layer::eCompressionRC) ||
            (mSystem.GetGlobalRenderCompression() == Hwch::Layer::eCompressionCC_RC))
        {
            std::fprintf(stderr,"Warning: render compression ignore hint range specified with "
    ...
    int64_t maxUnblankingLatency = GetTimeParamUs("unblanking_time_limit", HWCVAL_MAX_UNBLANKING_LATENCY_DEFAULT_US) * HWCVAL_US_TO_NS;
...
void HwcTestRunner::SetBufferConfig()
{
    ...
    // Provide a global way to inhibit the fixup; could be useful for replay in particular.
    if (GetParam("no_adjust_sizes"))
    {
        Hwch::BufferFormatConfig deflt (1, 1,
            1, 1,
            1, 1,
            0.0, 1, 1);
    ...
    else
    {
        ...
        uint32_t minCropWidth = GetIntParam("min_crop_width", max(minCropSize, minBufWidth));
        uint32_t minCropHeight = GetIntParam("min_crop_height", max(minCropSize, minBufHeight));
        uint32_t minDisplayFrameSize = GetIntParam("min_display_frame_size", 2);
        uint32_t minDisplayFrameWidth = GetIntParam("min_display_frame_width", minDisplayFrameSize);
        uint32_t minDisplayFrameHeight = GetIntParam("min_display_frame_height", minDisplayFrameSize);

        uint32_t minNV12CropSize = GetIntParam("min_NV12_crop_size", max(4U, minCropSize));
        uint32_t minNV12CropWidth = GetIntParam("min_NV12_crop_width", max(minNV12CropSize, minCropWidth));
        uint32_t minNV12CropHeight = GetIntParam("min_NV12_crop_height", max(minNV12CropSize, minCropHeight));
        uint32_t minNV12DisplayFrameSize = GetIntParam("min_NV12_display_frame_size", max(4U, minDisplayFrameSize));
        uint32_t minNV12DisplayFrameWidth = GetIntParam("min_NV12_display_frame_width", max(minNV12DisplayFrameSize, minDisplayFrameWidth));
        uint32_t minNV12DisplayFrameHeight = GetIntParam("min_NV12_display_frame_height", max(minNV12DisplayFrameSize, minDisplayFrameHeight));
        ...
        uint32_t minYUY2CropWidth = GetIntParam("min_YUY2_crop_width", max(minYUY2CropSize, minCropWidth));
        uint32_t minYUY2CropHeight = GetIntParam("min_YUY2_crop_height", max(minYUY2CropSize, minCropHeight));
        uint32_t minYUY2DisplayFrameSize = GetIntParam("min_YUY2_display_frame_size", max(4U, minDisplayFrameSize));
        uint32_t minYUY2DisplayFrameWidth = GetIntParam("min_YUY2_display_frame_width", max(minYUY2DisplayFrameSize, minDisplayFrameWidth));
        uint32_t minYUY2DisplayFrameHeight = GetIntParam("min_YUY2_display_frame_height", max(minYUY2DisplayFrameSize, minDisplayFrameHeight));
        uint32_t yuy2DfMask = ~GetIntParam("YUY2_display_frame_alignment", 1) - 1;

        float alignment=GetFloatParam("crop_alignment", 0.0);
        float NV12Alignment=GetFloatParam("NV12_crop_alignment", max(alignment, float(2.0)));
        float YUY2Alignment=GetFloatParam("YUY2_crop_alignment", alignment);

        Hwch::BufferFormatConfig deflt (minDisplayFrameWidth, minDisplayFrameHeight,
            minBufWidth, minBufHeight,
            max(int(alignment),1), max(int(alignment),1),
            alignment, minCropWidth, minCropHeight);
        mgr.SetDefault(deflt);

        // NV12 must not have odd width or height, or small display frame.
        Hwch::BufferFormatConfig nv12 (minNV12DisplayFrameWidth, minNV12DisplayFrameHeight,
            minBufWidth, minBufHeight,
            max(int(NV12Alignment),2), max(int(NV12Alignment),2),
            NV12Alignment, minNV12CropWidth, minNV12CropHeight,
            nv12DfMask, nv12DfMask);
        mgr.add(HAL_PIXEL_FORMAT_NV12_Y_TILED_INTEL, nv12);
        mgr.add(HAL_PIXEL_FORMAT_NV12_X_TILED_INTEL, nv12);
        mgr.add(HAL_PIXEL_FORMAT_NV12_LINEAR_INTEL, nv12);
        mgr.add(HAL_PIXEL_FORMAT_NV12_LINEAR_PACKED_INTEL, nv12);
        mgr.add(HAL_PIXEL_FORMAT_NV12_LINEAR_CAMERA_INTEL, nv12);
        mgr.add(HAL_PIXEL_FORMAT_YV12, nv12);

        // YUY2 must not have odd width.
        Hwch::BufferFormatConfig yuy2 (minYUY2DisplayFrameWidth, minYUY2DisplayFrameHeight,
            minBufWidth, minBufHeight,
            max(int(YUY2Alignment),2), max(int(YUY2Alignment),1),
            YUY2Alignment, minYUY2CropWidth, minYUY2CropHeight,
            yuy2DfMask, yuy2DfMask);
        mgr.add(HAL_PIXEL_FORMAT_YCbCr_422_I, yuy2);
        mgr.add(HAL_PIXEL_FORMAT_YCbCr_422_V_INTEL, yuy2);
        mgr.add(HAL_PIXEL_FORMAT_YCbCr_422_H_INTEL, yuy2);
    }
...
void HwcTestRunner::ConfigureStall(Hwcval::StallType ix, const char* optionName)
{
...
void HwcTestRunner::ConfigureFrameDump()
{
    ...
    if (GetRangeParam("dump_frames", range))
    {
        ...
        mState->ConfigureImageDump(new Hwch::Range(range), 100);
    }

    if (GetRangeParam("dump_tgt_buffers", range))
    {
        HWCLOGD("Got dump_tgt_buffers");
        mState->ConfigureTgtImageDump(new Hwch::Range(range));
    }
}
...
void HwcTestRunner::LogTestResult()
{
    ...
    android::String8 testArgs = mArgs + mCurrentTest->UsedArgs();
...
void HwcTestRunner::EntryPriorityOverride()
{
    ...
    // Initially reduce the priority of this check so that in the log it appears as a warning
...
void HwcTestRunner::LogTestResult(const char* testName, const char* args)
{
    ...
    android::String8 strTestName(testName);
    mResults.replaceValueFor(strTestName, result);

    result.SetStartEndTime(mStartTime, mEndTime);
    result.Log(config,
               android::String8::format("%s %s", testName, args).string(),
               mBrief);
    ...
    else
    {
        if (mBrief && (mHwclogPath != ""))
        {
                ...
                (result.mCheckFailCount[eCheckHwcCompMatchesRef] > 0))
            {
                ...
                {
                    char* str = dumpPath.lockBuffer(dumpPath.length());
                    strcpy(str + pos, "dump_");

                    char* p;

                    for (p = str+pos+5; *(p+2); p++)
                    {
                        char ch = *(p+2);
...
{
    android::String8 resultsPath("/data/validation/hwc/results");
    ...
    resultsPath += ".csv";
    FILE* f = fopen(resultsPath.string(), "w");
    HWCLOGD_COND(eLogHarness, "Writing %s", resultsPath.string());

    if (f == 0)
    {
        ALOGD("Can't write %s", resultsPath.string());
    }
...
void HwcTestRunner::WriteDummyCsvFile()
{
    ...
    {
        const android::String8& name = mTestNames.itemAt(j);
...
void HwcTestRunner::WriteCsvFile()
{
    ...
    for (uint32_t j=0; j<mResults.size(); ++j)
    {
        const android::String8& name = mResults.keyAt(j);
        fprintf(f, ",%s", name.string());
    }
    ...
    for (uint32_t i=0; i<eHwcTestNumChecks; ++i)
    {
        ...
        {
            fprintf(f, "%s,%s", HwcTestConfig::GetDescription(i), HwcTestConfig::GetComponentName(i));

            for (uint32_t j=0; j<mResults.size(); ++j)
            {
                HwcTestResult& result = mResults.editValueAt(j);
                ...
                if (evals > 0)
                {
                    ...
                    else
                    {
                        ...
                        // That means that we passed some overall condition as to how many times this condition
                        // may occur, and therefore permitted a downgrade to warning, which here means a pass.
                        fprintf(f, ",0");
                    }
                }
                else
...
void HwcTestRunner::ParseCSV(const char* p, android::Vector<android::String8>& sv)
{
    while (p)
    {
        while (*p && !isprint(*p))
...
void HwcTestRunner::CombineFiles(int err)
{
    ...
    if (d)
    {
        ...
        if (out)
        {
            ...
            {
                fprintf(out, "Test Pass/Fail/Error,%d,%d,%d\n", mNumPasses, mNumFails, 0);
            }

            android::Vector<android::String8> files;
            android::Vector<android::String8> allTests;
            uint32_t numColumns = 0;
            uint32_t prevNumColumns = 0;

            struct CheckResData
            {
                android::String8 component;
                android::Vector<android::String8> res;
            };

            android::KeyedVector<android::String8, CheckResData> results;
            ...
            while ((dir = readdir(d)) != 0)
            {
                ...
                if ((strcmp(name, "results.csv") == 0) ||
                    ((strncmp(name, "results_", 8) == 0) && (strcmp(name+namelen-4, ".csv") == 0)))
                {
                    char path[256];
                    ...
                    if (in)
                    {
                        char* line = 0;
                        size_t len = 0;
                        android::Vector<android::String8> tests;
                        ...
                        if ((getline(&line, &len, in)) != -1)
                        {
                            ...
                            {
                                files.add(android::String8(name));
                            }
                        ...
                        {
                            android::Vector<android::String8> checkResults;
                            ...
                            android::String8 check = checkResults[0];
                            android::String8 component = checkResults[1];
                            checkResults.removeItemsAt(0, 2);

                            while (checkResults.size() < tests.size())
                            {
                                checkResults.add(android::String8());
                            }

                            ssize_t ix = results.indexOfKey(check);
                            if (ix < 0)
                            {
                                CheckResData rd;
                                ...
                                {
                                    rd.res.add(android::String8());
                                }

                                ix = results.add(check, rd);
                            }

                            CheckResData& rd = results.editValueAt(ix);

                            if (rd.component != component)
                            {
                                printf("CombineFiles: component inconsistency! %s %s %s\n",
                                    check.string(), component.string(), rd.component.string());
                            }
            ...
            {
                fprintf(out, ",%s", allTests.itemAt(i).string());
            }
            fprintf(out, "\n");

            for (uint32_t i=0; i<results.size(); ++i)
            {
                const android::String8& check = results.keyAt(i);
                CheckResData& rd = results.editValueAt(i);

                fprintf(out, "%s,%s", check.string(), rd.component.string());

                for (uint32_t r=0; r<numColumns; ++r)
                {
                    if (r < rd.res.size())
                    {
                        fprintf(out, ",%s", rd.res.itemAt(r).string());
                    }
                    else
                    {
                        fprintf(out, ",");
...
int HwcTestRunner::CreateTests()
{
    ...
    // Include -val_hwc_composition/-val_ivp_composition in the args logged by Jenkins
    mArgs += UsedArgs();
    ...
    fenceTimeoutMs = GetTimeParamUs("fence_timeout", fenceTimeoutMs * HWCVAL_MS_TO_US) / HWCVAL_MS_TO_US;
    ...
    if (mHWCLReplay || mDSReplay || mReplayTest)
    {
        ...
        if (mHWCLReplay)
        {
            ...
            replay = new Hwch::ReplayHWCLRunner(mInterface, mReplayFileName, mReplayMatch,
                mReplayNoTiming, mReplayNoProtected, mAlpha);
        }
        else if (mDSReplay)
        {
            replay = new Hwch::ReplayDSRunner(mInterface, mReplayFileName, mDSReplayNumFrames);
        }
        else if (mReplayTest)
        {
            // The unit-tests run on the HWCL regular expressions. Create a dummy HWCL runner
            // so that we can access the parser.
            replay = new Hwch::ReplayHWCLRunner(mInterface, "", 0, false, false, 0);
        }
        else
        {
            HWCERROR(eCheckCommandLineParam, "Unsupported sequence of replay command-line options");
            rc = -1;
        }

        replay->SetName(android::String8("Replay ") + mReplayFileName);

        if (replay->IsReady() || mReplayTest)
        {
            // Run the replay or run the unit-tests depending on the command-line options
            if (!mReplayTest)
            {
                mTests.add(replay);
            }
    ...
    else
    {
        ...
        if (mAllTests)
        {
            ...
            {
                android::String8 name(mTests.itemAt(i)->GetName());
                bool avoid = false;

                for (uint32_t j=0; j<mAvoidNames.size(); ++j)
                {
                    if (name == mAvoidNames.itemAt(j))
                    {
                        avoid = true;
                        break;
                    }
        ...
        else
        {
            ...
            {
                const android::String8& testName = mTestNames[i];
                Hwch::Test* test = Hwch::BaseReg::mHead->CreateTest(testName.string(), mInterface);
...
{
    // This is stuff that needs to be done after teh HwcTestKernel has been created.
    //
...
int HwcTestRunner::RunTests()
{
    ...
    android::String8 allTestNames;
    for (uint32_t i=0; i<mTests.size(); ++i)
    {
        Hwch::Test* test = mTests.itemAt(i);
    ...
    for (uint32_t i=0; i<mTests.size(); ++i)
    {
        ...
        HWCLOGA("============ Starting test %s ============", mTestName.string());
        if (mState->IsOptionEnabled(eOptKmsgLogging))
        {
        ...
        // Take copy of check priorities (severities) in results, so the test can amend them
        ...
        HWCLOGA("============ Finished test %s %s ============", mTestName.string(), testArgs.string());
        if (mState->IsOptionEnabled(eOptKmsgLogging))
        {
            mState->LogToKmsg("============ Finished test %s %s ============\n", mTestName.string(), testArgs.string());
        }

        ExitChecks();
        mRunTimeStat.Set(double(mEndTime - mStartTime) / HWCVAL_SEC_TO_NS);
        Hwcval::Statistics::getInstance().Dump(mStatsFile, mTestName.string());
    ...
    android::String8 parametersNotChecked;
    for (uint32_t i=0; i<mParams.size(); ++i)
    {
        Hwch::UserParam& up = mParams.editValueAt(i);
        const android::String8& pname = mParams.keyAt(i);
...
void HwcTestRunner::CRCTerminate(HwcTestConfig& config)
{
    if (config.mCheckConfigs[eCheckCRC].enable)
    {
        ...
        for (loop = 0; loop < cLoopMax && HwcTestState::getInstance()->IsFrameControlEnabled(); ++loop)
        {
        ...
        if (loop == cLoopMax)
        {
            HWCLOGW("HwcTestRunner::RunTests - ERROR: TIMED OUT (after %dms) WAITING FOR FRAME RELEASE", loop * cLoopWaitMilliseconds);
        }

        HWCLOGD("HwcTestRunner::RunTests - released after %dms", loop * cLoopWaitMilliseconds);
...
void HwcTestRunner::ReportVersion()
{
    ...
    {
        if (strcmp(hwcVersionWords[0], "VERSION:") == 0)
        {
            sha.setTo(hwcVersionWords[2]);
        }
        else
        {
            sha.setTo(hwcVersionWords[1]);
        }
    }

    if (sha != S(HWC_VERSION_GIT_SHA))
    {
        // We built against a different HWC source than which we are runnign against.
        if (mBrief || (GetParam("version") != 0))
        {
            printf("HWC VERSION INCONSISTENCY:\n");
            printf("HWC version (running):             %s\n", hwcBinVersion.string());
        ...
        if (!mBrief)
        {
            HwcGetTestConfig()->mCheckConfigs[eCheckHwcVersion].priority = ANDROID_LOG_WARN;
        }

        HWCERROR(eCheckHwcVersion, "Running HWC version:         %s\nHWCVAL uses include files in %s %s\n",
            hwcBinVersion.string(), S(HWC_VERSION_GIT_BRANCH), S(HWC_VERSION_GIT_SHA));
...
int main(int argc, char **argv)
{
    ...
    system.SetDefaultNumBuffers(runner.GetIntParam("default_num_buffers", HWCH_DEFAULT_NUM_BUFFERS));

    system.GetVSync().SetVSyncDelay(runner.GetTimeParamUs("vsync_delay", 6500)); // default 6.5 ms
    system.GetVSync().SetTimeout(runner.GetTimeParamUs("vsync_timeout", 50000)); // default 50 ms
    system.GetVSync().SetRequestedVSyncPeriod(runner.GetTimeParamUs("vsync_period", 16666)); // default 16.666 ms i.e. 60 Hz.

    system.GetKernelEventGenerator().SetEsdConnectorId(runner.GetIntParam("esd_connector_id", 0));
    ...
    else
    {
        ...
        // Note that when panel is turned off in Extended Mode, Vsyncs from display 1
