...
void HwcTestRunner::LogTestResult(const char* testName, const char* args) {
  ...
  result.Log(config, android::String8::format("%s %s", testName, args).string(),
             mBrief);
...
void HwcTestRunner::ParseCSV(const char* p,
                             android::Vector<android::String8>& sv) {
  while (p) {
    while (*p && !isprint(*p)) {
      ++p;
    }
  ...
  if (mHWCLReplay || mDSReplay || mReplayTest) {
    ...
    if (mAllTests) {
      ...
      for (uint32_t i = 0; i < mTestNames.size(); ++i) {
        ...
        Hwch::Test* test =
            Hwch::BaseReg::mHead->CreateTest(testName.string(), mInterface);
...
int HwcTestRunner::RunTests() {
  ...
  for (uint32_t i = 0; i < mTests.size(); ++i) {
    ...
    HWCLOGA("============ Finished test %s %s ============", mTestName.string(),
            testArgs.string());
    if (mState->IsOptionEnabled(eOptKmsgLogging)) {
...
void HwcTestRunner::ReportVersion() {
  ...
  if (sha != S(HWC_VERSION_GIT_SHA)) {
    ...
    HWCERROR(
        ...
        hwcBinVersion.string(), S(HWC_VERSION_GIT_BRANCH),
