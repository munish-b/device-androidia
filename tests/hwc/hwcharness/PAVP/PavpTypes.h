...
{
    PAVP_SESSION_GET_ANY_DECODE_SESSION_IN_USE = 0, // Input to check if any decode session is in use
...
typedef enum tagPAVP_GPU_REGISTER_TYPE
{
    ...
    PAVP_REGISTER_CONN_STATE_WRITE,         //!< Where to write a connection state value (pre-silicon)
    PAVP_SIM_DELIVER_SESSION_KEY,           //!< Where to write a new (Kb)KF1 to use for the session (pre-silicon)
    PAVP_SIM_WIDI_SESSION_KEY,                //!< Where to write a new (Kb)KF1 to use for the widi session (pre-silicon)
