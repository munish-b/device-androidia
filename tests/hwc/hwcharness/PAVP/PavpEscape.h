...
{
    PAVP_ESC_SYSTEM_PAVP_STATUS AvailablePavpMode;   // Indicates if system is heavy mode capable
} PAVP_ESC_QUERY_CAPS_PARAMS;
...
typedef struct tagPAVP_ESC_CONFIG_HW_KEY_EXCHANGE_PARAMS
{
    ...
    DWORD           PchCommandId; //BDW - To support PCH initiated commands submission to GPU over DMI channel
...
//
// Description:
//     This structure defined to return IsPavpInUse. TRUE if PAVP session is in use else FALSE
//---------------------------------------------------------------------------
...
typedef struct _PAVP_PROTECTION_INFO
{
    ...
    union {
        ...
        PAVP_ESC_WIDI_SET_STREAM_KEY_PARAMS     SetNewKey;
        PAVP_ESC_WIDI_GET_WIDI_STATUS_PARAMS    GetWiDiStatus;
        PAVP_ESC_WIDI_SET_WIDI_STATUS_PARAMS    SetWiDiStatus;
        //PAVP_ESC_WIDI_QUERY_WIDI_STATUS         QueryWidiStatus;
...
typedef enum
{
    ...
    GMM_ESCAPE_ACTION_32BIT_ALIAS,              // Create/destroy 32-bit alias. (OCL32 WA)
    GMM_ESCAPE_ACTION_GET_TRASH_PAGE_GFXADDR,   // Query gfx Address of the Trash Page
    GMM_ESCAPE_ACTION_GET_HEAP32_GFXADDR,       // Query gfx Address of the Heap32
    GMM_ESCAPE_ACTION_GET_SLM_ADDRESS_RANGE,    // Query OpenCL SLM gfx address range
    GMM_ESCAPE_ACTION_READ_PAGE_TABLE,          // Read PTE
    GMM_ESCAPE_ACTION_CONFIGURE_ADDRESS_SPACE,  // Configure device address space
    GMM_ESCAPE_ACTION_TILED_RESOURCE_OPERATION, // Create/Destroy/Map a sparse/tiled resource
    GMM_ESCAPE_ACTION_SVM_DESTROY_ALLOC,        // Pre-declare alloc to be "marked for death"
    GMM_ESCAPE_ACTION_SET_HW_PROTECTION_BIT,    // Set HW protection bit for serpent encryption according to given value. GMM_BLOCKDESC for the surface will be updated with the correct value of this bit.
    GMM_ESCAPE_ACTION_RES_UPDATE_AFTER_SHARED_OPEN, // Update ResInfo after a shared resource is opened
