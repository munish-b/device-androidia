...
void CoreUDestroy(COREUINTERFACE *Interface);

bool CoreUDoPavpAction(COREUINTERFACE *Interface, void *data,
                       uint32_t dataSize);

bool CoreUPavpSetMDRMMode(COREUINTERFACE *Interface,
                          bool bMultiPavpSessionSupport);

bool CoreUPavpGetMDRMMode(COREUINTERFACE *Interface,
                          bool *bMultiPavpSessionSupport);

bool CoreUPavpQuerySessionStatus(COREUINTERFACE *Interface,
                                 PAVP_SESSION_ID SessionID,
                                 PAVP_SESSION_STATUS *Status,
                                 KM_PAVP_SESSION_MODE *SessionMode,
                                 uint32_t *InstanceCount);

bool CoreUPavpChangeSession(COREUINTERFACE *Interface,
