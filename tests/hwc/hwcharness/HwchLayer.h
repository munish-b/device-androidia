...
namespace Hwch
{
    ...
    class Rect : public hwc_rect_t
    {
        public:
            Rect(uint32_t l=0, uint32_t t=0, uint32_t r=0, uint32_t b=0);
            void Rotate(RotationType rot, Display& display);
            const char* Str();
    };

    class FRect : public hwc_frect_t
    {
        public:
            FRect(float l=0.0, float t=0.0, float r=0.0, float b=0.0);
            FRect(double l, double t=0.0, double r=0.0, double b=0.0);
            FRect(uint32_t l, uint32_t t, uint32_t r, uint32_t b);
            const char* Str();
    };
    ...
    class Layer
    {
        public:
            ...
            android::sp<Pattern> mPattern;

            BufferSetPtr mBufs;   // current buffer

            LogCropRect           mLogicalCropf;
            FRect                 mSourceCropf;
            FRect                 mOldSourceCropf;
            LogDisplayRect        mLogicalDisplayFrame;
            Rect                  mDisplayFrame;
            Rect                  mOldDisplayFrame;
            android::Vector<Rect> mVisibleRegion;
        ...
        public:
            ...
            void FillExcluding(const hwc_rect_t& rect, const hwc_rect_t& exclRect);
