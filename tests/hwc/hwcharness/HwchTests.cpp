...
REGISTER_TEST(Game)
Hwch::GameTest::GameTest(Hwch::Interface& interface)
  : Hwch::Test(interface)
{
}
...
REGISTER_TEST(Home)
Hwch::HomeTest::HomeTest(Hwch::Interface& interface)
  : Hwch::Test(interface)
{
}
...
int Hwch::NV12FullVideo2Test::RunScenario()
{
...

    {
        ...
        for (uint32_t r=0; r<8; ++r)
        {
            ...
            {
                layer1.SetLogicalDisplayFrame(LogDisplayRect(0, 0, screenWidth, screenHeight));
            }
            else
            {
...
void Hwch::RotationAnimationTest::DoRotations(Hwch::Frame& frame,
    uint32_t num_rotations, uint32_t num_frames_to_send)
{
    ...
    {
        HWCLOGD("RotationAnimation: Rotating panel by 90 degrees clockwise" \
                "Rotation number %d of %d.", r+1, num_rotations);
        frame.RotateBy(eRotate90, true);
        frame.Send(num_frames_to_send);
    }

    HWCLOGD("RotationAnimation: Rotating panel by 180 degrees");
    frame.RotateBy(eRotate180, true);
    frame.Send(num_frames_to_send);

    HWCLOGD("RotationAnimation: Rotating panel by 270 degrees");
    frame.RotateBy(eRotate270, true);
    frame.Send(num_frames_to_send);
}
...
int Hwch::NV12PartVideoTest::RunScenario()
{
    ...
    layer3.SetLogicalDisplayFrame( LogDisplayRect(MaxRel(-779), 260, MaxRel(-20), 260+460));    // Scale the video into a popout window
    Hwch::StatusBarLayer layer4;
...
void Hwch::NV12PartVideo2Test::TestLayer(Hwch::Frame& frame, Hwch::Layer& layer)
{
    // NV12 layer with no scaling. For BXT, this should be put directly onto a plane
    layer.SetLogicalDisplayFrame(LogDisplayRect(0,yOffset,layerWidth,layerHeight+yOffset));
    frame.Send(numToSend);

    // Downscale layer to 75% (constant aspect-ratio) - should use plane scaler
    layer.SetLogicalDisplayFrame(LogDisplayRect(0,yOffset,layerWidth*0.75,(layerHeight*0.75)+yOffset));
    frame.Send(numToSend);

    // Downscale layer to 40% (constant aspect-ratio) - should use composition
    layer.SetLogicalDisplayFrame(LogDisplayRect(0,yOffset,layerWidth*0.40,(layerHeight*0.40)+yOffset));
    frame.Send(numToSend);

    // Downscale layer to 75% (different aspect-ratio) - should use plane scaler
    layer.SetLogicalDisplayFrame(LogDisplayRect(0,yOffset,layerWidth*0.75,(layerHeight*0.7)+yOffset));
    frame.Send(numToSend);

    // Downscale layer to 40% (different aspect-ratio) - should use composition
    layer.SetLogicalDisplayFrame(LogDisplayRect(0,yOffset,layerWidth*0.40,(layerHeight*0.35)+yOffset));
    frame.Send(numToSend);
}
...
int Hwch::NV12PartVideo2Test::RunScenario()
{
    ...
    {
        HWCERROR(eCheckScreenNotBigEnough, "Layer width (%d) is too big for panel in all rotations!", layerWidth);
        return 0;
    }
...
void Hwch::NetflixScaledTest::ScaleLayer(
    Hwch::Layer& layer,
...
int Hwch::MovieStudioTest::RunScenario()
{
    ...
    layer1.SetLogicalDisplayFrame(LogDisplayRect(145, 27, min(790, screenheight), MaxRel(-27)));
    layer1.SetBlending(HWC_BLENDING_NONE);

    Hwch::RGBALayer layer2(MaxRel(0), MaxRel(-eNavigationBarHeight), 1.0, ePurple, Alpha(eWhite, 16));
    layer2.SetTransform(eTransformRot270);
    layer2.SetCrop(Hwch::LogCropRect(0, 38, MaxRelF(0), MaxRelF(-eNavigationBarHeight)));
    layer2.SetLogicalDisplayFrame(LogDisplayRect(38, 0, MaxRel(-eNavigationBarHeight), MaxRel(0)));
    ...
    // This is the Nav bar. But I'm not using the NavigationBarLayer because we are in portrait mode and it seems that
    ...
    layer4.SetLogicalDisplayFrame(LogDisplayRect(MaxRel(-eNavigationBarHeight), 0, MaxRel(0), MaxRel(0)));
    ...
    // Calculate the y values for the video on HDMI to ensure it will fit on any screen.
    // For 1280x1024 case at least, this matches the original scenario.
#pragma GCC diagnostic ignored "-Wstrict-overflow"
    int32_t hdmiHeight = mSystem.GetDisplay(1).IsConnected() ? mSystem.GetDisplay(1).GetHeight() : 1024;
    ...
    hdmi1.SetLogicalDisplayFrame(LogDisplayRect(0, videoTop, MaxRel(0), videoTop + videoHeight));
...
int Hwch::FlipRotTest::RunScenario()
{
    ...
    layer1.SetTransform(eTransformFlipH);
    ...
    layer1.SetTransform(eTransformFlipV);
...
REGISTER_TEST(Smoke)
Hwch::SmokeTest::SmokeTest(Hwch::Interface& interface)
  : Hwch::Test(interface)
{
}

#define NOT_BRIEF(X) if (!IsOptionEnabled(eOptBrief)) {X}
...
int Hwch::SmokeTest::RunScenario()
{
    ...
    android::String8 suspendMethod (GetStrParam("screen_disable_method"));
    ...
    {
        layer3.SetLogicalDisplayFrame(LogDisplayRect(Scaled(220+i,1920),
                                                     Scaled(260-i,1280),
    ...
    // If it isn't we could use SetExpectedMode to indicate that clone mode is not expected
    // when a flipped layer is rotated
    // or simply set the expectation to eDontCare.
    layer11.SetTransform(eTransformFlipH);

    for (RotationType rot=eRotateNone; rot<eMaxRotate; ++rot)
    {
        frame.RotateTo(rot);
        frame.Send(30);
    }
...
int Hwch::PartCompTest::RunScenario()
{
    ...
    layer3.SetLogicalDisplayFrame(LogDisplayRect(Scaled(220+30,1920),
                                                 Scaled(260-30, 1280),
...
REGISTER_TEST(Png)
Hwch::PngTest::PngTest(Hwch::Interface& interface)
  : Hwch::Test(interface)
{
}
...
{
    const char *filename1 = "Spiderman.png";
...
Hwch::TransparencyCompositionTest::TransparencyCompositionTest(Hwch::Interface& interface)
    : Hwch::OptionalTest(interface)
{
}
...
int Hwch::TransparencyCompositionTest::RunScenario()
{
    ...
    const char *filename1 = "globe-scene-fish-bowl.png";
    ...
    for (int i = 0; i < NUM_LAYERS; i++)
    {
        ...
        layer[i]->SetLogicalDisplayFrame(LogDisplayRect(random_origin_x, random_origin_y,
                (random_origin_x+random_display_width), (random_origin_y+random_display_height)));
        ...
        if ((i & 1) == 0)
        {
            ...
            layer[i]->SetCrop(Hwch::LogCropRect(cropX, cropY, cropX + cropWidth, cropY + cropHeight));
        }
...
REGISTER_TEST(Skip)
Hwch::SkipTest::SkipTest(Hwch::Interface& interface)
    : Hwch::Test(interface)
{
}
...
int Hwch::PanelFitterStressTest::RunScenario()
{
    ...
    // Use -source_landscape on a portrait mode device if you want the source to be 16x9 aspect ratio,
    ...
    Hwch::RGBALayer layer1(screenWidth, screenHeight + topMargin, 60.0, eRed, eGreen, ePurple);
    layer1.SetLogicalDisplayFrame(LogDisplayRect(0, 0, screenWidth, screenHeight));
    frame.Add(layer1);
